/* ==================== СКЛАД: начальные данные ==================== */
const WAREHOUSE_SEED = {"parts":[{"code":"AB.00.0009.14","name":"Реанимотор","qty":0},{"code":"025JB50-01-0","name":"Подножка (wind 5.0)","qty":0},{"code":"125HL50-01-0","name":"Дека / Рама большая (wind 25)","qty":0},{"code":"141HL50-02-0","name":"Крышка аккумуляторного отсека (Дека) (Wind 25)","qty":0},{"code":"145QY50-01-0","name":"Резиновая накладка крышки батарейного отсека (квадратная)","qty":0},{"code":"145ZD50-01-0","name":"Резиновая накладка на крышку батарейного отсека","qty":0},{"code":"145QY50-02-0","name":"Резиновый коврик крышки батарейного отсека (с точками)","qty":0},{"code":"148ZD50-01-0","name":"Амортизирующая накладка на крышку батарейного отсека","qty":500},{"code":"148QY50-01-0","name":"Прокладка крышки батарейного отсека","qty":0},{"code":"150XH50-02-0","name":"Задний держатель аккумулятора (wind 5.0)","qty":0},{"code":"150XH50-01-0","name":"Передний держатель аккумулятора (wind 5.0)","qty":0},{"code":"153QY50-01-0","name":"Накладка крышки деки","qty":0},{"code":"160JB50-01-0","name":"Задняя защита деки (Rear curved deck)","qty":0},{"code":"161XH50-01-0","name":"Передняя защита деки","qty":0},{"code":"1630700865","name":"Кнопка старт/стоп","qty":0},{"code":"164ZD50-01-0","name":"Внешняя водонепроницаемая станция","qty":0},{"code":"164QY50-01-0","name":"Резиновая заглушка на разъём зарядки контроллера","qty":0},{"code":"169XH50-01-A","name":"Комплект накладок на заднее крыло (W5.0)","qty":0},{"code":"170QY50-01-0","name":"Накладка заднего крыла","qty":0},{"code":"170ZD50-01-0","name":"Пылезащитная планка","qty":0},{"code":"182ZD50-01-0","name":"Антивибрационная прокладка АКБ","qty":0},{"code":"182QY50-01-0","name":"Уплотнительная резинка для аккумуляторного отсека","qty":0},{"code":"183YX50-01-0","name":"Фиксатор","qty":0},{"code":"185YX50-01-0","name":"Держатель крепления маяка (Wind25)","qty":0},{"code":"195XH50-03-0","name":"Крышка кабеля мотора (тормозная линия)","qty":0},{"code":"195XH50-02-0","name":"Крышка кабеля стоп-сигнала / мотор-колеса","qty":0},{"code":"205AN50-01-0","name":"IOT (25)","qty":0},{"code":"210NM50-01-0","name":"Силиконовая теплоотводящая прокладка / силикон для контроллера","qty":0},{"code":"216ST36-02-0","name":"Контроллер (wind 5.0)","qty":0},{"code":"234JY35-01-0-01","name":"Обод (ромашка) заднего колеса","qty":0},{"code":"235JY36-01-0 / 234XD35-01-0","name":"Мотор-колесо (с покрышкой)","qty":0},{"code":"261DP40-01-0 / 261WX50-01-0","name":"Курок газа (электро-выключатель)","qty":0},{"code":"262TZ50-01-0","name":"Рычаг тормоза левый (Wind 25)","qty":0},{"code":"262TZ50-02-0","name":"Рычаг тормоза правый (Wind 25)","qty":0},{"code":"264JL50-02-0","name":"Кабель замка стойки","qty":0},{"code":"264JL50-04-0","name":"Кабель кнопки (Wind 5.0)","qty":0},{"code":"264JL50-03-0","name":"Кабель постоянного тока","qty":0},{"code":"264JL50-05-0","name":"Кабель стоп-сигнала","qty":0},{"code":"264JL50-01-0","name":"Основной кабель / центральник (Main Cable)","qty":0},{"code":"266JK50-01-0","name":"Замок аккумуляторного отсека (Battery lock)","qty":0},{"code":"268JK50-01-0","name":"Кнопка Я (Wind 5.0)","qty":0},{"code":"272JK50-01-0","name":"Замок стойки (цепной замок)","qty":0},{"code":"277BK50-01-0","name":"QR-код (Wind 5.0)","qty":0},{"code":"279HQ50-01-0","name":"Держатель замка","qty":0},{"code":"279CR50-01-0","name":"Заглушка держателя замка","qty":0},{"code":"279GF50-01-0","name":"Трос замка стойки","qty":0},{"code":"280JK50-01-0","name":"Дисплей / дашборд (Display)","qty":0},{"code":"281BS50-01-0","name":"Датчик (Beacon)","qty":0},{"code":"326CD50-01-0","name":"Передний амортизатор левый (Wind 25)","qty":0},{"code":"326CD50-02-0","name":"Передний амортизатор правый (Wind 25)","qty":0},{"code":"329HL50-01-0","name":"Траверса / шток передней вилки (Wind 25)","qty":0},{"code":"405HL50-02-0","name":"Рулевая планка (Wind 25)","qty":0},{"code":"416XH50-01-0","name":"Крышка фары (Wind 25)","qty":0},{"code":"419XH50-01-0","name":"Крышка держателя (Wind 25)","qty":0},{"code":"423XH22-11-0","name":"Левая ручка руля / грипс (Wind 25)","qty":0},{"code":"423XH22-12-0","name":"Правая ручка руля / грипс (Wind 25)","qty":0},{"code":"425XH22-03-0","name":"Заглушка грипс","qty":0},{"code":"426QY22-01-0","name":"Силиконовая наклейка «Я»","qty":0},{"code":"427HL22-01-0","name":"Ограничитель грипс (Wind 25)","qty":0},{"code":"434HL50-01-0","name":"Фиксатор рулевой планки / зажим корпуса переднего фонаря (Wind 25)","qty":0},{"code":"443HL50-01-0","name":"Стойка (wind 5.0)","qty":0},{"code":"446JB50-01-0","name":"Зажим главной трубки / зажим стойки (wind 5.0)","qty":0},{"code":"446XH50-01-0","name":"Комплект фиксаторов номерного знака передняя Wind 5.0","qty":0},{"code":"448HL50-01-0","name":"Закладная для парковочного замка / деталь замка на стойку (wind 25)","qty":0},{"code":"448XH50-01-0","name":"Крепление кабеля","qty":0},{"code":"448XH50-02-0","name":"Крюк (держатель)","qty":0},{"code":"456JY50-01-0","name":"Подшипник вилки (Fork bearing) (wind25)","qty":0},{"code":"457DL50-01-0","name":"Винт стойки крепления вилки","qty":0},{"code":"458HL50-01-0","name":"Фиксатор передней вилки / стакан стойки (wind 25)","qty":0},{"code":"471XX22-02-0","name":"Звонок","qty":0},{"code":"484XH50-01-0","name":"Держатель телефона (wind 5.0)","qty":0},{"code":"513TZ50-02-0","name":"Передний тормозной трос (Wind 25)","qty":0},{"code":"514TZ50-01-0","name":"Задний тормозной трос (Wind 25)","qty":0},{"code":"523TZ80-07-0","name":"Колодка передняя с барабаном NEW","qty":0},{"code":"524TZ80-01-0-01","name":"Колодка зад без барабана","qty":0},{"code":"524TZ80-05-0","name":"Колодка задняя с барабаном NEW","qty":0},{"code":"531XX40-01-0 / 531SQ40-01-0","name":"Левый защитный кожух тормоза (Wind 25)","qty":0},{"code":"531XX40-02-0 / 531SQ40-02-0","name":"Правый защитный кожух тормоза (Wind 25)","qty":0},{"code":"603LK10-01-0","name":"Покрышка задняя","qty":0},{"code":"604LK12-01-0","name":"Покрышка передняя","qty":0},{"code":"613TZ12-02-0","name":"Ступица переднего колеса (wind 5.0)","qty":0},{"code":"633XH50-01-0","name":"Переднее крыло (Wind 25)","qty":0},{"code":"643XH50-01-0","name":"Заднее крыло (Rear Fender)","qty":0},{"code":"659XH50-01-0","name":"Защитная крышка (бабыха) (Wind 25)","qty":0},{"code":"703CC50-01-0","name":"Передний фонарь (Head light)","qty":0},{"code":"723SP50-01-0","name":"Задний стоп-сигнал","qty":0},{"code":"743SP40-01-0","name":"Передний светоотражатель","qty":0},{"code":"744XH50-01-0","name":"Крепежный элемент для переднего светоотражателя","qty":0},{"code":"744XH45-01-0","name":"Крепежный элемент переднего отражателя (Wind 5.0)","qty":0},{"code":"763KG50-01-0","name":"Боковой светоотражатель (оранжевый)","qty":0},{"code":"773XD50-01-0","name":"Status LED индикатор состояния","qty":0},{"code":"832YX12-05-0","name":"Передняя ось колеса (Wind25)","qty":0},{"code":"902XL50-01-0","name":"Коробка","qty":0},{"code":"903XL50-01-0","name":"Упаковочный материал","qty":0},{"code":"932WM20-02-0","name":"Наклейка для передней деки","qty":0},{"code":"932WM50-01-0","name":"Наклейка на номерной знак","qty":0},{"code":"934HL50-01-0","name":"Информационный стикер","qty":0},{"code":"935WM50-01-0","name":"Наклейка для держателя телефона","qty":0},{"code":"938WM50-01-0","name":"Наклейка номер на раму (большая)","qty":0},{"code":"938WM50-02-0","name":"Наклейка передней панели рамы (малая)","qty":0},{"code":"947XX20-01-0","name":"Рамка номерного знака","qty":0},{"code":"960YX50-01-0","name":"Ключ от секретной двери","qty":0},{"code":"961YX50-01-0","name":"Установка запорного блока цепного замка","qty":0},{"code":"6003W","name":"Подшипник 6003 VV","qty":0},{"code":"NBR70\\2","name":"Сальник манжета армированная 20х35х7","qty":0}],"consumables":[{"code":"274HL50-01-0","name":"Лента на липучке","qty":0},{"code":"651YX50-01-0","name":"Стопорная шайба мотор-колеса","qty":0},{"code":"800YX10-03-0","name":"Винт M10x8","qty":0},{"code":"803YX03-09-0","name":"Винт с потайной головкой M3x9","qty":0},{"code":"804YX04-06-0","name":"Винт M4x11","qty":0},{"code":"804YX04-08-0","name":"Винт M4x11","qty":0},{"code":"804YX04-09-0","name":"Винт M4x16","qty":0},{"code":"804YX04-10-0","name":"Винт M4x15","qty":0},{"code":"804YX04-19-0","name":"Винт с потайной головкой M4x20","qty":0},{"code":"804YX04-20-0","name":"Винт с потайной головкой M4x26,5","qty":0},{"code":"804YX04-21-0","name":"Винт цилиндрический M4x35","qty":0},{"code":"804YX04-23-0","name":"Заклепка на номера","qty":0},{"code":"804YX04-0X-0","name":"Винт (код неразборчив)","qty":0},{"code":"805YX05-02-0","name":"Винт M5x18","qty":0},{"code":"805YX05-10-0","name":"Винт M5x14","qty":0},{"code":"805YX05-18-0","name":"Винт M5x14","qty":0},{"code":"805YX05-25-0","name":"Винт M5x20","qty":0},{"code":"806YX06-03-0","name":"Винт M6x25","qty":0},{"code":"806YX06-07-0","name":"Винт M6x20","qty":0},{"code":"808YX08-01-0","name":"Винт M8x25","qty":0},{"code":"808YX08-03-0","name":"Винт M8x30","qty":0},{"code":"808YX08-11-0","name":"Винт M8x25","qty":0},{"code":"812YX04-01-0","name":"Гайка оси переднего колеса M4","qty":0},{"code":"813YX05-01-0","name":"Гайка M5","qty":0},{"code":"817YX12-01-0","name":"Гайка M12 (пятиугольная, задняя ось)","qty":0},{"code":"817YX12-02-0","name":"Гайка M12 (передняя ось)","qty":0},{"code":"820ST08-01-0","name":"Стопорная шайба с насечками M8","qty":0},{"code":"820ST10-01-0","name":"Стопорная шайба с насечками M10","qty":0},{"code":"820ST12-01-0","name":"Стопорная шайба с насечками M12","qty":0},{"code":"822YX23-01-0","name":"Шайба большая","qty":0},{"code":"824YX04-03-0","name":"Прокладка M10","qty":0},{"code":"911YX09-01-0","name":"Бита T4","qty":0},{"code":"911YX12-01-0","name":"Бита T3","qty":0},{"code":"911YX17-01-0","name":"Бита пятиугольная","qty":0},{"code":"911YX50-01-0","name":"Бита T5","qty":0},{"code":"913YX20-01-0","name":"Бита T20x70","qty":0},{"code":"915YX27-01-0","name":"Бита T27x50","qty":0},{"code":"916YX40-01-0","name":"Бита T40x50","qty":0},{"code":"916YX40-02-0","name":"Бита T40x90","qty":0},{"code":"918YX45-01-0","name":"Бита T45x50","qty":0},{"code":"15611179","name":"Космофен","qty":0},{"code":"16529751","name":"Респиратор","qty":0},{"code":"22754961","name":"Ros Fix спрей-активатор","qty":0},{"code":"18131749","name":"Жидкий ключ «Валера»","qty":0},{"code":"25072-000","name":"Саморез DIN 4,2x13 по металлу","qty":0},{"code":"23057367","name":"Эмаль черная","qty":0},{"code":"00-00002861","name":"Защитная пластина номера","qty":0}]};
const ONEC_SEED = {
  parts: [
    { code: "524TZ80-01-0-01", name: "Accessory of Rear Brake Задняя колодка без барабана", qty: 680 },
    { code: "266JK50-01-0", name: "Battery lock Замок аккумуляторного отсека (Wind 5.0)", qty: 140 },
    { code: "471XX22-01-0", name: "Bell Звонок", qty: 390 },
    { code: "723SP50-01-0", name: "Brake light Задний стоп сигнал", qty: 34 },
    { code: "820ST06-01-0", name: "Disk-lock Gasket Стопорная шайба с насечками М6", qty: 3639 },
    { code: "280JK50-01-0", name: "Display Дисплей", qty: 113 },
    { code: "825YX05-01-0", name: "Flat gasket Плоская шайба D12", qty: 1301 },
    { code: "456JY50-01-0", name: "Fork bearing Подшипник вилки (Wind 25)", qty: 986 },
    { code: "150XH50-01-0", name: "Front battery holder Передний держатель аккумулятора", qty: 196 },
    { code: "161XH50-01-0", name: "Front curved deck Передняя защита деки", qty: 18 },
    { code: "744XH50-01-0", name: "Front reflector holder Крепежный элемент для переднего светоотражателя", qty: 77 },
    { code: "261DP40-01-0", name: "Gas Throttle/Accelerator Курок газа", qty: 729 },
    { code: "703CC50-01-0", name: "Head light Передний фонарь", qty: 36 },
    { code: "914YX25-02-0", name: "Heptagon Screw Driver Bit Бита T25*100 длинные и короткие Медного цвета (Китай)", qty: 80 },
    { code: "205AN50-01-0", name: "IOT (25)", qty: 231 },
    { code: "204BY40-02-0", name: "IOT Устройство управления самоката Wind 4.0", qty: 1 },
    { code: "262DP40-01-0-01", name: "Left Brake Lever Левая ручка тормоза", qty: 478 },
    { code: "531SQ40-01-0", name: "Left Brake Protect Cover Защитная крышка левого тормаза", qty: 82 },
    { code: "264JL50-01-0", name: "Main Cable Основной кабель", qty: 22 },
    { code: "234JY35-01-0-04", name: "Motor PCBA Плата PCBA Заднего колеса с двигателем", qty: 60 },
    { code: "234JY35-01-0-01", name: "Motor Wheel Hub Cap Обод \"Ромашка\" Заднего колеса с двигателем", qty: 72 },
    { code: "234JY35-01-0-02", name: "Motor Wheel Hub(Magnet) Чаша с магнитами Заднего колеса с двигателем", qty: 360 },
    { code: "840BY05-01-0", name: "Nylon cable ties Нейлоновая стяжка", qty: 3395 },
    { code: "960YX50-01-0", name: "Pentagon Socket Wrench Ключ от секретной двери", qty: 7 },
    { code: "160JB50-01-0", name: "Rear curved deck Задняя защита деки", qty: 38 },
    { code: "643XH50-01-0", name: "Rear Frender Заднее крыло (Wind 5.0)", qty: 108 },
    { code: "262DP40-02-0-01", name: "Right Brake Lever Правая ручка тормоза", qty: 278 },
    { code: "531SQ40-02-0", name: "Right Brake Protect Cover Защитная крышка правого тормаза", qty: 160 },
    { code: "6003W", name: "RU Подшипник 6003 VV", qty: 150 },
    { code: "NBR70\\2", name: "RU Сальник манжета армированная 20х35х7", qty: 93 },
    { code: "806YX06-02-0", name: "Screw Винт М6х20", qty: 1000 },
    { code: "841JT10-01-0", name: "Stainless steel collar Стальная стяжка", qty: 139 },
    { code: "773XD50-01-0", name: "Status LED Индикатор состояния", qty: 15 },
    { code: "261WX50-01-0", name: "Throttle Курок газа", qty: 521 },
    { code: "148ZD50-01-0", name: "Амортизирующая накладка на крышку батарейного отсека (Wind 5.0)", qty: 100 },
    { code: "182ZD50-01-0", name: "Антивибрационная прокладка АКБ", qty: 100 },
    { code: "523TZ80-07-0", name: "Барабан передний (Wind 25)", qty: 118 },
    { code: "763KG50-01-0", name: "Боковой светоотражатель", qty: 1053 },
    { code: "910KN71-01-0", name: "Бокорезы", qty: 19 },
    { code: "457DL50-01-0", name: "Винт крепления стойки  передней вилки (Wind 5.0)", qty: 304 },
    { code: "164ZD50-01-0", name: "Внешняя водонепроницаемая зарядная станция (W5.0)", qty: 194 },
    { code: "125HL50-01-0", name: "Дека (Wind 25)", qty: 10 },
    { code: "185YX50-01-0", name: "Держатель крепления маяка (Wind 25)", qty: 46 },
    { code: "484XH50-01-0", name: "Держатель телефона (Wind 5.0)", qty: 6 },
    { code: "524TZ80-05-0", name: "Задний барабан (Wind 5.0)", qty: 20 },
    { code: "150XH50-02-0", name: "Задний держатель аккумулятора (Wind 5.0)", qty: 241 },
    { code: "514TZ50-01-0", name: "Задний тормозной трос (Wind 25)", qty: 123 },
    { code: "446JB50-01-0", name: "Зажим главной трубки (Wind 5.0)", qty: 86 },
    { code: "448HL50-01-0", name: "Закладная для крепления парковочного замка (Wind 25)", qty: 22 },
    { code: "659XH50-01-0", name: "Защитная крышка (бабыха) (Wind 25)", qty: 378 },
    { code: "264JL50-04-0", name: "Кабель кнопки (Wind 5.0)", qty: 193 },
    { code: "264JL50-05-0", name: "Кабель стоп сигнала (Wind 5.0)", qty: 346 },
    { code: "268JK50-01-0", name: "Кнопочный переключатель фары (Wind 5.0)", qty: 762 },
    { code: "169XH50-01-A", name: "Комплект накладок на заднее крыло (W5.0)", qty: 446 },
    { code: "446XH50-01-0", name: "Комплект фиксаторов номерного знака (Wind 5.0)", qty: 195 },
    { code: "216ST36-02-0", name: "Контроллер (wind 5.0)", qty: 7 },
    { code: "141HL50-02-0", name: "Крышка аккумуляторного отсека (Дека)(Wind 25)", qty: 102 },
    { code: "419XH50-01-0", name: "Крышка держателя (Wind 25)", qty: 83 },
    { code: "195XH50-03-0", name: "Крышка томозного троса (Wind 5.0)", qty: 594 },
    { code: "416XH50-01-0", name: "Крышка фары (Wind 25)", qty: 84 },
    { code: "423XH22-11-0", name: "Левая ручка руля (Wind 25)", qty: 188 },
    { code: "531XX40-01-0", name: "Левый защитный кожух тормоза (Wind 25)", qty: 365 },
    { code: "910TB76-01-0", name: "Натяжитель тормоза", qty: 44 },
    { code: "427HL22-01-0", name: "Ограничитель грипс (Wind 25)", qty: 88 },
    { code: "633XH50-01-0", name: "Переднее крыло (Wind 25)", qty: 133 },
    { code: "326CD50-01-0", name: "Передний амортизатор левый (Wind 25)", qty: 44 },
    { code: "326CD50-02-0", name: "Передний амортизатор правый (Wind 25)", qty: 45 },
    { code: "743SP40-01-0", name: "Передний светоотражатель", qty: 389 },
    { code: "513TZ50-02-0", name: "Передний тормозной трос (Wind 25)", qty: 165 },
    { code: "832YX12-05-0", name: "Передняя ось колеса (Wind 25)", qty: 280 },
    { code: "025JB50-01-0", name: "Подножка (Wind 5.0)", qty: 146 },
    { code: "603LK10-01-0", name: "Покрышка задняя", qty: 634 },
    { code: "604LK12-01-0", name: "Покрышка передняя", qty: 88 },
    { code: "423XH22-12-0", name: "Правая ручка руля (Wind 25)", qty: 176 },
    { code: "531XX40-02-0", name: "Правый защитный кожух тормоза (Wind 25)", qty: 354 },
    { code: "170ZD50-01-0", name: "Пылезащитная планка (W5.0)", qty: 50 },
    { code: "AB.00.0009.14", name: "Реаниматор (22)", qty: 19 },
    { code: "145ZD50-01-0", name: "Резиновая накладка на крышку батарейного отсека (Wind 5.0)", qty: 100 },
    { code: "405HL50-02-0", name: "Рулевая планка (Wind 25)", qty: 101 },
    { code: "262TZ50-01-0", name: "Рычаг тормоза левый (Wind 25)", qty: 116 },
    { code: "262TZ50-02-0", name: "Рычаг тормоза правый (Wind 25)", qty: 127 },
    { code: "210NM50-01-0", name: "Силиконовая теплоотводящая прокладка (W5.0)", qty: 120 },
    { code: "443HL50-01-0", name: "Стойка (Wind 5.0)", qty: 66 },
    { code: "613TZ12-02-0", name: "Ступица переднего колеса (Wind 5.0)", qty: 141 },
    { code: "329HL50-01-0", name: "Траверса передней вилки (Wind 25)", qty: 129 },
    { code: "458HL50-01-0", name: "Фиксатор передней вилки (Wind 25)", qty: 85 },
    { code: "434HL50-01-0", name: "Фиксатор рулевой планки (Wind 25)", qty: 38 },
    { code: "14.01.0304.00", name: "Винт М6Х20", qty: 0 },
    { code: "14.01.0362.00", name: "Водонепроницаемая заглушка для рулевой трубы (23)", qty: 0 },
    { code: "142ZD40-01-0", name: "Battery Compartment Cover Pad Grip Центральный коврик", qty: 30 },
    { code: "454LB40-01-0", name: "Bearing Подшипник", qty: 117 },
    { code: "162ZD40-01-0", name: "Curved Deck Base Grip Передний коврик", qty: 14 },
    { code: "214ST36-01-0", name: "ECU Контроллер управления самоката (Wind 4.0)", qty: 30, model: "YGW 4.0" },
    { code: "266ZH40-01-0", name: "Electronic Battery Lock Assembly Замок", qty: 70 },
    { code: "168ZD40-01-0", name: "Grip for Tail Seat Cover Pad Задний коврик", qty: 14 },
    { code: "448SQ55-01-0", name: "Hook Крючок из пластмассы для сумки (Wind 4.0)", qty: 20, model: "YGW 4.0" },
    { code: "024HJ40-01-0", name: "Kickstand Ножка-подставка электросамоката Wind 4.0", qty: 66, model: "YGW 4.0" },
    { code: "783XD40-01-0", name: "Left Side Lamp Боковой левый индикатор", qty: 12 },
    { code: "940DM40-01-0", name: "License Plate Номерной знак", qty: 550 },
    { code: "264CH40-01-0", name: "Main Cable Основной кабель", qty: 30 },
    { code: "234XD35-01-0", name: "Motor Заднее колесо с двигателем", qty: 50 },
    { code: "484RX40-01-0", name: "Phone Holder Держатель телефона", qty: 80 },
    { code: "643SQ40-01-0", name: "Rear Frender Заднее крыло", qty: 90 },
    { code: "193LY40-01-0", name: "Rear Shock Absorber Задний амортизатор", qty: 60 },
    { code: "783XD40-02-0", name: "Right Side Lamp Боковой правый индикатор", qty: 11 },
    { code: "16004W", name: "RU Подшипник 16004 VV", qty: 80 },
    { code: "174XH40-01-0", name: "Scooter Frame Left Side Cover Левая защита бокового индикатора", qty: 30 },
    { code: "174XH40-02-0", name: "Scooter Frame Right Side Cover Правая защита бокового индикатора", qty: 30 },
    { code: "524TZ80-01-0", name: "Барабан задний (Wind 4.0)", qty: 96, model: "YGW 4.0" },
    { code: "14.01.0605.00", name: "Болт вилки (22)", qty: 0 },
    { code: "14.01.0578.00", name: "Винт (М6Х14)", qty: 0 },
    { code: "805YX05-02-0", name: "Винт М5х18", qty: 300 },
    { code: "195XH50-02-0", name: "Крышка кабеля мотор колеса (Wind 5.0)", qty: 440, model: "YGW 5.0" },
    { code: "14.01.0378.00", name: "Пылезащитная крышка (Шайба) (22)", qty: 0 },
    { code: "AB.16.02.00.3903", name: "Рама самоката (25)", qty: 31, model: "YGW 5.0" },
    { code: "14.01.0373.00", name: "Цилиндрический крепежный элемент", qty: 0 }
  ],
  consumables: [
    { code: "15945947", name: "A-VM вафельное полотно 40см*50см VL-5401", qty: 14 },
    { code: "37335648", name: "A1 Medium CUT FPD 125 мм полировальный круг твердый синий", qty: 2 },
    { code: "17465982", name: "ACG Пистолет для пеногенератора 75см (пластиковая ручка)", qty: 10 },
    { code: "15983294", name: "AIRLINE Лопатка монтажная \"пика\" 850мм", qty: 1 },
    { code: "16211941", name: "APEX Сгон для воды 55см", qty: 2 },
    { code: "17465982", name: "Arnezi Головка 1/2 12-и гр. 14мм", qty: 10 },
    { code: "19496706", name: "Astrohim автошампунь для бесконтактной мойки STANDARD концентрат 20 кг АС3013", qty: 10 },
    { code: "16343272", name: "Attache ножницы 195мм с пласт прорезиненными ручками цвет бирюзово/черный остр 47585", qty: 2 },
    { code: "18859512", name: "AutoExpert шланг соеденительный высокого давления K-Q10m для моек К2-К7", qty: 1 },
    { code: "18859584", name: "AutoExpert шланг соединительный высокого давления К-Q15м для моек Karcher серии К2-К7", qty: 2 },
    { code: "15634086", name: "Berger Головка свечная 12-гранная 1/2 14мм", qty: 1 },
    { code: "16074068", name: "BIBICARE Силиконовая смазка 400 мл", qty: 1 },
    { code: "33777043", name: "Brabix Кресло стул, хром, пластик черный, экокожа черная", qty: 2 },
    { code: "27927242", name: "BRAUBERG Блокнот А5 146х206 мм Хищники", qty: 2 },
    { code: "15867162", name: "BRAUBERG Клейкая лента 48мм х 66м, упаковочная, желтая, гарантированная длина, 45мкм", qty: 3 },
    { code: "16728631", name: "BRAUBERG Маркер перманентный нестираемый Classic черный 10шт", qty: 3 },
    { code: "16933165", name: "BRAUBERG Маркер-краска лаковый синий 4мм", qty: 3 },
    { code: "15995934", name: "BRAUBERG Резинки банковские универсальные 200г", qty: 5 },
    { code: "35855718", name: "Cablexpert Сетевой фильтр 10А, 20Вт белый", qty: 2 },
    { code: "15611179", name: "COSMO Цианоакрилатный клей СА-12 20гр СА-500.200-(20)", qty: 47 },
    { code: "15973570", name: "CROWN Точильный станок", qty: 1 },
    { code: "15499303", name: "CUTOP диск профессиональный шлифовальный по металлу", qty: 5 },
    { code: "23057367", name: "Decorix Аэрозольная эмаль универсальная акриловая черный матовый 520мл", qty: 276 },
    { code: "15942721", name: "Delta-Park Органайзер OR-9", qty: 1 },
    { code: "37853082", name: "DEWALT Набор сверл по металлу 1-10мм 19шт", qty: 1 },
    { code: "22065862", name: "ECO Фильтр тонкой очистки большой для очистителя высокого давления HPW-X20011", qty: 5 },
    { code: "18573235", name: "ECOLUX Жидкое стекло Натриевое 1.3 кг", qty: 1 },
    { code: "30430754", name: "EVOLITE Антиграффити", qty: 2 },
    { code: "22117803", name: "Fachmann Шланг садовый 50м", qty: 1 },
    { code: "30784737", name: "FLYSEA Маркер-краска синий FS-120 blue", qty: 2 },
    { code: "15897831", name: "FORCE Головка длинная 8мм", qty: 5 },
    { code: "15917968", name: "FORCE держатель бит 6мм", qty: 2 },
    { code: "31313211", name: "Forsage Стул слесарный передвижной", qty: 1 },
    { code: "16529751", name: "FORT Респиратор лепесткового типа с клапаном выдоха KN95 FFP2", qty: 27 },
    { code: "18813432", name: "Forward Gear смазка синяя ЕР-2 ведро пластик 0.9 кг", qty: 5 },
    { code: "16664498", name: "FOXWELD AERO шланг воздушный с фитингами рапид 20бар 9х15мм 5м", qty: 1 },
    { code: "27164952", name: "GIGANT вафельное полотно 40см х 50м 125гр\\м2", qty: 2 },
    { code: "15558584", name: "Gigant Измерительная рулетка", qty: 2 },
    { code: "34952491", name: "Gigant изолента ПВХ 15*20м синяя", qty: 40 },
    { code: "15558504", name: "GIGANT Инструмент для снятия изоляции 200мм", qty: 1 },
    { code: "16034064", name: "Gigant Клещи переставные 250мм", qty: 1 },
    { code: "15913322", name: "Gigant Комбинированный ключ 8мм", qty: 6 },
    { code: "15949428", name: "Gigant кордщетка-кисть для дрели 25мм", qty: 1 },
    { code: "15948784", name: "Gigant Кувалда 1500 грамм с фибергласовой рукояткой", qty: 2 },
    { code: "19499610", name: "Gigant Лента клейкая малярная 19мм 50м GIP-28", qty: 9 },
    { code: "19499712", name: "Gigant Лента клеящая 48мм х 140м", qty: 3 },
    { code: "31313229", name: "Gigant Лом-гвоздодер усиленный", qty: 1 },
    { code: "15558509", name: "Gigant Отвертка PH1x100 с магнитным наконечником GS PH1100", qty: 6 },
    { code: "15558691", name: "GIGANT Отвертка с магнитным наконечником GS SL 6x75", qty: 3 },
    { code: "15894844", name: "Gigant Перчатки маслобензостойкие Гранат 12 пар в упаковке", qty: 4 },
    { code: "20133226", name: "GIGANT Пинцет прямой 120мм", qty: 1 },
    { code: "16213293", name: "Gigant Пистолетная быстрозажимная струбцина 300мм GPQC 300", qty: 2 },
    { code: "15774974", name: "Gigant Пластиковая воронка", qty: 2 },
    { code: "15872268", name: "Gigant плоская кисть 25мм FB-25 упаковка 10шт", qty: 1 },
    { code: "16661348", name: "Gigant Припой Sn60 Pb40 трубка 2.0мм 100г GT-095", qty: 1 },
    { code: "16014008", name: "Gigant Ручной заклепочник 2.4-4.8 GHR-48", qty: 2 },
    { code: "38774848", name: "Gigant Сетевой фильтр с заземлением, 6 розеток", qty: 2 },
    { code: "19499736", name: "GIGANT Сигнальная лента желто-черная 50ммх33м", qty: 1 },
    { code: "16167680", name: "Gigant Смазка силиконовая морозостойкая 400 мл, GSGR-400", qty: 3 },
    { code: "19959208", name: "Gigant Строительный нож металлический корпус 18мм GWK632", qty: 3 },
    { code: "32602548", name: "Gigant Тефлоновая смазка 520мл GTL-520", qty: 1 },
    { code: "19499598", name: "Gigant упаковочная клейкая лента 48мм *100м GIT-36", qty: 3 },
    { code: "15854429", name: "Gigant усиленная страховочная подставка 3Т 2шт", qty: 1 },
    { code: "26661278", name: "Gigant шланг пневматический маслостойкий 10м 8х14мм", qty: 1 },
    { code: "20715519", name: "Gigant Ящик для инструментов", qty: 2 },
    { code: "38360166", name: "GP Батарейка крона 9V", qty: 2 },
    { code: "15783858", name: "GP Батарейки мизинчиковые алкалиновые ААА набор 10шт", qty: 2 },
    { code: "15783860", name: "GP Батарейки мизинчиковые алкалиновые ААА набор 30шт", qty: 1 },
    { code: "15783864", name: "GP Батарейки пальчиковые алкалиновые AA набор 10шт", qty: 1 },
    { code: "13946310", name: "GROSS Набор отверток 6шт", qty: 1 },
    { code: "33907783", name: "Handskit Отсос припоя электрический GS-108", qty: 1 },
    { code: "35121013", name: "Hanskonner Дрель аккумуляторная б/щ 1BatterySystem HCD2065C", qty: 1 },
    { code: "913YX20-01-0", name: "Heptagon Screw Driver Bit Бита T20*70 длинные и короткие Медного цвета (Китай)", qty: 83 },
    { code: "915YX27-01-0", name: "Heptagon Screw Driver Bit Бита T27*50 длинные и короткие Медного цвета (Китай)", qty: 115 },
    { code: "916YX40-01-0", name: "Heptagon Screw Driver Bit Бита T40*50 длинные и короткие Медного цвета (Китай)", qty: 780 },
    { code: "916YX40-02-0", name: "Heptagon Screw Driver Bit Бита T40*90 длинные и короткие Медного цвета (Китай)", qty: 762 },
    { code: "918YX45-01-0", name: "Heptagon Screw Driver Bit Бита T45*50 длинные и короткие Медного цвета (Китай)", qty: 735 },
    { code: "16720571", name: "Huter Фильтр для моек 71/5/4", qty: 8 },
    { code: "15840039", name: "Inforce бита РН2*50мм 2 шт 11-01-291", qty: 2 },
    { code: "15976043", name: "Inforce Быстрозажимная струбцина 60х150мм 06-03-33", qty: 5 },
    { code: "15558556", name: "Inforce Комбинированные плоскогубцы 200мм 06-18-03", qty: 5 },
    { code: "31659753", name: "INFORCE Масло лубрикаторное для пневматического инструмента", qty: 1 },
    { code: "15849827", name: "Inforce Отвертка ударная SL5,5*100мм 06-09-88", qty: 3 },
    { code: "15849828", name: "Inforce Отвертка ударная SL6,5x125мм 06-09-89", qty: 3 },
    { code: "30909555", name: "Inforce Плоская стаместка 1\\4 32мм", qty: 2 },
    { code: "15955505", name: "INFORCE Шестигранная торцевая головка 1\\4 10мм", qty: 4 },
    { code: "21392338", name: "JETA PRO Прокладка защитная 125мм 8 отверстий", qty: 8 },
    { code: "15523234", name: "JTC Набор экстракторов для поврежденных болтов 6шт", qty: 1 },
    { code: "15523588", name: "JTC Соединитель быстросъемный 1/4", qty: 2 },
    { code: "32438778", name: "KRAFTOOL Антистатический антимагнитный прецизионный пинцет 135мм", qty: 1 },
    { code: "26708577", name: "KREPFIELD Заклепка вытяжная со ст. бортиком 4.8х14 20 шт алюминий", qty: 2 },
    { code: "27745476", name: "KREPFIELD Заклепка вытяжная со ст. бортиком 4.8х14 500 шт алюминий", qty: 1 },
    { code: "16424914", name: "Laima швабра стяжка /водосгон ширина 80см алюминиевый черенок 118см металлическая основа/резина проф", qty: 1 },
    { code: "28015772", name: "LAVR Очиститель томозных дисков 650мл", qty: 7 },
    { code: "16073721", name: "LAVR удалитель наклеек антискотч 400мл", qty: 14 },
    { code: "16692722", name: "LOM Щетка металлическая для УШМ 22ММ - 125мм", qty: 1 },
    { code: "15500267", name: "Luga Abrasiv круг отрезной абразивный по металлу 125*1.2*22.2мм", qty: 34 },
    { code: "13693298", name: "Luga-Abrasiv 125*1*22 А54 мет+нерж круг отрезной", qty: 36 },
    { code: "16506804", name: "LUXLITE Газ универсальный для газовых приборов", qty: 3 },
    { code: "30543518", name: "Master-Pro Перчатки рабочие х/б 50 пар", qty: 1 },
    { code: "13662566", name: "MasterProf Головка распылитель 30см", qty: 26 },
    { code: "15876904", name: "Matrix круг абразивный на ворс подложке под липучку перфорированный 5шт 73816", qty: 1 },
    { code: "15876923", name: "Matrix круг образивный на ворсовой подложке под липучку перфорированный 5шт", qty: 1 },
    { code: "15947827", name: "Menzerna Heavy Cut Compound 1000 Высокообразивная полировальная паста", qty: 3 },
    { code: "15947815", name: "Menzerna Medium Cut Polish 2000 полировальная паста", qty: 2 },
    { code: "16488026", name: "Mertech Сканер CL-2210 BLE P2D Black", qty: 1 },
    { code: "15604103", name: "Milwaukee Магнитный держатель для бит 60мм", qty: 11 },
    { code: "18586969", name: "Milwaukee Переходник с 1/4 на 1/2 квадратный хвостовик", qty: 2 },
    { code: "16012270", name: "MIRAX Слесарные тиски 150 мм", qty: 2 },
    { code: "34508497", name: "MTE Бита с профилем TORX длина 25мм", qty: 3 },
    { code: "13662566", name: "OLFA Excel Black Лезвие сегментированное 18*100*0.5мм 10шт OL-LBB-10B", qty: 26 },
    { code: "22058770", name: "OLFA Лезвия сегментированные 25мм 20 шт в кейсе", qty: 1 },
    { code: "15958520", name: "OMBRA Переходник для бит 1\\4 SDRx1\\4", qty: 5 },
    { code: "15457269", name: "PALISAD Опрыскиватель ручной, 2л, с насос и клапаном сброса давления", qty: 4 },
    { code: "16326173", name: "PARK Набор для полива", qty: 1 },
    { code: "13642060", name: "Patriot масло компрессорное 1л", qty: 2 },
    { code: "15607623", name: "Patriot щетка круглая для дрели 50мм", qty: 18 },
    { code: "16841389", name: "Pegas Быстросъемное соединенение 1/2", qty: 2 },
    { code: "911YX12-01-0", name: "Pentagon Socket Wrench Бита T3", qty: 11 },
    { code: "911YX09-01-0", name: "Pentagon Socket Wrench Бита T4", qty: 16 },
    { code: "911YX17-01-0", name: "Pentagon Socket Wrench Бита пятиугольная", qty: 21 },
    { code: "21929938", name: "Plantic Лопата совковая Terra", qty: 2 },
    { code: "26087465", name: "POLYAGRO Соединитель для шланга 3/4", qty: 2 },
    { code: "30978741", name: "Power Cube Куб фильтр удлинитель 5м 6 розеток (черный) 16а\\3.5 квт", qty: 1 },
    { code: "15881566", name: "PROSEPT Duty Graffiti средство для удаления граффити 0.4л", qty: 4 },
    { code: "15832376", name: "Proskit Кусачки 1РК-5101-Е антистатические", qty: 1 },
    { code: "15832402", name: "Proskit Щетка антистатическая AS-501A", qty: 1 },
    { code: "16272985", name: "Proskit Щетка пластиковая антистатическая прямая 25мм", qty: 1 },
    { code: "31716069", name: "R2R Заклепка вытяжная станд.бортик 4.8х25 100шт", qty: 1 },
    { code: "31715925", name: "R2R Заклепка вытяжная станд.бортик 4.8х30 20шт", qty: 2 },
    { code: "16061710", name: "Rage by vira молоток слесарный 200г", qty: 10 },
    { code: "21807520", name: "REDVERG Переходник для шуроповерта  угловой 1\\4", qty: 3 },
    { code: "15985125", name: "REDVERG Удлинитель для бит магнитный 1/4 100мм", qty: 3 },
    { code: "9001208357", name: "REMIX Укрывная пленка", qty: 65 },
    { code: "15734401", name: "REXANT Зажим монтажный изогнутый 200мм", qty: 6 },
    { code: "15734397", name: "REXANT Зажим монтажный прямой 200мм", qty: 4 },
    { code: "19902424", name: "REXANT Номерная пломба пластиковая 350мм\\\\красная\\\\ 50 шт", qty: 1 },
    { code: "17834770", name: "Rockforce Губка для полировки самоцепляющаяся 125мм ораньжевый", qty: 15 },
    { code: "22754961", name: "Rosfix 500мл активатор для цианакрилатного клея", qty: 1 },
    { code: "16497636", name: "RTP Кран шаровой латунный", qty: 1 },
    { code: "ВРЕМЕННЫЙ", name: "RU Бита Т25 7-лучевая", qty: 1 },
    { code: "6001 2RS", name: "RU Подшипник колеса (передний) 6001 2RS", qty: 633 },
    { code: "NBR72", name: "RU Сальник манжета армированная 15х25х7", qty: 676 },
    { code: "NBR71", name: "RU Сальник манжета армированная 17х25х7", qty: 670 },
    { code: "NBR70", name: "RU Сальник манжета армированная 17х28х7", qty: 48 },
    { code: "ВРЕМЕННЫЙ", name: "RU Светоотражающий элемент для катафота (белый)", qty: 209 },
    { code: "ВРЕМЕННЫЙ", name: "RU Светоотражающий элемент для катафота (желтый)", qty: 398 },
    { code: "15611440", name: "RUSKLAD Каркасное ограждение", qty: 2 },
    { code: "15642340", name: "Ryobi ONE+ Компатная дрель-шуруповерт R18DD3-120S", qty: 1 },
    { code: "15083153", name: "RYOBI Эксцентриковая шлифмашина ROS300A", qty: 6 },
    { code: "20530558", name: "SAMGRUPP Наждачная бумага 120/А4", qty: 8 },
    { code: "20531194", name: "SAMGRUPP Наждачная бумага 180/А4", qty: 31 },
    { code: "20530918", name: "SAMGRUPP Наждачная бумага 240/А4", qty: 8 },
    { code: "22548261", name: "SAMGRUPP Наждачная бумага 80/А4", qty: 15 },
    { code: "815.07.150.07", name: "Sandwox Диск полировальный жесткий 150 мм зеленый", qty: 8 },
    { code: "815.07.150.08", name: "Sandwox Диск полировальный средней жесткости 150 мм", qty: 5 },
    { code: "23799387", name: "SCHTAER Беспроводная полировальная машина 18В 125мм эксцентрик 15мм", qty: 1 },
    { code: "19585704", name: "SGM Валик прикаточный усиленный ДКГ-35", qty: 1 },
    { code: "33804973", name: "Shtapler Весы торговые PW 300 40*50 складная стойка", qty: 1 },
    { code: "16378093", name: "SKRAB Биты Т 10*150мм 10шт Торсионные", qty: 1 },
    { code: "20871423", name: "Smart Open Профессиональный обезжириватель кузова автомобиля от жира масла силикона 0,5л", qty: 2 },
    { code: "16252057", name: "Solins спрей-очиститель от жира и масла 400мл", qty: 2 },
    { code: "13947416", name: "SPARTA щетка металлическая с пластмассовой ручкой", qty: 2 },
    { code: "23894367", name: "Staff Маркер перманентный, черный, 0,5мм", qty: 22 },
    { code: "16729111", name: "Staff Маркер перманентный, черный, 0,8мм/2,2мм", qty: 3 },
    { code: "21719116", name: "STARTUL Зажим с фиксатром 250мм изогнутые губки PRO-6051-02", qty: 10 },
    { code: "22421535", name: "STARTUL Ключ комбинированный 14мм", qty: 5 },
    { code: "15125712", name: "STAYER Набор напильников 200мм", qty: 1 },
    { code: "15595630", name: "Stayer черная резиновая киянка 900г", qty: 2 },
    { code: "30446498", name: "Strong стм-524 щетка дисковая для дрели (нейлон) 100мм стм-52400100", qty: 1 },
    { code: "21567070", name: "Sturm Опрыскиватель 3016-22-02", qty: 1 },
    { code: "18805152", name: "Thorvik Ключ гаечный короткий CSRW14", qty: 5 },
    { code: "31126386", name: "TOUA Аккумулятор Li-ion 18V DBP001", qty: 1 },
    { code: "29303624", name: "TOUA Винтоверт DBLSD180 аккумуляторный 18v (1АКБ+ЗУ) DBLSD180-1", qty: 14 },
    { code: "17167465", name: "TOUA Шуруповерт DBLID180 аккумуляторный 18v", qty: 1 },
    { code: "32023572", name: "Tsunami 125мм А60 диск самозацепляющийся с отверстиями упак 5шт", qty: 1 },
    { code: "32023566", name: "TSUNAMI Диск шлифовальный 125мм А100", qty: 49 },
    { code: "32023476", name: "TSUNAMI Диск шлифовальный 125мм А120", qty: 33 },
    { code: "32023542", name: "TSUNAMI Диск шлифовальный 125мм А80", qty: 9 },
    { code: "37962444", name: "Turbosky Скотч двухсторонний DT-01 6мм*5м", qty: 1 },
    { code: "16168569", name: "ULTIMA Перчатки BLACK TOUCH нейлоновые с полеуретановым покрытием черные ULT615XL", qty: 15 },
    { code: "16506998", name: "ULTIMA Перчатки нитриловые GREEN GUARD 10XL", qty: 10 },
    { code: "16507046", name: "ULTIMA перчатки нитриловые GREEN GUARD ULT 150 р8М", qty: 5 },
    { code: "16518734", name: "UNIBOB Лента клейкая двухсторонняя 50мм*25м 211741", qty: 3 },
    { code: "16518595", name: "UNIBOB Лента клейкая малярная 48мм х 25м 214980", qty: 1 },
    { code: "16518706", name: "UNIBOB Лента клейкая малярная 50мм х 50м", qty: 3 },
    { code: "28162562", name: "USP Стрейч пленка UH100", qty: 4 },
    { code: "15508164", name: "Valtec лента-фум 12мм 20м", qty: 3 },
    { code: "20866563", name: "Vertextools набор презиционных отверток 115 предметов", qty: 2 },
    { code: "16293221", name: "Vertextools редуктор для бит угловой 1/4 140мм", qty: 1 },
    { code: "23114481", name: "Vixen Клей универсальный аэрозоль 520мл VX90015", qty: 3 },
    { code: "41725241", name: "WHEHOME Металлическая полка для ручного инструмента", qty: 1 },
    { code: "15600329", name: "WIEDERKRAFT Композитное пневматическое быстроразъёмное соединение WDK-820P", qty: 3 },
    { code: "18639229", name: "YATO Адаптер угловой для бит 1/4 YT-04630", qty: 7 },
    { code: "16562672", name: "YATO Быстроразъемное соединение 1/4 с внутренней резьбой", qty: 2 },
    { code: "15744082", name: "YATO Набор головок со вставками 12шт", qty: 1 },
    { code: "25238883", name: "YATO Ракель для поклейки пленки YT-52550", qty: 3 },
    { code: "16473892", name: "ZOLDER Круги шлифовальные наждачные 10шт-Р220 125мм", qty: 19 },
    { code: "16473829", name: "ZOLDER Круги шлифовальные наждачные 10шт, Р320, 15мм, 8 отверстий, липучка", qty: 8 },
    { code: "16230296", name: "АвтоДело Набор Г-образных шестигранников", qty: 1 },
    { code: "15500464", name: "Автошампунь для б/м K-PARTS SOFT (20 кг) Karcher", qty: 47 },
    { code: "9001180206", name: "Активатор клея ROSFIX", qty: 7 },
    { code: "41244485", name: "Атлант Удлинитель морозостойкий КГ Электрон 2х2,5м 20м", qty: 1 },
    { code: "15969132", name: "Борфреза твердосплавная коническая с закругленным концом, тип L 12-28-М-06-L73 КБ001464", qty: 1 },
    { code: "15945991", name: "Вафельное полотно,40см х 50м. VL5401", qty: 7 },
    { code: "800YX10-03-0", name: "Винт М10x8", qty: 80 },
    { code: "804YX04-06-0", name: "Винт М4х11", qty: 250 },
    { code: "804YX04-08-0", name: "Винт М4х11", qty: 500 },
    { code: "804YX04-10-0", name: "Винт М4х15", qty: 50 },
    { code: "804YX04-09-0", name: "Винт М4х16", qty: 300 },
    { code: "805YX05-10-0", name: "Винт М5х14", qty: 486 },
    { code: "805YX05-18-0", name: "Винт М5х14", qty: 88 },
    { code: "806YX06-07-0", name: "Винт М6х20", qty: 290 },
    { code: "808YX08-01-0", name: "Винт М8х25", qty: 248 },
    { code: "808YX08-11-0", name: "Винт М8х25", qty: 250 },
    { code: "808YX08-03-0", name: "Винт М8х30", qty: 2417 },
    { code: "803YX03-09-0", name: "Винт с потайной головкой М3x9 (Wind 25)", qty: 500 },
    { code: "804YX04-20-0", name: "Винт с потайной головкой М4x26,5", qty: 50 },
    { code: "16336778", name: "ВИХРЬ Лезвия для ножа 18мм", qty: 1 },
    { code: "25180143", name: "ВИХРЬ Эксцентриковая шлифмашина ЭШМ-125А", qty: 4 },
    { code: "18131749", name: "ВМПАВТО Жидкий ключ ВАЛЕРА 650мл флакон-аэрозоль 8609", qty: 8 },
    { code: "18131365", name: "ВМПАВТО Мастер-смазка ВАЛЕРА 650мл флакон-аэрозоль 8606", qty: 1 },
    { code: "817YX12-01-0", name: "Гайка М12 на мотор-колесо", qty: 769 },
    { code: "817YX12-02-0", name: "Гайка М12 на ось переднего колеса", qty: 500 },
    { code: "813YX05-01-0", name: "Гайка М5", qty: 1039 },
    { code: "ВРЕМЕННЫЙ", name: "Головка торцевая 1/2\" 7 граней наружный (Китай)", qty: 10 },
    { code: "16043756", name: "Головка-пульверизатор для пластиковых бутылок Grinda", qty: 43 },
    { code: "16013657", name: "Двуручный заклепочник для вытяжных заклепок Gigant 2.4-6.4мм", qty: 3 },
    { code: "15290509", name: "Дело Техники Головка 6-ти гранная длинная 8мм 1/4 ДТ576", qty: 13 },
    { code: "15290579", name: "Дело Техники Головка шестигранная 7мм 1/4", qty: 5 },
    { code: "15290642", name: "Дело Техники Держатель для бит 1/4 ДТ/500 609501", qty: 3 },
    { code: "15290643", name: "Дело Техники Зажим с фиксацией с полукруглым захватом 180мм 430175", qty: 1 },
    { code: "15290672", name: "Дело Техники Клещи переставные 250мм", qty: 1 },
    { code: "15290840", name: "Дело Техники ключ комбинированный 26мм", qty: 2 },
    { code: "15290736", name: "Дело Техники Ключ комбинированный трещоточный 14мм", qty: 7 },
    { code: "20984913", name: "ДЕЛО ТЕХНИКИ Переходник для торцевых головок 1\\4 50мм", qty: 7 },
    { code: "15291351", name: "Дело техники плоскогубцы удлиненные загнутые 160мм на холдере ДТ 48/6 413160", qty: 8 },
    { code: "15592494", name: "дело техники тарелка магнитная 108мм", qty: 3 },
    { code: "15291400", name: "дело техники трещетка 45 зб 1\\4", qty: 9 },
    { code: "27139052", name: "Дело Техники Щётка-мини с прорезиненной рукояткой, нейлоновая щетина", qty: 3 },
    { code: "27139094", name: "Дело Техники Щетка-мини, нейлоновая щетина", qty: 3 },
    { code: "38670280", name: "ДМД Упаковочная клейкая лента 48мм*44мм 60м", qty: 5 },
    { code: "38669992", name: "ДМД-УПАК Клейкая лента 48мм/43мкм/145м", qty: 22 },
    { code: "ZU001", name: "Зарядное устройство Wind 4.0 (б/у)", qty: 2 },
    { code: "796", name: "Защитная пластина номера", qty: 900 },
    { code: "16510112", name: "Зубр Захват размер L-XL перчатки с нитриловым обливом 10 шт УПАК", qty: 27 },
    { code: "16510108", name: "Зубр Захват размер S-M перчатки с нитриловым обливом", qty: 10 },
    { code: "13916436", name: "Зубр Ножницы 175 мм", qty: 3 },
    { code: "16700701", name: "ЗУБР Очки защитные открытого типа", qty: 25 },
    { code: "17617740", name: "Зубр Перманентный маркер МП-50 черный", qty: 14 },
    { code: "15889639", name: "ЗУБР Плашкодержатель Мастер 38х14мм М12-М14 L - 315мм", qty: 1 },
    { code: "16700251", name: "ЗУБР Прозрачные очки защитные открытого типа пластиковые дужки", qty: 10 },
    { code: "15886105", name: "Зубр Сигнальная лента цвет красно-белый 75мм*200м", qty: 7 },
    { code: "15886104", name: "Зубр Сигнальная лента цвет черно-желтый 75мм*200м", qty: 3 },
    { code: "15493382", name: "ЗУБР Универсал перчатки L-XL с одинарным обливом 10 пар", qty: 3 },
    { code: "18372379", name: "Изолента Gigant ПВХ 15 мм х 20 м, черная GT-0-1", qty: 33 },
    { code: "ВРЕМЕННЫЙ", name: "Кабель для прошивки самоката", qty: 3 },
    { code: "16201531", name: "Кедр набор щеток для дрели 3шт плоская 50мм  2 чашки  25-50мм", qty: 2 },
    { code: "32448342", name: "Комус маркер перманент. черный круглый 1-4мм", qty: 3 },
    { code: "Б210426", name: "Костюм RAGNAR черный, 44-46/170-176", qty: 3 },
    { code: "Б210426", name: "Костюм RAGNAR черный, 52-54/170-176", qty: 5 },
    { code: "Б210426", name: "Костюм RAGNAR черный, 56-58/182-188", qty: 5 },
    { code: "Б210426", name: "Костюм Профессионал-2 т-серый/св-серый, 84-86/182-188", qty: 1 },
    { code: "15605871", name: "Кратон щетка концевая для дрели 25мм", qty: 1 },
    { code: "20977929", name: "Круг шлифовальный перфорированный, 8 отверстий, 125 мм, P1000, 5 шт Росомаха", qty: 5 },
    { code: "Б1211254", name: "Куртка ЭКСПЕРТ-К НЬЮ утепленная тк. Оксфорд т.синяя, 56-58/182-188", qty: 2 },
    { code: "Б1211256", name: "Куртка ЭКСПЕРТ-НЬЮ утепленная тк. Оксфорд т.синяя, 52-54/182-188", qty: 1 },
    { code: "29978252", name: "КЭС Ледоруб 142х1400мм кованый со стальным черенком", qty: 1 },
    { code: "22139313", name: "Лукойл Масло GENESIS 4л", qty: 2 },
    { code: "Б1211251", name: "Модель 635. Nordman 15 S Сапоги из ПВХ оливковые, 44", qty: 5 },
    { code: "Б1211252", name: "Модель 635. Nordman 15 S Сапоги из ПВХ оливковые, 45", qty: 5 },
    { code: "22106.261.870", name: "МСР 2000 Паста полировальная среднеобразивная 1л", qty: 6 },
    { code: "15824703", name: "Набор для экспресс склеивания Akfix 705 125 гр+400 мл AN705_125", qty: 3 },
    { code: "РЕМ000", name: "Набор ремонтника", qty: 25 },
    { code: "9001100571", name: "Набор склеивания Akfix", qty: 2 },
    { code: "WNB003", name: "Наклейка логотип на стойку Яндекс GO (Orajet 3640) ( W5.0 - NB)", qty: 531 },
    { code: "W4001", name: "Наклейка на всю стойку (Saviar) (W4.0)", qty: 311 },
    { code: "37278270", name: "Нефтехимик STRONG ANTIGRAFFITI Антиграффити для удаления граффити и старой краски 0,5Л с триггером", qty: 1 },
    { code: "16493258", name: "НЕФТЕХИМИК Обезжириватель 10л", qty: 10 },
    { code: "22825029", name: "Нефтехимик Обезжириватель 20л 03200000", qty: 6 },
    { code: "16493386", name: "НЕФТЕХИМИК Растворитель 10л", qty: 17 },
    { code: "22825053", name: "Нефтехимик растворитель 646 20л 6462000000", qty: 1 },
    { code: "15543231", name: "Нож Vira с сегментированным лезвием металлический 18мм", qty: 8 },
    { code: "22984.260.870", name: "НСС 1000 Паста полировальная высокоабразивная 1кг", qty: 6 },
    { code: "28898558", name: "ООО Комус Стрейч-пленка перв.сыр для ручн.уп 20мкр 50см", qty: 26 },
    { code: "15826423", name: "ООО ТМТ групп плашка М 12*1.25", qty: 2 },
    { code: "34667725", name: "ОФИСМАГ Маркер преманентный, черный, тонкий наконечник", qty: 10 },
    { code: "16194689", name: "Очиститель электрических контактов Synthetium аэрозоль 520мл", qty: 14 },
    { code: "32622252", name: "ПАРТНЕР Стрейч с ручкой 20МКМ 100мм х 108м", qty: 4 },
    { code: "23894367", name: "Перманентный маркер STAFF Profit PM-105, черный, тонкий", qty: 22 },
    { code: "16480321", name: "Пластик система Пластиковый ящик для метизов красный 12.401.2", qty: 5 },
    { code: "16480343", name: "Пластик система Пластиковый ящик для метизов красный 12.402.2", qty: 5 },
    { code: "Б1211258", name: "Плащ Альбатрос влагозащитный ЛАЙТ серый, XXL", qty: 3 },
    { code: "Б121125", name: "Плащ Альбатрос влагозащитный ЛАЙТ серый, XXXXL", qty: 7 },
    { code: "Б210426", name: "Полуботинки СОЮЗ ПЛЮС черные, 38", qty: 2 },
    { code: "Б210426", name: "Полуботинки СОЮЗ ПЛЮС черные, 41", qty: 2 },
    { code: "Б210426", name: "Полуботинки СОЮЗ ПЛЮС черные, 43", qty: 5 },
    { code: "Б210426", name: "Полуботинки СОЮЗ ПЛЮС черные, 44", qty: 6 },
    { code: "Б210426", name: "Полуботинки СОЮЗ ПЛЮС черные, 45", qty: 3 },
    { code: "824YX04-03-0", name: "Прокладка М10 (Wind 25)", qty: 225 },
    { code: "9002057678", name: "Редуктор угловой для бит 60мм", qty: 2 },
    { code: "15978042", name: "РЕСАНТА Строительный фен ФЭ-2000К", qty: 3 },
    { code: "16146761", name: "Ресанта Углошлифовальная машина УШМ-125/1400Э 75/12/5", qty: 1 },
    { code: "18219301", name: "РОКОТ Клейкая лента 75х48мм 472-008", qty: 9 },
    { code: "16412848", name: "РОСОМАХА Круг шлифовальный перфорированный 125мм 8 отв. зерно 180 5шт УПАК", qty: 6 },
    { code: "16412853", name: "РОСОМАХА Круг шлифовальный перфорированный 125мм 8 отв. зерно 240 5шт УПАК", qty: 18 },
    { code: "20978175", name: "РОСОМАХА Круг шлифовальный перфорированный 125мм зерно 1500 5шт УПАК", qty: 3 },
    { code: "15494631", name: "РОСОМЗ Щиток защитный лицевой НБТ2 ВИЗИОН ТИТАН", qty: 6 },
    { code: "AB.50.0032.18", name: "Рычаг тормоза (левый)(22)", qty: 319 },
    { code: "AB.50.0032.19", name: "Рычаг тормоза (правый)(22)", qty: 123 },
    { code: "25072-000", name: "Саморез DIN 7981 (4,2Х13) по металлу", qty: 25000 },
    { code: "15565557", name: "Сибртех кисть флейцевая Стандарт 25*6 натуральная щетина пластиковая ручка", qty: 3 },
    { code: "Б210426", name: "Стелька гигиеническая", qty: 25 },
    { code: "820ST10-01-0", name: "Стопорная шайба с насечками М10", qty: 633 },
    { code: "820ST12-01-0", name: "Стопорная шайба с насечками М12", qty: 3100 },
    { code: "820ST08-01-0", name: "Стопорная шайба с насечками М8", qty: 327 },
    { code: "00000775", name: "Стопорное кольцо внешнее Мотор-колеса", qty: 129 },
    { code: "ВРЕМЕННЫЙ", name: "Строительный фен Ресанта ФЭ-2000ЭК 75/2/2", qty: 1 },
    { code: "24973623", name: "Сыктывкарские Протирочная бумага 2 сл. 1000 листов", qty: 1 },
    { code: "15915413", name: "Тара.ру ящик п/э 400*300*230 мм сплошной с ручками серый", qty: 3 },
    { code: "16091068", name: "Тара.ру Ящик п/э 710*500*455 мм сплошной 120л", qty: 11 },
    { code: "19009734", name: "ТЕМП Аппарат высокого давления 200 бар 14л/мин ТХ 14/200", qty: 1 },
    { code: "31444179", name: "Тундра Насадка для шуруповерта d=4-19 мм для снятия фаски на болтах", qty: 1 },
    { code: "31443219", name: "Тундра насадка щетка на шуруповерт d=100мм высота 25мм шестигранный хвостовик", qty: 20 },
    { code: "31443981", name: "Тундра насадка щетка на шуруповерт d=85мм высота 125мм шестигранный хвостовик 9022476", qty: 3 },
    { code: "Б210426", name: "Футболка KOTLINE BASIC черная, 44-46", qty: 3 },
    { code: "Б210426", name: "Футболка KOTLINE BASIC черная, 48-50", qty: 1 },
    { code: "Б210426", name: "Футболка KOTLINE BASIC черная, 50-52", qty: 1 },
    { code: "Б210426", name: "Футболка KOTLINE BASIC черная, 52-54", qty: 6 },
    { code: "Б210426", name: "Футболка KOTLINE BASIC черная, 56-58", qty: 4 },
    { code: "Б1211257", name: "Чулок меховой, 44", qty: 5 },
    { code: "Б1211255", name: "Чулок меховой, 45", qty: 5 },
    { code: "822YX23-01-0", name: "Шайба (Wind 25)", qty: 150 },
    { code: "15995917", name: "Эврика Адаптер для бит 1/4 DR ER-92305", qty: 7 },
    { code: "15996742", name: "Эврика головка торцевая 14мм 1/4 6-гранная глубокая", qty: 1 },
    { code: "15951982", name: "Эврика Держатель инструмента магнитный ER-55324", qty: 4 },
    { code: "15536423", name: "ЭРА Изолена 19мм х 20м красная", qty: 3 },
    { code: "15920820", name: "ЯСХИМ Уайт-спирит, ПЭТ 0,5л.", qty: 3 },
    { code: "18216889", name: "Inforce Эргономичная лопата 445х330 с двумя ручками", qty: 1 },
    { code: "18216859", name: "Inforce Эргономичная лопата 555х410 с двумя ручками", qty: 1 },
    { code: "23256363", name: "ООО Металекс Стол-Верстак с тумбой 950х1400х600мм", qty: 1 }
  ]
};

/* ==================== Ключи хранения ==================== */
/* ==================== Панели поиска (Учёт/Склад/1С) ====================
   Разметка одинаковая для всех трёх вкладок — генерируем один раз вместо
   трёх копий в HTML. Должно выполниться раньше, чем что-либо ниже
   обратится к whSearchInput/caSearchClear и т.п. через getElementById. */
function searchBarHTML(prefix) {
  return `
    <span class="search-icon">🔍</span>
    <input type="text" id="${prefix}SearchInput" placeholder="Поиск" autocomplete="off" style="flex:1;">
    <button class="iconbtn clear" id="${prefix}SearchClear" title="Очистить поле">✕</button>
    <span class="search-count" id="${prefix}SearchCount"></span>
    <button class="iconbtn nav" id="${prefix}SearchPrev" title="Предыдущее совпадение">‹</button>
    <button class="iconbtn nav" id="${prefix}SearchNext" title="Следующее совпадение">›</button>
  `;
}
["wh", "ca", "oc"].forEach((prefix) => {
  const bar = document.getElementById(prefix + "SearchBar");
  if (bar) bar.innerHTML = searchBarHTML(prefix);
});

/* ==================== Страховка: переключение вкладок ====================
   Регистрируется в самом начале скрипта. Даже если ниже произойдёт ошибка
   загрузки, вкладки продолжат переключаться (функция switchTab всплывает
   благодаря hoisting объявлений функций). */
const suggestZones = [];               // { box, inputs[] } — закрываются кликом мимо
function registerSuggestZone(box, inputs) { if (box) suggestZones.push({ box, inputs }); }

let lastTabSwitch = 0;
function handleTabPress(e) {
  const btn = e.target && e.target.closest ? e.target.closest(".tab-btn") : null;
  if (!btn) return;
  const now = Date.now();
  if (now - lastTabSwitch < 400) return;   // click + pointerup на тачскрине
  lastTabSwitch = now;
  if (typeof switchTab === "function") switchTab(tabTargetFromId(btn.id));
}

/* Один обработчик на все глобальные реакции клика:
   переключение вкладок, снятие временной подсветки, закрытие подсказок. */
document.addEventListener("click", function (e) {
  handleTabPress(e);
  document.querySelectorAll(".row-flash").forEach((el) => el.classList.remove("row-flash"));
  suggestZones.forEach(({ box, inputs }) => {
    if (!box.contains(e.target) && inputs.indexOf(e.target) === -1) box.classList.remove("open");
  });
});
document.addEventListener("pointerup", handleTabPress);   // страховка для тачскрина

// Номер версии файлов — держим руками синхронно с CACHE_NAME в sw.js
// (при каждом поднятии кэша меняем и тут). Просто отображается в углу
// шапки — чтобы проверить, долетело ли обновление до устройства.
//
// НУМЕРАЦИЯ: шаг 0.1 на каждое обновление — v1.0, v1.1, v1.2 и так далее.
// Раньше был сплошной счётчик (…v98, v99, v100), с версии v1.0 — этот
// формат. Версия нигде не сравнивается как число, только показывается и
// пишется в резервную копию, так что смена формата ничего не ломает.
const APP_VERSION = "v1.3";
{
  const el = document.getElementById("appVersionBadge");
  if (el) el.textContent = APP_VERSION;
}

const DATA_BY_DATE_KEY = "potreblenie_data_by_date_v3";
const LAST_DATE_KEY = "potreblenie_last_date_v1";
const WAREHOUSE_KEY = "potreblenie_warehouse_v1";
// Остатки второго склада («Бестужевская») — храним отдельно, в основной
// список 1С они не подмешиваются. Нужны для будущего сравнения остатков
// между складами.
const BEST_STOCK_KEY = "potreblenie_best_stock_v1";
function loadBestStock() {
  try {
    const raw = localStorage.getItem(BEST_STOCK_KEY);
    return raw ? JSON.parse(raw) : null;   // { savedAt, rows: [{code,name,qty}] }
  } catch (e) { return null; }
}
function saveBestStock(data) {
  try { localStorage.setItem(BEST_STOCK_KEY, JSON.stringify(data)); } catch (e) {}
}
let bestStock = loadBestStock();

// Список на перемещение — что нужно привезти со второго склада.
const TRANSFER_KEY = "potreblenie_transfer_list_v1";
function loadTransferList() {
  try {
    const raw = localStorage.getItem(TRANSFER_KEY);
    return raw ? JSON.parse(raw) : [];    // [{code,name,mine,other}]
  } catch (e) { return []; }
}
function saveTransferList() {
  try { localStorage.setItem(TRANSFER_KEY, JSON.stringify(transferList)); } catch (e) {}
}
let transferList = loadTransferList();

const MONTHS_FULL_GEN = ["января","февраля","марта","апреля","мая","июня","июля","августа","сентября","октября","ноября","декабря"];

function todayStr() { return new Date().toISOString().slice(0, 10); }
function formatShort(dateStr) {
  if (!dateStr) return "—";
  const [y, m, d] = dateStr.split("-").map(Number);
  if (!y || !m || !d) return dateStr;
  return `${String(d).padStart(2,"0")}.${String(m).padStart(2,"0")}`;
}
function formatFull(dateStr) {
  if (!dateStr) return "";
  const [y, m, d] = dateStr.split("-").map(Number);
  if (!y || !m || !d) return dateStr;
  return `${d} ${MONTHS_FULL_GEN[m - 1]} ${y}`;
}
function escapeHtml(str) {
  return String(str).replace(/[&<>"']/g, (c) => ({
    "&": "&amp;", "<": "&lt;", ">": "&gt;", '"': "&quot;", "'": "&#39;"
  }[c]));
}
/* Если код склеен из двух через "/", показываем вторым уровнем под первым */
function formatCodeDisplay(code) {
  const parts = escapeHtml(code).split("/").map((s) => s.trim()).filter(Boolean);
  if (parts.length <= 1) return escapeHtml(code);
  return `<span class="code-split">${parts.map((p) => `<span class="code-line">${p}</span>`).join("")}</span>`;
}

/* ==================== Модалки: открыть/закрыть ====================
   Раньше каждая из 17 модалок открывалась/закрывалась своей парой
   document.getElementById(id).classList.add/remove("open") — один и тот же
   код почти 50 раз. Принимает и id (строку), и уже найденный элемент —
   в паре мест элемент был закэширован в константу заранее. */
function openModal(idOrEl) {
  const el = typeof idOrEl === "string" ? document.getElementById(idOrEl) : idOrEl;
  if (el) el.classList.add("open");
}
function closeModal(idOrEl) {
  const el = typeof idOrEl === "string" ? document.getElementById(idOrEl) : idOrEl;
  if (el) el.classList.remove("open");
}

/* ==================== Загрузка/сохранение ==================== */
function loadDataByDate() {
  try {
    const raw = localStorage.getItem(DATA_BY_DATE_KEY);
    if (!raw) return {};
    const parsed = JSON.parse(raw);
    return (parsed && typeof parsed === "object") ? parsed : {};
  } catch (e) { return {}; }
}
function saveDataByDate() {
  try { localStorage.setItem(DATA_BY_DATE_KEY, JSON.stringify(dataByDate)); } catch (e) {}
  scheduleSync();
}
function loadWarehouse() {
  try {
    const raw = localStorage.getItem(WAREHOUSE_KEY);
    if (!raw) return JSON.parse(JSON.stringify(WAREHOUSE_SEED));
    const parsed = JSON.parse(raw);
    if (parsed && Array.isArray(parsed.parts) && Array.isArray(parsed.consumables)) return parsed;
    return JSON.parse(JSON.stringify(WAREHOUSE_SEED));
  } catch (e) { return JSON.parse(JSON.stringify(WAREHOUSE_SEED)); }
}
function saveWarehouse() {
  try { localStorage.setItem(WAREHOUSE_KEY, JSON.stringify(warehouse)); } catch (e) {}
  scheduleSync();
}
// Локально, без синхронизации — для чисто визуальных действий (сортировка),
// которые не должны считаться правкой данных и уходить на сервер.
function saveWarehouseLocalOnly() {
  try { localStorage.setItem(WAREHOUSE_KEY, JSON.stringify(warehouse)); } catch (e) {}
}

let dataByDate = loadDataByDate();
let warehouse = loadWarehouse(); // { parts: [{code,name,qty,model}], consumables: [...] }
// Дата при каждом открытии приложения — всегда сегодняшняя (а не та, на которой
// остановились в прошлый раз): открыли на следующий день — календарь сам на нём.
let currentDate = todayStr();

function entryFor(date) {
  if (!dataByDate[date]) {
    dataByDate[date] = { sections: { parts: [], consumables: [] }, counts: {}, checked: {} };
  }
  if (!dataByDate[date].checked) dataByDate[date].checked = {};
  if (!dataByDate[date].sections) dataByDate[date].sections = { parts: [], consumables: [] };
  return dataByDate[date];
}

let sections = entryFor(currentDate).sections;
let counts = entryFor(currentDate).counts;
let checked = entryFor(currentDate).checked;

function persistCurrent() {
  dataByDate[currentDate] = { sections, counts, checked };
  saveDataByDate();
}
// Локально, без синхронизации — используется при переключении даты
// (сохраняем то, что уже было введено, но само переключение — не правка).
function persistCurrentLocalOnly() {
  dataByDate[currentDate] = { sections, counts, checked };
  try { localStorage.setItem(DATA_BY_DATE_KEY, JSON.stringify(dataByDate)); } catch (e) {}
}

/* ==================== Общий механизм отмены/повтора (для всех 3 вкладок) ====================
   Раньше у «Учёта», «Склада» и «1С» было по три копии одного и того же:
   свой стек отмены, свой стек повтора, своя функция снимка состояния,
   своя привязка кнопок ↶/↷. Теперь это один универсальный конструктор —
   каждая вкладка просто передаёт «как снять снимок» и «как восстановить
   состояние из снимка» (там разная логика: разные глобальные переменные,
   разный рендер, разная подсветка изменившейся строки). */
function makeUndoRedo({ snapshotFn, restoreFn, undoBtnId, redoBtnId, limit }) {
  const undoStack = [];
  const redoStack = [];
  const undoBtn = document.getElementById(undoBtnId);
  const redoBtn = document.getElementById(redoBtnId);
  function updateButtons() {
    undoBtn.disabled = undoStack.length === 0;
    redoBtn.disabled = redoStack.length === 0;
  }
  function push() {
    undoStack.push(snapshotFn());
    if (undoStack.length > limit) undoStack.shift();
    redoStack.length = 0;
    updateButtons();
  }
  function undo() {
    if (undoStack.length === 0) return;
    redoStack.push(snapshotFn());
    restoreFn(undoStack.pop());
    updateButtons();
  }
  function redo() {
    if (redoStack.length === 0) return;
    undoStack.push(snapshotFn());
    restoreFn(redoStack.pop());
    updateButtons();
  }
  function reset() {
    undoStack.length = 0;
    redoStack.length = 0;
    updateButtons();
  }
  undoBtn.addEventListener("click", undo);
  redoBtn.addEventListener("click", redo);
  updateButtons();
  return { push, undo, redo, reset, updateButtons };
}

/* ==================== История (отмена/повтор) — вкладка «Учёт» ==================== */
const HISTORY_LIMIT = 50;
function snapshot() { return JSON.stringify({ sections, counts, checked }); }

/* ==================== Подсветка изменённой строки при отмене ====================
   Сравниваем состояние до и после отката и прыгаем к первой различающейся
   позиции, чтобы было видно, что именно изменилось. */
function firstDiffIndex(a, b) {
  a = a || []; b = b || [];
  const n = Math.max(a.length, b.length);
  for (let i = 0; i < n; i++) {
    if (JSON.stringify(a[i]) !== JSON.stringify(b[i])) return i;
  }
  return -1;
}

/* Ищет, в каком разделе и на какой строке произошло изменение */
function findChange(beforeLists, afterLists, beforeMap, afterMap, keyFn) {
  for (const section of ["parts", "consumables"]) {
    const i = firstDiffIndex(beforeLists[section], afterLists[section]);
    if (i >= 0) {
      const len = (afterLists[section] || []).length;
      if (len === 0) continue;
      return { section, index: Math.min(i, len - 1) };
    }
  }
  // списки совпали — значит изменилось значение по ключу (кол-во, галочка)
  if (beforeMap && afterMap) {
    const keys = new Set(Object.keys(beforeMap).concat(Object.keys(afterMap)));
    for (const k of keys) {
      if (beforeMap[k] === afterMap[k]) continue;
      for (const section of ["parts", "consumables"]) {
        const list = afterLists[section] || [];
        const idx = list.findIndex((it) => keyFn(section, it) === k);
        if (idx >= 0) return { section, index: idx };
      }
    }
  }
  return null;
}

/* Находит строку таблицы по её индексу в МАССИВЕ данных.
   Раньше по всему файлу писали tbody.querySelectorAll("tr")[index], то есть
   индекс в данных использовали как номер строки на экране. Пока фильтров не
   было, это совпадало. Но «Склад» фильтруется по модели, а «1С» — по
   разделу/остатку, и тогда на экране остаётся лишь подмножество строк:
   обращение по номеру попадало в чужую позицию — отсюда переходы «на
   случайную строку». Теперь у строк этих таблиц есть data-index с настоящим
   индексом, и ищем именно по нему.
   Возвращает null, если строка сейчас скрыта фильтром. */
function rowByDataIndex(tbodyId, index) {
  const tbody = document.getElementById(tbodyId);
  if (!tbody) return null;
  if (tbody.querySelector("tr[data-index]")) {
    return tbody.querySelector(`tr[data-index="${index}"]`);
  }
  // «Учёт» рисуется без фильтра — там номер строки и индекс совпадают.
  return tbody.querySelectorAll("tr")[index] || null;
}

/* Подсвечивает строку и прокручивает к ней */
function highlightAt(bodyPrefix, section, index) {
  const tr = rowByDataIndex(bodyPrefix + section, index);
  if (tr) highlightRows([tr]);
}

function restoreSnapshot(json) {
  const state = JSON.parse(json);
  const beforeLists = sections, beforeCounts = counts, beforeChecked = checked;
  sections = state.sections;
  counts = state.counts;
  checked = state.checked || {};
  persistCurrent();
  renderAll();
  const hit = findChange(beforeLists, sections, beforeCounts, counts, countKey)
           || findChange(beforeLists, sections, beforeChecked, checked, countKey);
  if (hit) highlightAt("body-", hit.section, hit.index);
}
const caHistory = makeUndoRedo({
  snapshotFn: snapshot,
  restoreFn: restoreSnapshot,
  undoBtnId: "undoBtn",
  redoBtnId: "redoBtn",
  limit: HISTORY_LIMIT
});
function pushHistory() { caHistory.push(); }

/* ==================== Ключ позиции ====================
   Если у позиции есть код склада — ключ общий по коду (для сквозного расчёта
   и для связи учёта со складом). Если кода нет — ключ локальный (раздел+название). */
function countKey(section, item) {
  return item.code ? ("code::" + item.code) : (section + "::" + item.name);
}

/* ==================== Выделение строк кликом/тапом (мульти-выбор) ==================== */
const selectedCA = { parts: new Set(), consumables: new Set() };
const selectedWH = { parts: new Set(), consumables: new Set() };

function toggleRowPick(store, section, key, tr) {
  if (store[section].has(key)) {
    store[section].delete(key);
    tr.classList.remove("row-picked");
  } else {
    store[section].add(key);
    tr.classList.add("row-picked");
  }
}
function applyPickHighlight(store, section, tbody, list, keyFn) {
  // Только строки позиций (у заголовков групп нет data-index) и только по
  // индексу в массиве: на «Учёте» строки идут группами по моделям, поэтому
  // позиция строки на экране ничего не значит.
  Array.from(tbody.querySelectorAll("tr[data-index]")).forEach((tr) => {
    const item = list[parseInt(tr.dataset.index, 10)];
    if (!item) return;
    tr.classList.toggle("row-picked", store[section].has(keyFn(item)));
  });
}

/* ==================== Временная жёлтая подсветка (новая/дублирующая позиция) ====================
   Держится до первого клика в любом месте страницы. */
function flashRow(tr) {
  if (!tr) return;
  setTimeout(() => tr.classList.add("row-flash"), 0);
}

/* ==================== Рендер: вкладка «Учёт» ==================== */
/* Порядок групп в «Учёте». Всё, чего здесь нет, идёт следом по алфавиту,
   а позиции без модели — в самом конце. */
const CA_MODEL_ORDER = ["9Bot", "YGW 4.0", "YGW 5.0"];
function caModelOf(item) {
  const info = item.code ? warehouseInfoFor(item.code) : null;
  return (info && info.model) || "";
}
/* Раскладывает позиции раздела по моделям в нужном порядке.
   Возвращает [{ model, pairs: [{ item, i }] }] — i остаётся ИНДЕКСОМ В
   МАССИВЕ, а не номером строки на экране: на него завязаны кнопки строки,
   поиск, подсветка и переходы. */
function caGroups(section) {
  const byModel = new Map();
  sections[section].forEach((item, i) => {
    const m = caModelOf(item);
    if (!byModel.has(m)) byModel.set(m, []);
    byModel.get(m).push({ item, i });
  });
  const rank = (m) => {
    if (!m) return 3000;                                  // без модели — в конец
    const known = CA_MODEL_ORDER.indexOf(m);
    return known === -1 ? 2000 : known;                   // прочие — между
  };
  return [...byModel.keys()]
    .sort((a, b) => (rank(a) - rank(b)) || a.localeCompare(b, "ru"))
    .map((model) => ({ model, pairs: byModel.get(model) }));
}

function renderSection(section) {
  const tbody = document.getElementById("body-" + section);
  tbody.innerHTML = "";
  const groups = caGroups(section);
  // Заголовок нужен, только если групп больше одной: у «Расходников» модели
  // обычно нет вовсе, и единственная надпись «Без модели» была бы мусором.
  const showHeaders = groups.length > 1;
  groups.forEach(({ model, pairs }) => {
    let rowNo = 0;                     // нумерация своя внутри каждой модели
    if (showHeaders) {
      const head = document.createElement("tr");
      head.className = "group-row";
      head.innerHTML = `<td colspan="5">${escapeHtml(model || "Без модели")} · ${pairs.length}</td>`;
      tbody.appendChild(head);
    }
    pairs.forEach(({ item, i }) => {
    rowNo++;
    const key = countKey(section, item);
    if (counts[key] === undefined) counts[key] = 0;
    const isChecked = !!checked[key];
    const guest = isGuestMode();
    // Модель берём со склада по коду — показываем той же серой подписью
    // в углу, что и на вкладке «Склад».
    const whInfoForModel = item.code ? warehouseInfoFor(item.code) : null;
    const modelLabel = (whInfoForModel && whInfoForModel.model) || "";
    const tr = document.createElement("tr");
    tr.dataset.key = key;
    tr.dataset.index = i;          // индекс в массиве: строки идут не по нему
    tr.innerHTML = `
      <td class="num">${rowNo}</td>
      <td class="code">${item.code ? `<span class="code-text" data-code="${escapeHtml(item.code)}">${formatCodeDisplay(item.code)}</span>` : "—"}</td>
      <td class="name">
        ${escapeHtml(item.name)}
        ${modelLabel ? `<span class="ca-model-badge">${escapeHtml(modelLabel)}</span>` : ""}
        ${guest ? "" : `<button class="ca-del-badge" data-section="${section}" data-index="${i}" title="Удалить строку">🗑</button>`}
      </td>
      <td class="check">
        <button class="chk-toggle${isChecked ? " on" : ""}" data-section="${section}" data-index="${i}" title="Отметить как учтено">${isChecked ? "✓" : ""}</button>
      </td>
      <td class="count count-merged">
        <div class="count-tier count-tier-num">
          <input type="number" min="0" value="${counts[key]}" class="countInput"
            data-section="${section}" data-index="${i}"${guest ? " readonly tabindex=\"-1\"" : ""}>
        </div>
        ${guest ? "" : `
        <div class="count-tier count-tier-plus">
          <button class="btn plus" data-section="${section}" data-index="${i}" title="Тап: +1. Зажать: указать количество">+</button>
        </div>
        <div class="count-tier count-tier-minus">
          <button class="btn minus" data-section="${section}" data-index="${i}" title="Тап: −1. Зажать: указать количество">–</button>
        </div>`}
      </td>
    `;
    tbody.appendChild(tr);
    });
  });
  applyPickHighlight(selectedCA, section, tbody, sections[section], (it) => countKey(section, it));
  if (typeof applyCaSearch === "function") applyCaSearch();
}
function renderAll() {
  renderSection("parts");
  renderSection("consumables");
  document.getElementById("dateDisplayText").textContent = formatShort(currentDate);
}

function itemAt(section, index) { return sections[section][index]; }

function handleCountClick(e) {
  const section = e.currentTarget.id.replace("body-", "");
  const btn = e.target.closest("button");
  if (btn) {
    const index = parseInt(btn.dataset.index, 10);
    const item = itemAt(section, index);
    if (!item) return;
    const key = countKey(section, item);
    // «plus»/«minus» теперь обрабатываются через pointerdown/pointerup
    // (см. setupQtyLongPress) — короткий тап и долгое нажатие делают разное.
    if (btn.classList.contains("chk-toggle")) {
      checked[key] = !checked[key];
      // Отметка «учтено» — рабочая пометка для себя, а не изменение данных:
      // сохраняем только локально, без запуска синхронизации. Иначе при
      // отметке позиций одна за другой каждый тап дёргал загрузку на сервер.
      persistCurrentLocalOnly();
      renderSection(section);
    } else if (btn.classList.contains("ca-del-badge")) {
      if (!confirm(`Удалить «${item.name}» из списка?`)) return;
      pushHistory();
      sections[section].splice(index, 1);
      delete counts[key];
      delete checked[key];
      selectedCA[section].delete(key);
      persistCurrent();
      renderSection(section);
    }
    return;
  }
  const nameTd = e.target.closest("td.name");
  if (nameTd) {
    const tr = nameTd.closest("tr");
    toggleRowPick(selectedCA, section, tr.dataset.key, tr);
  }
}
document.getElementById("body-parts").addEventListener("click", handleCountClick);
document.getElementById("body-consumables").addEventListener("click", handleCountClick);

/* ==================== Учёт: короткое / долгое нажатие на «+»/«–» ====================
   Короткий тап — просто +1 к количеству (или −1; если результат ≤ 0, позиция
   удаляется из списка — та же логика, что раньше была только в модалке).
   Долгое нажатие (зажать ~0.5 c) — открывает модалку с вводом произвольного
   количества (applyQtyAdjust), как было раньше на обычный тап. */
const LONG_PRESS_MS = 500;
const LONG_PRESS_MOVE_TOLERANCE = 12;

function applyQuickAdjust(section, index, mode) {
  const item = itemAt(section, index);
  if (!item) return;
  const key = countKey(section, item);
  pushHistory();
  if (mode === "plus") {
    counts[key] = (counts[key] || 0) + 1;
  } else {
    const newVal = (counts[key] || 0) - 1;
    if (newVal <= 0) {
      sections[section].splice(index, 1);
      delete counts[key];
      delete checked[key];
      selectedCA[section].delete(key);
    } else {
      counts[key] = newVal;
    }
  }
  persistCurrent();
  renderSection(section);
}

function setupQtyLongPress(tbody) {
  let timer = null;
  let longFired = false;
  let activeBtn = null;
  let startX = 0, startY = 0;

  function clearTimer() {
    if (timer) { clearTimeout(timer); timer = null; }
  }
  function reset() {
    clearTimer();
    longFired = false;
    activeBtn = null;
  }
  tbody.addEventListener("pointerdown", (e) => {
    const btn = e.target.closest(".btn.plus, .btn.minus");
    if (!btn) return;
    if (e.pointerType === "mouse" && e.button !== 0) return;
    activeBtn = btn;
    longFired = false;
    startX = e.clientX; startY = e.clientY;
    clearTimer();
    const section = btn.dataset.section;
    const index = Number(btn.dataset.index);
    const mode = btn.classList.contains("plus") ? "plus" : "minus";
    timer = setTimeout(() => {
      longFired = true;
      timer = null;
      if (navigator.vibrate) navigator.vibrate(15);
      openQtyAdjust(section, index, mode);
    }, LONG_PRESS_MS);
  });
  tbody.addEventListener("pointermove", (e) => {
    if (!timer || !activeBtn) return;
    if (Math.abs(e.clientX - startX) > LONG_PRESS_MOVE_TOLERANCE || Math.abs(e.clientY - startY) > LONG_PRESS_MOVE_TOLERANCE) {
      reset();
    }
  });
  tbody.addEventListener("pointerup", (e) => {
    const btn = activeBtn;
    const hadTimer = !!timer;
    clearTimer();
    if (btn && hadTimer && !longFired) {
      const section = btn.dataset.section;
      const index = Number(btn.dataset.index);
      const mode = btn.classList.contains("plus") ? "plus" : "minus";
      applyQuickAdjust(section, index, mode);
    }
    reset();
  });
  tbody.addEventListener("pointercancel", reset);
  tbody.addEventListener("contextmenu", (e) => {
    if (e.target.closest(".btn.plus, .btn.minus")) e.preventDefault();
  });
}
setupQtyLongPress(document.getElementById("body-parts"));
setupQtyLongPress(document.getElementById("body-consumables"));

function handleCountEdit(e) {
  const input = e.target.closest(".countInput");
  if (!input) return;
  const section = input.dataset.section;
  const index = parseInt(input.dataset.index, 10);
  const item = itemAt(section, index);
  if (!item) return;
  const key = countKey(section, item);
  const raw = input.value.trim();
  let val = parseInt(raw, 10);
  if (raw === "" || isNaN(val) || val <= 0) {
    // Стёрли количество (или поставили 0) — позиция целиком удаляется
    // из списка, как и при списании кнопкой «–» до нуля.
    pushHistory();
    sections[section].splice(index, 1);
    delete counts[key];
    delete checked[key];
    selectedCA[section].delete(key);
    persistCurrent();
    renderSection(section);
    return;
  }
  pushHistory();
  counts[key] = val;
  persistCurrent();
  renderSection(section);
}
document.getElementById("body-parts").addEventListener("change", handleCountEdit);
document.getElementById("body-consumables").addEventListener("change", handleCountEdit);
["body-parts","body-consumables"].forEach((id) => {
  document.getElementById(id).addEventListener("keydown", (e) => {
    if (e.key === "Enter" && e.target.classList.contains("countInput")) e.target.blur();
  });
});

/* ==================== Учёт: списать / добавить количество через модалку ====================
   Кнопки «–» и «+» больше не меняют количество напрямую на 1 — открывают
   модалку, где вводится сколько именно списать/добавить. Если после
   списания количество становится 0 или уходит в минус — позиция целиком
   удаляется из списка (отдельной кнопки удаления больше нет). */
let pendingQtyAdjust = null; // { section, index, mode: 'plus'|'minus' }
const qtyAdjustInput = document.getElementById("qtyAdjustInput");

function openQtyAdjust(section, index, mode) {
  const item = itemAt(section, index);
  if (!item) return;
  const key = countKey(section, item);
  const current = counts[key] || 0;
  pendingQtyAdjust = { section, index, mode };
  document.getElementById("qtyAdjustTitle").textContent = (mode === "plus" ? "Добавить: " : "Списать: ") + item.name;
  document.getElementById("qtyAdjustInfo").textContent = "Сейчас в учёте: " + current + " шт."
    + (mode === "minus" ? " Если списать всё (или больше) — позиция будет удалена из списка." : "");
  document.getElementById("qtyAdjustConfirm").textContent = mode === "plus" ? "Добавить" : "Списать";
  document.getElementById("qtyAdjustConfirm").style.background = mode === "plus" ? "var(--accent)" : "var(--danger)";
  qtyAdjustInput.value = "";
  openModal("qtyAdjustOverlay");
  setTimeout(() => qtyAdjustInput.focus(), 50);
}
function closeQtyAdjust() {
  pendingQtyAdjust = null;
  closeModal("qtyAdjustOverlay");
}
document.getElementById("qtyAdjustCancel").addEventListener("click", closeQtyAdjust);
document.getElementById("qtyAdjustOverlay").addEventListener("click", (e) => {
  if (e.target.id === "qtyAdjustOverlay") closeQtyAdjust();
});
function applyQtyAdjust() {
  if (!pendingQtyAdjust) return;
  const { section, index, mode } = pendingQtyAdjust;
  const item = itemAt(section, index);
  if (!item) { closeQtyAdjust(); return; }
  const key = countKey(section, item);
  const val = parseInt(qtyAdjustInput.value, 10);
  if (isNaN(val) || val <= 0) { qtyAdjustInput.focus(); return; }
  pushHistory();
  if (mode === "plus") {
    counts[key] = (counts[key] || 0) + val;
  } else {
    const newVal = (counts[key] || 0) - val;
    if (newVal <= 0) {
      sections[section].splice(index, 1);
      delete counts[key];
      delete checked[key];
      selectedCA[section].delete(key);
    } else {
      counts[key] = newVal;
    }
  }
  persistCurrent();
  renderSection(section);
  closeQtyAdjust();
}
document.getElementById("qtyAdjustConfirm").addEventListener("click", applyQtyAdjust);
qtyAdjustInput.addEventListener("keydown", (e) => {
  if (e.key === "Enter") applyQtyAdjust();
});

/* ==================== Подтверждение удаления (общее для Склада; в «Учёте» удаление
   теперь происходит автоматически при списании количества до 0, см. applyQtyAdjust) ==================== */
let pendingDelete = null; // { kind: 'reset'|'warehouse'|'whReset'|'onec'|'deleteAll', section, index }

/* ==================== Подтверждение удаления/обнуления (общее для 3 вкладок) ====================
   Раньше у каждого вида удаления (склад, 1С, обнулить учёт, обнулить склад,
   удалить всё) была своя копия «записать pendingDelete → показать текст →
   открыть модалку», а обработчик «Да» разруливал их через 5 почти
   одинаковых if. Теперь одна функция открытия + один конфиг действий. */
function openConfirmDelete(kind, text, section, index) {
  pendingDelete = { kind, section, index };
  document.getElementById("confirmText").textContent = text;
  openModal("confirmOverlay");
}
function openConfirmWarehouse(section, index) {
  const item = warehouse[section][index];
  openConfirmDelete("warehouse",
    `Удалить «${item.name}» (${item.code}) со склада? Прошлые записи в учёте сохранятся, но перестанут быть связаны со складом.`,
    section, index);
}
function closeConfirm() {
  pendingDelete = null;
  closeModal("confirmOverlay");
}
document.getElementById("confirmNo").addEventListener("click", closeConfirm);
document.getElementById("confirmOverlay").addEventListener("click", (e) => {
  if (e.target.id === "confirmOverlay") closeConfirm();
});

// Что делает каждый вид подтверждения. pendingDelete доступен внутри
// (section/index — если действие точечное, для «обнулить»/«удалить всё» не нужны).
const DELETE_ACTIONS = {
  whReset: () => {
    pushWhHistory();
    ["parts", "consumables"].forEach((sec) => {
      warehouse[sec].forEach((it) => { it.qty = 0; });
    });
    saveWarehouse();
    renderWarehouseAll();
  },
  warehouse: () => {
    pushWhHistory();
    const { section, index } = pendingDelete;
    const item = warehouse[section][index];
    const key = countKey(section, item);
    warehouse[section].splice(index, 1);
    selectedWH[section].delete(key);
    saveWarehouse();
    renderWarehouseAll();
  },
  onec: () => {
    pushOcHistory();
    const { section, index } = pendingDelete;
    onec[section].splice(index, 1);
    saveOneC();
    renderOneCAll();
  },
  deleteAll: () => {
    pushHistory();
    sections.parts = [];
    sections.consumables = [];
    counts = {};
    checked = {};
    selectedCA.parts.clear();
    selectedCA.consumables.clear();
    persistCurrent();
    renderAll();
  },
};
document.getElementById("confirmYes").addEventListener("click", () => {
  if (!pendingDelete) return;
  const action = DELETE_ACTIONS[pendingDelete.kind];
  if (action) action();
  closeConfirm();
});

document.getElementById("deleteAllBtn").addEventListener("click", () => {
  openConfirmDelete("deleteAll",
    `Удалить ВСЕ позиции (детали и расходники) за ${formatFull(currentDate)}? Счётчики и сами строки будут стёрты полностью. Это можно отменить кнопкой ↶.`);
});

/* ==================== Поиск по складу/1С (автоподбор для панелей добавления/редактирования) ====================
   Раньше searchWarehouse/searchOneC и renderSuggestions/renderOneCSuggestions
   были парами почти одинаковых функций — отличались только тем, в каком
   хранилище искать и что написать, если совпадений нет. Теперь один общий
   поиск и один общий рендер подсказок с этими двумя как параметрами. */
// Разбивает текст на «слова» по любым не-буквенно-цифровым разделителям
// (пробел, /, -, скобки и т.п.) — одинаково и для кириллицы, и для латиницы.
function splitWords(text) {
  return String(text).toLowerCase().split(/[^\p{L}\p{N}]+/u).filter(Boolean);
}
// Поиск «по слову»: каждое слово запроса должно быть НАЧАЛОМ какого-то
// слова в названии — иначе короткий запрос («мк») находил бы буквы внутри
// чужого слова («каМКа»). Код по-прежнему ищется подстрокой в любом месте —
// так удобнее находить позицию по фрагменту кода.
function nameMatchesQuery(name, query) {
  const qWords = splitWords(query);
  if (qWords.length === 0) return false;
  const nWords = splitWords(name);
  return qWords.every((qw) => nWords.some((nw) => nw.startsWith(qw)));
}
function searchStore(store, query) {
  const q = query.trim().toLowerCase();
  if (!q) return [];
  const results = [];
  ["parts", "consumables"].forEach((section) => {
    store[section].forEach((item) => {
      const name = (item.name || "").toLowerCase();
      const code = (item.code || "").toLowerCase();
      if (nameMatchesQuery(name, q) || code.includes(q)) {
        results.push({ section, code: item.code || "", name: item.name, model: item.model || "" });
      }
    });
  });
  return results.slice(0, 8);
}
function renderSuggestionsFrom(box, query, onPick, store, emptyText) {
  const results = searchStore(store, query);
  if (!query.trim()) { box.classList.remove("open"); box.innerHTML = ""; return; }
  if (results.length === 0) {
    box.innerHTML = `<div class="suggest-empty">${emptyText}</div>`;
    box.classList.add("open");
    return;
  }
  box.innerHTML = results.map((r, idx) =>
    `<div class="suggest-item" data-idx="${idx}">
       <span class="code">${escapeHtml(r.code || "—")}</span> — ${escapeHtml(r.name)}${r.model ? `<span class="suggest-model">${escapeHtml(r.model)}</span>` : ""}
     </div>`
  ).join("");
  box.classList.add("open");
  Array.from(box.querySelectorAll(".suggest-item")).forEach((el) => {
    el.addEventListener("click", () => {
      onPick(results[parseInt(el.dataset.idx, 10)]);
      box.classList.remove("open");
      box.innerHTML = "";
    });
  });
}
function renderSuggestions(box, query, onPick) {
  renderSuggestionsFrom(box, query, onPick, warehouse,
    "Совпадений на складе нет — можно добавить как новую позицию без кода.");
}

/* --- Добавление новой позиции в дневной список --- */
let addPicked = null; // { code, name } если выбрано из подсказки
const newItemInput = document.getElementById("newItemName");
const addSuggestBox = document.getElementById("addSuggestBox");
// На iOS при открытии клавиатуры видимая область экрана уменьшается уже
// ПОСЛЕ того, как страница отрисовалась — из-за этого поле может оказаться
// под прилипающей шапкой. Подстраховываемся: после открытия клавиатуры
// прокручиваем поле в центр видимой области.
/* ==================== Панель добавления: подгонка под содержимое ==========
   Ширина «Детали/Расходники» — по выбранному пункту. Фиксированная ширина
   либо оставляла пустое место у короткого «Детали», либо резала длинное
   «Расходники». Меряем текст скрытым span с тем же шрифтом и прибавляем
   место под стрелку списка. */
const sectionSelect = document.getElementById("newItemSection");
let selectProbe = null;
function fitSectionSelectWidth() {
  if (!sectionSelect) return;
  if (!selectProbe) {
    selectProbe = document.createElement("span");
    selectProbe.style.cssText =
      "position:absolute;visibility:hidden;white-space:nowrap;pointer-events:none;left:-9999px;top:0;";
    document.body.appendChild(selectProbe);
  }
  const cs = getComputedStyle(sectionSelect);
  selectProbe.style.font = cs.font || (cs.fontSize + " " + cs.fontFamily);
  selectProbe.textContent = sectionSelect.options[sectionSelect.selectedIndex]?.text || "";
  // Место под стрелку уже заложено в padding-right (стрелку рисуем сами
  // фоном, см. style.css), поэтому здесь только пара пикселей запаса.
  const ARROW = 2;
  sectionSelect.style.width =
    Math.ceil(selectProbe.offsetWidth + parseFloat(cs.paddingLeft) + parseFloat(cs.paddingRight) + ARROW) + "px";
}
if (sectionSelect) {
  sectionSelect.addEventListener("change", fitSectionSelectWidth);
  fitSectionSelectWidth();
  // Шрифты подгружаются позже — пересчитываем, когда они готовы.
  if (document.fonts && document.fonts.ready) document.fonts.ready.then(fitSectionSelectWidth).catch(() => {});
}

/* Крестик очистки поля названия: виден только когда есть что стирать. */
const newItemClearBtn = document.getElementById("newItemClear");
function updateNewItemClear() {
  if (newItemClearBtn) newItemClearBtn.classList.toggle("show", !!newItemInput.value);
}
if (newItemClearBtn) {
  newItemClearBtn.addEventListener("click", () => {
    newItemInput.value = "";
    addPicked = null;
    addSuggestBox.classList.remove("open");
    addSuggestBox.innerHTML = "";
    updateNewItemClear();
    newItemInput.focus();
  });
}
newItemInput.addEventListener("input", updateNewItemClear);
updateNewItemClear();

/* Тап по полю — курсор в конец набранного, как у полей количества. Только
   на фокус: повторным тапом внутри текста курсор можно поставить где нужно. */
newItemInput.addEventListener("focus", () => {
  const end = newItemInput.value.length;
  setTimeout(() => { try { newItemInput.setSelectionRange(end, end); } catch (e) {} }, 0);
  setTimeout(() => {
    newItemInput.scrollIntoView({ block: "center", behavior: "smooth" });
  }, 300);
});
newItemInput.addEventListener("input", () => {
  addPicked = null;
  renderSuggestions(addSuggestBox, newItemInput.value, (r) => {
    addPicked = r;
    newItemInput.value = r.name;
    document.getElementById("newItemSection").value = r.section;
    fitSectionSelectWidth();   // раздел мог смениться — подгоняем ширину
    updateNewItemClear();
  });
});

registerSuggestZone(addSuggestBox, [newItemInput]);

document.getElementById("addItemBtn").addEventListener("click", () => {
  const section = document.getElementById("newItemSection").value;
  const name = newItemInput.value.trim();
  if (!name) return;
  const code = (addPicked && addPicked.name === name) ? addPicked.code : "";
  const qtyInput = document.getElementById("newItemQty");
  let qty = parseInt(qtyInput.value, 10);
  if (isNaN(qty) || qty < 1) qty = 1;   // не указали — считаем как 1
  const clearForm = () => {
    newItemInput.value = "";
    qtyInput.value = "";
    addPicked = null;
    addSuggestBox.classList.remove("open");
    updateNewItemClear();
  };
  // Уникальность позиции определяет КОД, а не название: одинаково
  // названные детали под разные модели (например «Подшипник вилки»
  // с кодами 456JY50-01-0 и 454LB40-01-0) — это разные позиции, и обе
  // должны добавляться. Совпадением считаем только одинаковый код;
  // если кода нет ни у новой, ни у существующей — сравниваем по названию.
  const dupIndex = sections[section].findIndex((it) =>
    code ? (it.code === code) : (!it.code && it.name === name)
  );
  if (dupIndex !== -1) {
    // Позиция уже в списке — ПРИБАВЛЯЕМ количество к набранному. Раньше
    // здесь был отказ «такая позиция уже есть», и дописать ещё столько-то
    // можно было только кнопками +/− в самой строке.
    const existing = sections[section][dupIndex];
    const key = countKey(section, existing);
    pushHistory();
    counts[key] = (counts[key] || 0) + qty;
    persistCurrent();
    clearForm();
    renderSection(section);
    flashRow(rowByDataIndex("body-" + section, dupIndex));
    return;
  }
  pushHistory();
  const newItem = { name, code };
  sections[section].push(newItem);
  counts[countKey(section, newItem)] = qty;
  persistCurrent();
  clearForm();
  renderSection(section);
  // Новая позиция уходит в свою группу по модели, а не в конец списка —
  // ищем её по индексу, иначе подсветилась бы чужая строка.
  flashRow(rowByDataIndex("body-" + section, sections[section].length - 1));
});
newItemInput.addEventListener("keydown", (e) => {
  if (e.key === "Enter") document.getElementById("addItemBtn").click();
});
document.getElementById("newItemQty").addEventListener("keydown", (e) => {
  if (e.key === "Enter") document.getElementById("addItemBtn").click();
});

/* --- Редактирование позиции дневного списка (с поиском по складу) --- */
let editingTarget = null; // { section, index }
let editPicked = null;
const editItemInput = document.getElementById("editItemInput");
const editSuggestBox = document.getElementById("editSuggestBox");
registerSuggestZone(editSuggestBox, [editItemInput]);

function openEditItem(section, index) {
  editingTarget = { section, index };
  editPicked = null;
  const item = itemAt(section, index);
  editItemInput.value = item.name;
  document.getElementById("editItemSection").value = section;
  editSuggestBox.classList.remove("open");
  openModal("editItemOverlay");
  setTimeout(() => { editItemInput.focus(); editItemInput.select(); }, 50);
}
function closeEditItem() {
  editingTarget = null;
  closeModal("editItemOverlay");
}
editItemInput.addEventListener("input", () => {
  editPicked = null;
  renderSuggestions(editSuggestBox, editItemInput.value, (r) => {
    editPicked = r;
    editItemInput.value = r.name;
  });
});
document.getElementById("editItemCancel").addEventListener("click", closeEditItem);
document.getElementById("editItemOverlay").addEventListener("click", (e) => {
  if (e.target.id === "editItemOverlay") closeEditItem();
});
document.getElementById("editItemSave").addEventListener("click", () => {
  if (!editingTarget) return;
  const { section, index } = editingTarget;
  const newSection = document.getElementById("editItemSection").value;
  const oldItem = itemAt(section, index);
  const trimmed = editItemInput.value.trim();
  if (!trimmed) { closeEditItem(); return; }
  const code = (editPicked && editPicked.name === trimmed) ? editPicked.code : "";
  const sectionChanged = newSection !== section;
  if (!sectionChanged && trimmed === oldItem.name && code === oldItem.code) { closeEditItem(); return; }

  // проверка дублей в разделе назначения — по КОДУ (см. пояснение при
  // добавлении): одинаковые названия под разные модели допустимы.
  const clash = sections[newSection].some((it, i) => {
    if (!sectionChanged && i === index) return false;
    return code ? (it.code === code) : (!it.code && it.name === trimmed);
  });
  if (clash) {
    alert(sectionChanged
      ? `Такая позиция уже есть в разделе «${newSection === "parts" ? "Детали" : "Расходники"}».`
      : "Такая позиция уже есть в этом списке.");
    return;
  }

  pushHistory();
  const oldKey = countKey(section, oldItem);
  const newItem = { name: trimmed, code };
  const newKey = countKey(newSection, newItem);
  const savedCount = counts[oldKey] || 0;
  const savedChecked = checked[oldKey] || false;
  const wasPicked = selectedCA[section].has(oldKey);

  if (sectionChanged) {
    sections[section].splice(index, 1);
    sections[newSection].push(newItem);
  } else {
    sections[section][index] = newItem;
  }

  if (newKey !== oldKey) {
    counts[newKey] = savedCount;
    checked[newKey] = savedChecked;
    delete counts[oldKey];
    delete checked[oldKey];
  }
  selectedCA[section].delete(oldKey);
  if (wasPicked) selectedCA[newSection].add(newKey);

  persistCurrent();
  if (sectionChanged) {
    renderAll();
    // подсветим, куда переехала позиция (она в своей группе, не в конце)
    flashRow(rowByDataIndex("body-" + newSection, sections[newSection].length - 1));
  } else {
    renderSection(section);
  }
  closeEditItem();
});
editItemInput.addEventListener("keydown", (e) => {
  if (e.key === "Enter") document.getElementById("editItemSave").click();
});

/* ==================== История выдачи по позиции (только расход по дням) ==================== */
let historyTarget = null; // { key, name }

function collectHistoryEntries(key) {
  const map = {}; // date -> minus (кол-во выдано в этот день)
  Object.keys(dataByDate).forEach((date) => {
    const c = (dataByDate[date] && dataByDate[date].counts) || {};
    if (c[key]) map[date] = (map[date] || 0) + c[key];
  });
  return map;
}

function renderHistoryList() {
  if (!historyTarget) return;
  const from = document.getElementById("historyFrom").value;
  const to = document.getElementById("historyTo").value;
  const map = collectHistoryEntries(historyTarget.key);
  let dates = Object.keys(map).filter((d) => (!from || d >= from) && (!to || d <= to));
  dates.sort().reverse();
  const list = document.getElementById("historyList");
  if (dates.length === 0) {
    list.innerHTML = `<div class="history-empty">Нет данных о выдаче за выбранный период.</div>`;
    return;
  }
  list.innerHTML = dates.map((d) =>
    `<div class="history-row"><span class="h-date">${formatShort(d)}</span><span class="h-minus">−${map[d]} шт</span></div>`
  ).join("");
}

function openHistory(section, item) {
  const key = countKey(section, item);
  historyTarget = { key, name: item.name };
  document.getElementById("historyTitle").textContent = "История выдачи: " + item.name;
  document.getElementById("historyFrom").value = "";
  document.getElementById("historyTo").value = "";
  renderHistoryList();
  openModal("historyOverlay");
}
function closeHistory() {
  historyTarget = null;
  closeModal("historyOverlay");
}
document.getElementById("historyFrom").addEventListener("change", renderHistoryList);
document.getElementById("historyTo").addEventListener("change", renderHistoryList);
document.getElementById("historyClose").addEventListener("click", closeHistory);
document.getElementById("historyOverlay").addEventListener("click", (e) => {
  if (e.target.id === "historyOverlay") closeHistory();
});

/* ==================== Печать этикеток (эталон «большая») ====================
   90x45мм, 2x5=10 на лист. 4 условные строки высотой (10/10/10/15мм):
   1 — модель слева, «N шт» в правом углу; 2-3 — название (1 строка по
   центру обеих, длинное — переносом на 2); 4 — код, жирным, крупным
   шрифтом, растянутым под ширину/высоту строки. Единый дизайн — и для
   разовой печати со склада, и для формы ручного ввода.
   ВАЖНО: используем тот же #printArea + window.print(), что и обычный
   экспорт — открытие через window.open() в установленном на «Домой»
   PWA виснет без адресной строки и кнопки закрытия. */
function fitCodeFontPt(text, maxWidthMm, startPt, minPt) {
  const canvas = fitCodeFontPt._canvas || (fitCodeFontPt._canvas = document.createElement("canvas"));
  const ctx = canvas.getContext("2d");
  const mmToPx = 96 / 25.4;
  const maxWidthPx = maxWidthMm * mmToPx;
  let pt = startPt;
  while (pt > minPt) {
    const px = pt * 96 / 72;
    ctx.font = `700 ${px}px monospace`;
    if (ctx.measureText(text).width <= maxWidthPx) break;
    pt -= 1;
  }
  return pt;
}
function buildLabelHtmlV2(name, code, qty, model) {
  const qtyNum = parseInt(qty, 10);
  const showQty = qty && !isNaN(qtyNum) && qtyNum > 0; // "0" или пусто — не печатаем
  const codeSize = code ? fitCodeFontPt(code, 84, 26, 14) : 26; // 90мм - 2×3мм полей
  return `<div class="print-label v2">
    <div class="pl2-row1">
      <span class="pl2-model">${model ? escapeHtml(model) : ""}</span>
      <span class="pl2-qty">${showQty ? qtyNum + " шт" : ""}</span>
    </div>
    <div class="pl2-name">${escapeHtml(name)}</div>
    ${code ? `<div class="pl2-code" style="font-size:${codeSize}pt;">${escapeHtml(code)}</div>` : ""}
  </div>`;
}
const LABELS_V2_PER_PAGE = 10; // 2x5, 90x45мм
function printLabelsV2(entries) {
  const allLabels = [];
  entries.forEach((e) => {
    const html = buildLabelHtmlV2(e.name, e.code, e.qty, e.model);
    const copies = Math.max(1, e.copies || 1);
    for (let i = 0; i < copies; i++) allLabels.push(html);
  });
  const pages = [];
  for (let i = 0; i < allLabels.length; i += LABELS_V2_PER_PAGE) {
    const slice = allLabels.slice(i, i + LABELS_V2_PER_PAGE).join("");
    pages.push(`<div class="print-labels-page"><div class="print-labels-grid" style="grid-template-columns: repeat(2, 85mm);">${slice}</div></div>`);
  }
  document.getElementById("printArea").innerHTML = pages.join("");
  window.print();
}
function printWarehouseLabels(item) {
  printLabelsV2([{ name: item.name, code: item.code || "", model: item.model || "", copies: LABELS_V2_PER_PAGE }]);
}
function printManualLabels(entries) {
  printLabelsV2(entries);
}

/* --- Модалка ручного ввода этикеток (кнопка «⋮» → «🖨️ Печать») ====================
   Мини-таблица: каждая строка — своя позиция (Название+Шт / Код+Кол-во
   копий), с автоподбором по складу (как в панели добавления «Учёта») и
   крестиком удаления. «+» снизу добавляет ещё строку. */
let printLabelRowSeq = 0;
/* Нумерация строк в форме печати — только для ориентира на экране,
   в сам печатный лист не попадает (нумеруется форма, не этикетки). */
/* Черновик формы печати: набранные позиции сохраняются на устройстве,
   чтобы можно было закрыть окно (или выйти из приложения) и вернуться к
   тому же списку. Очищается только после самой печати. */
const PRINT_DRAFT_KEY = "potreblenie_print_draft_v1";
function savePrintDraft() {
  const rows = Array.from(document.querySelectorAll("#printLabelRows .print-label-row")).map((r) => ({
    name: r.querySelector(".plr-name").value,
    code: r.querySelector(".plr-code").value,
    qty: r.querySelector(".plr-qty").value,
    copies: r.querySelector(".plr-copies").value,
    model: r.querySelector(".plr-model").value
  }));
  // пустой черновик не храним
  const filled = rows.some((r) => r.name || r.code || r.qty || r.copies || r.model);
  try {
    if (filled) localStorage.setItem(PRINT_DRAFT_KEY, JSON.stringify(rows));
    else localStorage.removeItem(PRINT_DRAFT_KEY);
  } catch (e) {}
}
function loadPrintDraft() {
  try { return JSON.parse(localStorage.getItem(PRINT_DRAFT_KEY) || "[]"); } catch (e) { return []; }
}
function clearPrintDraft() {
  try { localStorage.removeItem(PRINT_DRAFT_KEY); } catch (e) {}
}
function renumberPrintLabelRows() {
  document.querySelectorAll("#printLabelRows .print-label-row").forEach((row, i) => {
    const el = row.querySelector(".plr-num");
    if (el) el.textContent = String(i + 1);
  });
}
function createPrintLabelRow(data) {
  const id = "plr" + (++printLabelRowSeq);
  const row = document.createElement("div");
  row.className = "print-label-row";
  row.dataset.rowId = id;
  row.innerHTML = `
    <button class="plr-del" title="Удалить эту позицию">✕</button>
    <div class="plr-line">
      <div class="plr-name-wrap">
        <input type="text" class="plr-name" placeholder="Название *" autocomplete="off">
        <div class="suggest-box plr-suggest"></div>
      </div>
      <div class="plr-side">
        <input type="number" class="plr-qty" min="0" placeholder="Шт">
      </div>
    </div>
    <div class="plr-line" style="margin-top:6px;">
      <div class="plr-name-wrap">
        <input type="text" class="plr-code" placeholder="Код (необязательно)" autocomplete="off">
      </div>
      <div class="plr-side">
        <input type="number" class="plr-copies" min="1" placeholder="Копий">
      </div>
    </div>
    <div class="plr-line" style="margin-top:6px;">
      <div class="plr-name-wrap">
        <input type="text" class="plr-model" placeholder="Модель (напр. YGW 5.0)" autocomplete="off">
      </div>
      <div class="plr-side"></div>
    </div>
    <span class="plr-num"></span>
  `;
  const nameInput = row.querySelector(".plr-name");
  const codeInput = row.querySelector(".plr-code");
  const modelInput = row.querySelector(".plr-model");
  const qtyInput = row.querySelector(".plr-qty");
  const copiesInput = row.querySelector(".plr-copies");
  const suggestBox = row.querySelector(".plr-suggest");
  if (data) {
    nameInput.value = data.name || "";
    codeInput.value = data.code || "";
    modelInput.value = data.model || "";
    qtyInput.value = data.qty || "";
    copiesInput.value = data.copies || "";
  }
  // любое изменение — сразу в черновик
  [nameInput, codeInput, modelInput, qtyInput, copiesInput].forEach((el) => {
    el.addEventListener("input", savePrintDraft);
  });
  // Подсказка остаётся в обычном потоке — прямо внутри своей строки, под
  // полем названия (position:absolute относительно .plr-name-wrap, см. CSS).
  // Раньше её выносили в <body> и считали координаты через JS, потому что
  // её обрезал скролл-контейнер строк; тот контейнер убран, и теперь
  // никаких координат считать не нужно — подсказка физически привязана к
  // полю и не может от него отстать или накрыть его при появлении
  // клавиатуры.
  let picked = null;
  function handleInput() {
    picked = null;
    renderSuggestions(suggestBox, nameInput.value || codeInput.value, (r) => {
      picked = r;
      nameInput.value = r.name;
      codeInput.value = r.code || "";
      modelInput.value = r.model || ""; // модель — как и код, подставляется автоматически из склада
      suggestBox.classList.remove("open");
      savePrintDraft();
    });
  }
  nameInput.addEventListener("input", handleInput);
  codeInput.addEventListener("input", handleInput);
  registerSuggestZone(suggestBox, [nameInput, codeInput]);
  row.querySelector(".plr-del").addEventListener("click", () => {
    const rows = document.querySelectorAll(".print-label-row");
    if (rows.length <= 1) {
      // последнюю строку не удаляем совсем — просто очищаем поля
      if (!confirm("Очистить эту позицию?")) return;
      nameInput.value = ""; codeInput.value = ""; modelInput.value = "";
      row.querySelector(".plr-qty").value = "";
      row.querySelector(".plr-copies").value = "";
      savePrintDraft();
      return;
    }
    if (!confirm("Удалить эту позицию?")) return;
    row.remove();   // подсказка лежит внутри строки и удалится вместе с ней
    renumberPrintLabelRows();
    savePrintDraft();
  });
  return row;
}
function openPrintLabelModal() {
  const container = document.getElementById("printLabelRows");
  container.innerHTML = "";
  const draft = loadPrintDraft();
  if (draft.length === 0) container.appendChild(createPrintLabelRow());
  else draft.forEach((d) => container.appendChild(createPrintLabelRow(d)));
  renumberPrintLabelRows();
  openModal("printLabelOverlay");
}
function closePrintLabelModal() {
  closeModal("printLabelOverlay");
}
document.getElementById("printLabelBtn").addEventListener("click", openPrintLabelModal);
document.getElementById("printLabelCancel").addEventListener("click", closePrintLabelModal);
document.getElementById("printLabelOverlay").addEventListener("click", (e) => {
  if (e.target.id !== "printLabelOverlay") return;
  // Защита от случайного тапа мимо формы — спрашиваем подтверждение,
  // а не закрываем сразу (иначе введённые позиции терялись без вопросов).
  // Позиции теперь сохраняются в черновик, поэтому закрытие безопасно —
  // подтверждение оставлено только чтобы не закрывать случайным тапом.
  if (!confirm("Закрыть окно печати? Набранные позиции сохранятся.")) return;
  closePrintLabelModal();
});
document.getElementById("printLabelAddRow").addEventListener("click", () => {
  document.getElementById("printLabelRows").appendChild(createPrintLabelRow());
  renumberPrintLabelRows();
  savePrintDraft();
});
document.getElementById("printLabelGo").addEventListener("click", () => {
  const rows = Array.from(document.querySelectorAll(".print-label-row"));
  const entries = [];
  let firstEmptyNameInput = null;
  rows.forEach((row) => {
    const name = row.querySelector(".plr-name").value.trim();
    const code = row.querySelector(".plr-code").value.trim();
    const qty = row.querySelector(".plr-qty").value.trim();
    const model = row.querySelector(".plr-model").value.trim();
    const copiesRaw = row.querySelector(".plr-copies").value.trim();
    const copies = copiesRaw ? Math.max(1, parseInt(copiesRaw, 10) || 1) : 1;
    if (!name) {
      if (!firstEmptyNameInput) firstEmptyNameInput = row.querySelector(".plr-name");
      return; // пустые строки (без названия) просто пропускаем
    }
    entries.push({ name, code, qty, model, copies });
  });
  if (entries.length === 0) {
    alert("Заполните название хотя бы для одной позиции.");
    if (firstEmptyNameInput) firstEmptyNameInput.focus();
    return;
  }
  closePrintLabelModal();
  clearPrintDraft();      // напечатали — начинаем со свежего списка
  printManualLabels(entries);
});

/* ==================== Переключение даты ==================== */
const dateInput = document.getElementById("reportDate");
dateInput.value = currentDate;
dateInput.addEventListener("change", () => {
  const newDate = dateInput.value;
  if (!newDate || newDate === currentDate) return;
  persistCurrentLocalOnly();
  currentDate = newDate;
  localStorage.setItem(LAST_DATE_KEY, currentDate);
  const entry = entryFor(currentDate);
  sections = entry.sections;
  counts = entry.counts;
  checked = entry.checked;
  caHistory.reset();
  renderAll();
});

/* --- Надёжное открытие календаря по клику/тапу в любом месте плашки с датой --- */
const datePickerBox = document.getElementById("datePickerBox");
datePickerBox.addEventListener("click", () => {
  try {
    if (typeof dateInput.showPicker === "function") {
      dateInput.showPicker();
    } else {
      dateInput.focus();
    }
  } catch (err) {
    dateInput.focus();
  }
});

/* ==================== Экспорт в PDF (через печать) ==================== */
function buildExportTable(title, section) {
  const rows = sections[section]
    .map((item) => [item.name, item.code, counts[countKey(section, item)] || 0, !!checked[countKey(section, item)]])
    .filter(([, , qty]) => qty > 0);
  if (rows.length === 0) return "";
  const rowsHtml = rows.map(([name, code, qty]) =>
    `<tr><td>${escapeHtml(name)}</td><td>${code ? escapeHtml(code) : "—"}</td><td style="text-align:center">${qty}</td></tr>`
  ).join("");
  return `
    <h3>${title}</h3>
    <table>
      <thead><tr><th>Наименование</th><th style="width:130px;">Код</th><th style="width:70px;">Кол-во</th></tr></thead>
      <tbody>${rowsHtml}</tbody>
    </table>`;
}
function runConsumptionExport() {
  const partsHtml = buildExportTable("Детали", "parts");
  const consumHtml = buildExportTable("Расходники", "consumables");
  if (!partsHtml && !consumHtml) {
    alert("За выбранную дату ещё нет выданных позиций (все счётчики равны 0).");
    return;
  }
  const printArea = document.getElementById("printArea");
  printArea.innerHTML = `
    <h1>Учёт потребления деталей</h1>
    <div class="print-date">Дата: ${escapeHtml(formatFull(currentDate))}</div>
    ${partsHtml}
    ${consumHtml}
  `;
  window.print();
}

/* ==================== ВКЛАДКА СКЛАД: логика ==================== */
/* ==================== Фильтр склада по модели ====================
   Всё / YGW 5.0 / YGW 4.0 / 9Bot. Только просмотр: данные не меняются,
   синхронизация не запускается, выбор запоминается на устройстве. */
const WH_FILTER_KEY = "potreblenie_wh_filter_v1";
let whFilterModel = (function () {
  try { return localStorage.getItem(WH_FILTER_KEY) || "all"; } catch (e) { return "all"; }
})();
function saveWhFilter() {
  try { localStorage.setItem(WH_FILTER_KEY, whFilterModel); } catch (e) {}
}
// Пары { item, i } — исходный индекс нужен, иначе кнопки в строках стали бы
// указывать не на те позиции.
function whFilteredPairs(section) {
  const pairs = warehouse[section].map((item, i) => ({ item, i }));
  if (whFilterModel === "all") return pairs;
  return pairs.filter(({ item }) => (item.model || "") === whFilterModel);
}
function renderWarehouseSection(section) {
  // Индекс код→позиция должен быть свежим ДО построения строк: иначе после
  // сортировки/добавления/редактирования (которые меняют порядок или состав
  // массива, но не всегда идут через renderWarehouseAll/renderOneCAll) тут
  // читался бы старый индекс — переход между «Складом» и «1С» и колонки
  // сравнения/остатка указывали бы не на ту позицию.
  rebuildCrossIndexes();
  const tbody = document.getElementById("wh-body-" + section);
  const picked = selectedWH[section];
  tbody.innerHTML = whFilteredPairs(section).map(({ item, i }) => {
    const key = countKey(section, item);
    const ocInfo = oneCInfoFor(item.code);
    return `<tr data-key="${escapeHtml(key)}" data-index="${i}"${picked.has(key) ? ' class="row-picked"' : ""}>
      <td class="num">${i + 1}</td>
      <td class="code wh-code">${item.code ? `<span class="code-text" data-code="${escapeHtml(item.code)}">${formatCodeDisplay(item.code)}</span>` : "—"}${item.model ? `<span class="wh-model-badge">${escapeHtml(item.model)}</span>` : ""}</td>
      <td class="name"><div class="main-name">${escapeHtml(item.name)}</div>${ocInfo ? altNameHtml(item.code, ocInfo, true) : ""}</td>
      <td class="count-merged wh-merged">
        <div class="count-tier count-tier-num">
          <input type="number" min="0" value="${item.qty || 0}" class="whQtyInput"
            data-section="${section}" data-index="${i}">
        </div>
        <div class="count-tier count-tier-oc">${ocInfo ? ocInfo.qty : "—"}</div>
      </td>
      <td class="buttons wh-buttons">
        <div class="menu-wrap wh-menu-wrap">
          <button class="iconbtn wh-menu-btn" data-section="${section}" data-index="${i}" title="Действия">⋮</button>
          <div class="menu-panel wh-menu-panel">
            <button class="wh-edit" data-section="${section}" data-index="${i}" title="Редактировать">✎</button>
            <button class="wh-history" data-section="${section}" data-index="${i}" title="История выдачи">🕘</button>
            <button class="wh-print" data-section="${section}" data-index="${i}" title="Печать этикеток">🖨️</button>
            <button class="wh-del danger" data-section="${section}" data-index="${i}" title="Удалить со склада">✕</button>
          </div>
        </div>
      </td>
    </tr>`;
  }).join("");
  updateWarehouseTotals(section);
  if (typeof applyWhSearch === "function") applyWhSearch();
}
function renderWarehouseAll() {
  rebuildCrossIndexes();
  renderWarehouseSection("parts");
  renderWarehouseSection("consumables");
  updateWhFilterUI();
}
/* Общая для «Склада» и «1С»: прячет пустой после фильтра раздел (таблицу и
   заголовок), кроме шапки с кнопкой фильтра — иначе из режима, где раздел
   пуст, нельзя было бы переключиться обратно. Заголовок раздела — только
   элемент прямо перед таблицей: идти назад «до первого .section-head»
   нельзя, у «Расходников» заголовок — простой h2, и поиск добирался бы до
   шапки «Деталей», пряча её вместе с кнопкой фильтра. */
function hideEmptySections(bodyPrefix, filterBtnId) {
  ["parts", "consumables"].forEach((section) => {
    const tbody = document.getElementById(bodyPrefix + section);
    if (!tbody) return;
    const empty = tbody.children.length === 0;
    const table = tbody.closest(".table-scroll");
    if (table) table.style.display = empty ? "none" : "";
    const head = table ? table.previousElementSibling : null;
    const hasFilter = head && head.querySelector && head.querySelector("#" + filterBtnId);
    if (head && !hasFilter &&
        (head.classList.contains("section-head") || head.classList.contains("section-title"))) {
      head.style.display = empty ? "none" : "";
    }
  });
}
function updateWhFilterUI() {
  const label = document.getElementById("whFilterLabel");
  if (label) {
    const shown = whFilteredPairs("parts").length + whFilteredPairs("consumables").length;
    label.textContent = (whFilterModel === "all" ? "Все" : whFilterModel) + " · " + shown;
  }
  document.querySelectorAll("#whFilterPanel [data-model]").forEach((b) => {
    b.classList.toggle("primary", b.dataset.model === whFilterModel);
  });
  hideEmptySections("wh-body-", "whFilterBtn");
}
document.getElementById("whFilterBtn").addEventListener("click", (e) => {
  e.stopPropagation();
  document.getElementById("whFilterPanel").classList.toggle("open");
});
document.querySelectorAll("#whFilterPanel [data-model]").forEach((btn) => {
  btn.addEventListener("click", () => {
    whFilterModel = btn.dataset.model;
    saveWhFilter();
    document.getElementById("whFilterPanel").classList.remove("open");
    renderWarehouseAll();      // только перерисовка, без синхронизации
    applyWhSearch();           // строки перерисованы — заново подсветим найденное
  });
});
document.addEventListener("click", (e) => {
  if (!e.target.closest(".wh-filter-wrap")) {
    const p = document.getElementById("whFilterPanel");
    if (p) p.classList.remove("open");
  }
});

/* Меню строки лежит внутри таблицы, а у контейнера таблицы overflow-x:auto —
   браузер при этом обрезает и по вертикали, поэтому у нижних строк меню
   уходило под край и было не видно. Если снизу не хватает места —
   раскрываем меню вверх. */
/* Меню строки лежит внутри таблицы, а контейнер таблицы обрезает всё, что
   выходит за его край (из-за overflow-x:auto браузер обрезает и по
   вертикали). Раскрытия «вверх» не хватало: у предпоследних строк меню всё
   равно упиралось в край. Поэтому переводим панель в режим «поверх
   страницы» (position: fixed) и сами считаем координаты — обрезать её
   тогда физически нечем. */
function placeRowMenu(panel) {
  const btn = panel.parentElement;
  const r = btn.getBoundingClientRect();
  panel.classList.add("floating");
  panel.style.position = "fixed";
  panel.style.top = "0px";
  panel.style.left = "0px";
  panel.style.bottom = "auto";
  panel.style.right = "auto";
  const w = panel.offsetWidth || 150;
  const h = panel.offsetHeight || 150;
  const pad = 8;
  // по горизонтали — прижимаем правый край панели к правому краю кнопки
  let left = r.right - w;
  left = Math.max(pad, Math.min(left, window.innerWidth - w - pad));
  // по вертикали — под кнопкой, а если не помещается, то над ней
  let top = r.bottom + 6;
  if (top + h > window.innerHeight - pad) {
    const above = r.top - h - 6;
    top = (above >= pad) ? above : Math.max(pad, window.innerHeight - h - pad);
  }
  panel.style.left = left + "px";
  panel.style.top = top + "px";
}
/* Возвращаем панель в обычное состояние при закрытии, иначе она осталась бы
   висеть в углу экрана. */
function resetRowMenu(panel) {
  panel.classList.remove("floating", "drop-up");
  panel.style.position = "";
  panel.style.top = panel.style.left = panel.style.bottom = panel.style.right = "";
}
/* Общая для «Склада» и «1С»: закрывает все открытые меню строк заданного
   класса, кроме `except` (обычно — только что открытое). */
function closeAllRowMenus(panelClass, except) {
  document.querySelectorAll("." + panelClass + ".open").forEach((p) => {
    if (p !== except) { p.classList.remove("open"); resetRowMenu(p); }
  });
}
function closeAllWhMenus(except) { closeAllRowMenus("wh-menu-panel", except); }
/* Общая для «Склада» и «1С»: открывает/закрывает меню строки по кнопке «⋮»,
   предварительно закрыв остальные такие же меню. */
function toggleRowMenu(btn, panelClass, closeAllFn) {
  const panel = btn.parentElement.querySelector("." + panelClass);
  const willOpen = !panel.classList.contains("open");
  closeAllFn();
  panel.classList.toggle("open", willOpen);
  if (willOpen) placeRowMenu(panel); else resetRowMenu(panel);
}
function handleWarehouseClick(e) {
  const section = e.currentTarget.id.replace("wh-body-", "");
  const btn = e.target.closest("button");
  if (btn) {
    const index = parseInt(btn.dataset.index, 10);
    if (btn.classList.contains("wh-menu-btn")) {
      e.stopPropagation();
      toggleRowMenu(btn, "wh-menu-panel", closeAllWhMenus);
      return;
    }
    closeAllWhMenus();
    if (btn.classList.contains("wh-del")) {
      openConfirmWarehouse(section, index);
    } else if (btn.classList.contains("wh-edit")) {
      openWhEdit(section, index);
    } else if (btn.classList.contains("wh-history")) {
      openHistory(section, warehouse[section][index]);
    } else if (btn.classList.contains("wh-print")) {
      printWarehouseLabels(warehouse[section][index]);
    }
    return;
  }
  const link = e.target.closest(".alt-link");
  if (link) {
    // Индекс берём из самой строки, а НЕ из её позиции среди отрисованных:
    // при включённом фильтре по модели на экране лишь часть склада, и позиция
    // указывала бы на чужую позицию массива.
    const tr = link.closest("tr");
    const index = parseInt(tr.dataset.index, 10);
    const item = warehouse[section][index];
    if (!item) return;
    jumpToOneC(item.code, section, index);
    return;
  }
  const nameTd = e.target.closest("td.name");
  if (nameTd) {
    if (String(window.getSelection())) return;      // идёт выделение текста — не переключаем
    const tr = nameTd.closest("tr");
    toggleRowPick(selectedWH, section, tr.dataset.key, tr);
  }
}
document.getElementById("wh-body-parts").addEventListener("click", handleWarehouseClick);
document.getElementById("wh-body-consumables").addEventListener("click", handleWarehouseClick);
document.addEventListener("click", (e) => {
  if (!e.target.closest(".wh-menu-wrap")) closeAllWhMenus();
  if (!e.target.closest(".oc-menu-wrap")) closeAllOcMenus();
});

function handleWarehouseQtyEdit(e) {
  const input = e.target.closest(".whQtyInput");
  if (!input) return;
  const section = input.dataset.section;
  const index = parseInt(input.dataset.index, 10);
  let val = parseInt(input.value, 10);
  if (isNaN(val) || val < 0) val = 0;
  if (warehouse[section][index].qty === val) { renderWarehouseSection(section); return; }
  pushWhHistory();
  warehouse[section][index].qty = val;
  saveWarehouse();
  renderWarehouseSection(section);
}
document.getElementById("wh-body-parts").addEventListener("change", handleWarehouseQtyEdit);
document.getElementById("wh-body-consumables").addEventListener("change", handleWarehouseQtyEdit);
["wh-body-parts","wh-body-consumables"].forEach((id) => {
  document.getElementById(id).addEventListener("keydown", (e) => {
    if (e.key === "Enter" && e.target.classList.contains("whQtyInput")) e.target.blur();
  });
});

/* ==================== Переключатель модели (3 положения) ====================
   Общие функции для .model-switch: читаем/ставим значение и подсвечиваем
   активную кнопку. Используется и в модалке редактирования склада, и в
   панели добавления. */
function setModelSwitch(el, value) {
  const v = value || "";
  el.dataset.value = v;
  el.querySelectorAll(".ms-opt").forEach((btn) => {
    btn.classList.toggle("active", (btn.dataset.model || "") === v);
  });
}
function getModelSwitch(el) { return el.dataset.value || ""; }
document.querySelectorAll(".model-switch").forEach((el) => {
  el.addEventListener("click", (e) => {
    const btn = e.target.closest(".ms-opt");
    if (!btn) return;
    e.preventDefault();
    setModelSwitch(el, btn.dataset.model || "");
  });
  setModelSwitch(el, "");
});

/* --- Редактирование позиции склада (код/название/кол-во) --- */
let whEditingTarget = null;
function openWhEdit(section, index) {
  whEditingTarget = { section, index };
  const item = warehouse[section][index];
  document.getElementById("whEditSection").value = section;
  document.getElementById("whEditCode").value = item.code;
  document.getElementById("whEditName").value = item.name;
  document.getElementById("whEditQty").value = item.qty || 0;
  setModelSwitch(document.getElementById("whEditModel"), item.model || "");
  const box = document.getElementById("whEditSuggestBox");
  if (box) { box.classList.remove("open"); box.innerHTML = ""; }
  openModal("whEditOverlay");
}
function closeWhEdit() {
  whEditingTarget = null;
  const box = document.getElementById("whEditSuggestBox");
  if (box) { box.classList.remove("open"); box.innerHTML = ""; }
  closeModal("whEditOverlay");
}
document.getElementById("whEditCancel").addEventListener("click", closeWhEdit);
document.getElementById("whEditOverlay").addEventListener("click", (e) => {
  if (e.target.id === "whEditOverlay") closeWhEdit();
});
document.getElementById("whEditSave").addEventListener("click", () => {
  if (!whEditingTarget) return;
  const { section, index } = whEditingTarget;
  const newSection = document.getElementById("whEditSection").value;
  const code = document.getElementById("whEditCode").value.trim();
  const name = document.getElementById("whEditName").value.trim();
  let qty = parseInt(document.getElementById("whEditQty").value, 10);
  if (isNaN(qty) || qty < 0) qty = 0;
  const model = getModelSwitch(document.getElementById("whEditModel"));
  if (!name) { closeWhEdit(); return; }
  const sectionChanged = newSection !== section;

  // проверка дублей по коду в разделе назначения
  if (code) {
    const clash = warehouse[newSection].some((it, i) => {
      if (!sectionChanged && i === index) return false;
      return it.code === code;
    });
    if (clash) {
      alert(`Позиция с кодом «${code}» уже есть в разделе «${newSection === "parts" ? "Детали" : "Расходники"}».`);
      return;
    }
  }

  pushWhHistory();
  const oldItem = warehouse[section][index];
  const oldKey = countKey(section, oldItem);
  const newItem = { code, name, qty, model };
  const newKey = countKey(newSection, newItem);
  const wasPicked = selectedWH[section].has(oldKey);

  if (sectionChanged) {
    warehouse[section].splice(index, 1);
    warehouse[newSection].push(newItem);
  } else {
    warehouse[section][index] = newItem;
  }

  selectedWH[section].delete(oldKey);
  if (wasPicked) selectedWH[newSection].add(newKey);

  syncModelByCode(code, model);   // модель едина для склада и 1С
  saveWarehouse();
  saveOneCLocalOnly();
  renderOneCAll();
  if (sectionChanged) {
    renderWarehouseAll();
    // подсветим, куда переехала позиция
    const tbody = document.getElementById("wh-body-" + newSection);
    const rows = tbody.querySelectorAll("tr");
    flashRow(rows[rows.length - 1]);
  } else {
    renderWarehouseSection(section);
  }
  closeWhEdit();
});

/* --- Добавление новой позиции на склад --- */
document.getElementById("whAddBtn").addEventListener("click", () => {
  const section = document.getElementById("whNewSection").value;
  const code = document.getElementById("whNewCode").value.trim();
  const name = document.getElementById("whNewName").value.trim();
  const model = getModelSwitch(document.getElementById("whNewModel"));
  let qty = parseInt(document.getElementById("whNewQty").value, 10);
  if (isNaN(qty) || qty < 0) qty = 0;
  if (!name) return;
  pushWhHistory();
  const newItem = { code, name, qty, model };
  warehouse[section].push(newItem);
  if (model && syncModelByCode(code, model)) {   // если такой код есть в 1С — подтянем модель и туда
    saveOneCLocalOnly();
    renderOneCAll();
  }
  saveWarehouse();
  document.getElementById("whNewCode").value = "";
  document.getElementById("whNewName").value = "";
  setModelSwitch(document.getElementById("whNewModel"), "");
  document.getElementById("whNewQty").value = "0";
  const addBox = document.getElementById("whAddSuggestBox");
  if (addBox) { addBox.classList.remove("open"); addBox.innerHTML = ""; }
  renderWarehouseSection(section);
});

/* ==================== Переключение вкладок ==================== */
const TAB_LABELS = { consumption: "Учёт", warehouse: "Склад", onec: "1C" };
function switchTab(target) {
  // 1) Сначала — гарантированное переключение вида. Ничего, что может упасть.
  const map = {
    consumption: ["tabConsumptionBtn", "viewConsumption"],
    warehouse:   ["tabWarehouseBtn",   "viewWarehouse"],
    onec:        ["tabOneCBtn",        "viewOneC"],
  };
  if (!map[target]) target = "consumption";
  Object.keys(map).forEach((k) => {
    const [btnId, viewId] = map[k];
    const on = (k === target);
    document.getElementById(btnId).classList.toggle("active", on);
    document.getElementById(viewId).classList.toggle("active", on);
  });
  const label = document.getElementById("tabsCurrentLabel");
  if (label) label.textContent = TAB_LABELS[target];
  const panel = document.getElementById("tabsMenuPanel");
  if (panel) panel.classList.remove("open");

  // 2) Остальное — в защищённом блоке: даже если что-то упадёт,
  //    вкладка уже переключилась и экран не «залипнет».
  try {
    showSearchBarFor(target);
    // Дата-пикер теперь виден на всех вкладках — шапка одинаковая
    // везде (сам он всегда управляет датой «Учёта», даже если открыта
    // «Склад»/«1С» — эти вкладки от даты не зависят, так что показ поля
    // им не мешает).


    if (backTarget && target !== backTarget.dest) hideBackBtn();
    if (target === "warehouse") renderWarehouseAll();
    if (target === "onec") renderOneCAll();
  } catch (err) {
    console.error("Ошибка при переключении вкладки:", err);
  }
}

function tabTargetFromId(id) {
  if (id === "tabWarehouseBtn") return "warehouse";
  if (id === "tabOneCBtn") return "onec";
  return "consumption";
}
document.getElementById("tabsCurrentBtn").addEventListener("click", (e) => {
  e.stopPropagation();
  document.getElementById("tabsMenuPanel").classList.toggle("open");
});
document.addEventListener("click", (e) => {
  const panel = document.getElementById("tabsMenuPanel");
  if (!panel.classList.contains("open")) return;
  if (e.target.closest("#tabsMenuPanel") || e.target.closest("#tabsCurrentBtn")) return;
  panel.classList.remove("open");
});


/* ==================== Сортировка по клику на заголовок столбца ====================
   Учёт: Наименование (алфавит), Код (по первым 3 цифрам), Кол-во (числовое).
   Склад: то же + На складе / Выдано / Остаток (числовые). */
const sortState = {
  ca: { parts: { col: null, dir: null }, consumables: { col: null, dir: null } },
  wh: { parts: { col: null, dir: null }, consumables: { col: null, dir: null } },
  oc: { parts: { col: null, dir: null }, consumables: { col: null, dir: null } }
};

function codeSortValue(code) {
  const digits = (code || "").replace(/\D/g, "");
  return digits ? parseInt(digits.slice(0, 3), 10) : -1;
}
function defaultDirFor(col) {
  return (col === "name" || col === "code") ? "asc" : "desc";
}
function getSortValue(table, section, item, col) {
  if (col === "name") return item.name.toLowerCase();
  if (col === "code") return codeSortValue(item.code);
  if (table === "ca" && col === "count") return counts[countKey(section, item)] || 0;
  if ((table === "wh" || table === "oc") && col === "qty") return item.qty || 0;
  if (table === "oc" && col === "whqty") {
    const info = warehouseInfoFor(item.code);
    return info ? info.qty : -1;
  }
  if (table === "oc" && col === "diff") {
    const info = warehouseInfoFor(item.code);
    return info ? info.qty - (item.qty || 0) : 0;
  }
  if (table === "wh" && col === "ocqty") {
    const info = oneCInfoFor(item.code);
    return info ? info.qty : -1;
  }
  return 0;
}
function updateSortArrows(table, section) {
  document.querySelectorAll(`th.sortable[data-table="${table}"][data-section="${section}"]`).forEach((th) => {
    const col = th.dataset.col;
    const state = sortState[table][section];
    const arrow = th.querySelector(".sort-arrow");
    th.classList.toggle("sort-active", state.col === col);
    arrow.textContent = state.col === col ? (state.dir === "asc" ? "↑" : "↓") : "↕";
  });
}
/* Порядок сортировки запоминается между запусками: сама сортировка меняет
   порядок в массиве и сохраняется ЛОКАЛЬНО (на сервер не уходит, чтобы не
   дёргать синхронизацию). Но данные с сервера приходят в своём порядке и
   затирали результат — со стороны это выглядело как «сортировка не
   срабатывает». Поэтому выбор храним отдельно и переприменяем после
   каждого получения данных (см. reapplySavedSort). */
const SORT_STATE_KEY = "potreblenie_sort_state_v1";
function saveSortState() {
  try { localStorage.setItem(SORT_STATE_KEY, JSON.stringify(sortState)); } catch (e) {}
}
function loadSortState() {
  try {
    const raw = localStorage.getItem(SORT_STATE_KEY);
    if (!raw) return;
    const saved = JSON.parse(raw);
    ["ca", "wh", "oc"].forEach((t) => {
      ["parts", "consumables"].forEach((s) => {
        if (saved[t] && saved[t][s]) sortState[t][s] = saved[t][s];
      });
    });
  } catch (e) {}
}
function applySortToList(table, section) {
  const state = sortState[table][section];
  if (!state.col) return false;
  const list = table === "ca" ? sections[section] : (table === "oc" ? onec[section] : warehouse[section]);
  if (!list) return false;
  list.sort((a, b) => {
    const va = getSortValue(table, section, a, state.col);
    const vb = getSortValue(table, section, b, state.col);
    const cmp = (typeof va === "string") ? va.localeCompare(vb, "ru") : (va - vb);
    return state.dir === "asc" ? cmp : -cmp;
  });
  return true;
}
// Переприменяет сохранённый порядок ко всем таблицам — вызывается после
// получения данных с сервера.
function reapplySavedSort() {
  ["ca", "wh", "oc"].forEach((t) => {
    ["parts", "consumables"].forEach((s) => applySortToList(t, s));
  });
  ["ca", "wh", "oc"].forEach((t) => {
    ["parts", "consumables"].forEach((s) => updateSortArrows(t, s));
  });
}
function sortTable(table, section, col) {
  const state = sortState[table][section];
  const dir = state.col === col ? (state.dir === "asc" ? "desc" : "asc") : defaultDirFor(col);
  if (table === "ca") pushHistory();
  if (table === "wh") pushWhHistory();
  if (table === "oc") pushOcHistory();
  state.col = col;
  state.dir = dir;
  saveSortState();
  applySortToList(table, section);
  if (table === "ca") {
    persistCurrentLocalOnly();
    renderSection(section);
  } else if (table === "oc") {
    saveOneCLocalOnly();
    renderOneCSection(section);
  } else {
    saveWarehouseLocalOnly();
    renderWarehouseSection(section);
  }
  updateSortArrows(table, section);
}
document.querySelectorAll("th.sortable").forEach((th) => {
  th.addEventListener("click", () => {
    sortTable(th.dataset.table, th.dataset.section, th.dataset.col);
  });
});

/* ==================== Поиск: панель активной вкладки видна всегда ====================
   Раньше applyCaSearch/applyWhSearch/applyOcSearch были тремя отдельными
   функциями-обёртками над runSearch(), каждая со своей подпиской на
   ввод текста. Теперь один конфиг на все три вкладки; сами функции
   applyCaSearch/applyWhSearch/applyOcSearch оставлены (их вызывают из
   других мест файла), но внутри — тонкие обёртки. */
const SEARCH_TABS = [
  { key: "ca", bodyPrefix: "body-",    getStore: () => sections },
  { key: "wh", bodyPrefix: "wh-body-", getStore: () => warehouse },
  { key: "oc", bodyPrefix: "oc-body-", getStore: () => onec },
];
const searchBars = {};
const searchInputs = {};
const applySearchFns = {};
SEARCH_TABS.forEach(({ key, bodyPrefix, getStore }) => {
  const bar = document.getElementById(key + "SearchBar");
  const input = document.getElementById(key + "SearchInput");
  searchBars[key] = bar;
  searchInputs[key] = input;
  const apply = () => runSearch(key, bodyPrefix, getStore(), nameOrCode);
  applySearchFns[key] = apply;
  input.addEventListener("input", apply);
});
function applyCaSearch() { applySearchFns.ca(); }
function applyWhSearch() { applySearchFns.wh(); }
function applyOcSearch() { applySearchFns.oc(); }

/* Панель добавления перенесена в общую шапку, поэтому её видимость больше не
   определяется вкладкой автоматически (раньше она лежала внутри «Учёта»).
   Показываем её только на «Учёте» и только когда устройство не в режиме
   просмотра, и сразу пересчитываем высоту распорки под шапкой. */
let currentTab = "consumption";
function updateAddPanelVisibility() {
  const panel = document.getElementById("addPanelConsumption");
  if (!panel) return;
  panel.style.display = (currentTab === "consumption" && !isGuestMode()) ? "flex" : "none";
  syncStickyTopHeight();
}

/* Показывает строку поиска нужной вкладки, остальные прячет и очищает */
function showSearchBarFor(tab) {
  currentTab = tab;
  const map = {
    consumption: "ca",
    warehouse: "wh",
    onec: "oc",
  };
  Object.keys(map).forEach((tabName) => {
    const key = map[tabName];
    const bar = searchBars[key];
    const input = searchInputs[key];
    if (tabName === tab) {
      bar.style.display = "flex";
    } else {
      bar.style.display = "none";
      if (input.value) { input.value = ""; applySearchFns[key](); }
    }
  });
  updateAddPanelVisibility();
  updateStickySearch();
}

/* ==================== Импорт остатков из Excel в 1С ====================
   Вставляем строки из Excel (Код / Наименование / Остаток — в любом
   порядке, заголовок распознаётся и пропускается). Правила слияния:
     • позиция уже есть в 1С (по коду, а если кода нет — по названию) →
       обновляем ТОЛЬКО остаток; название, код и связь со складом/учётом
       не трогаем;
     • позиции нет в 1С → добавляем новую (модель проставится
       автоматически по метке в названии);
     • склад не изменяется вообще — импорт только в 1С. */
/* ==================== Чтение .xlsx прямо в приложении ====================
   Файл .xlsx — это ZIP с XML внутри. Нужны только два файла:
   xl/sharedStrings.xml (тексты) и xl/worksheets/sheet1.xml (ячейки).
   Распаковываем встроенным DecompressionStream («deflate-raw») — без
   сторонних библиотек, чтобы приложение осталось автономным офлайн. */
async function inflateRaw(bytes) {
  const ds = new DecompressionStream("deflate-raw");
  const stream = new Blob([bytes]).stream().pipeThrough(ds);
  return new Uint8Array(await new Response(stream).arrayBuffer());
}
async function readZipEntries(buf, wanted) {
  const dv = new DataView(buf);
  const bytes = new Uint8Array(buf);
  // ищем End Of Central Directory с конца
  let eocd = -1;
  for (let i = buf.byteLength - 22; i >= 0 && i > buf.byteLength - 66000; i--) {
    if (dv.getUint32(i, true) === 0x06054b50) { eocd = i; break; }
  }
  if (eocd < 0) throw new Error("Это не похоже на файл .xlsx");
  const count = dv.getUint16(eocd + 10, true);
  let p = dv.getUint32(eocd + 16, true);
  const out = {};
  const dec = new TextDecoder("utf-8");
  for (let n = 0; n < count; n++) {
    if (dv.getUint32(p, true) !== 0x02014b50) break;
    const method = dv.getUint16(p + 10, true);
    const compSize = dv.getUint32(p + 20, true);
    const nameLen = dv.getUint16(p + 28, true);
    const extraLen = dv.getUint16(p + 30, true);
    const commLen = dv.getUint16(p + 32, true);
    const lho = dv.getUint32(p + 42, true);
    const name = dec.decode(bytes.subarray(p + 46, p + 46 + nameLen));
    if (wanted.indexOf(name) !== -1) {
      // локальный заголовок: смещение данных зависит от длин полей в нём
      const lNameLen = dv.getUint16(lho + 26, true);
      const lExtraLen = dv.getUint16(lho + 28, true);
      const start = lho + 30 + lNameLen + lExtraLen;
      const raw = bytes.subarray(start, start + compSize);
      out[name] = (method === 0) ? raw : await inflateRaw(raw);
    }
    p += 46 + nameLen + extraLen + commLen;
  }
  return out;
}
function colToIndex(ref) {          // "AB12" -> 27 (1-based номер столбца)
  const m = /^([A-Z]+)/.exec(ref || "");
  if (!m) return 0;
  let n = 0;
  for (const ch of m[1]) n = n * 26 + (ch.charCodeAt(0) - 64);
  return n;
}
/* Чтение таблицы .xlsx в «сетку»: массив строк, где строка — объект
   { номер_столбца: значение }. Общая часть для обоих разборов — выгрузки
   1С и простого файла склада. */
async function readXlsxGrid(file) {
  const buf = await file.arrayBuffer();
  const files = await readZipEntries(buf, ["xl/sharedStrings.xml", "xl/worksheets/sheet1.xml"]);
  const dec = new TextDecoder("utf-8");
  const parser = new DOMParser();
  // общие строки
  let shared = [];
  if (files["xl/sharedStrings.xml"]) {
    const doc = parser.parseFromString(dec.decode(files["xl/sharedStrings.xml"]), "application/xml");
    shared = Array.from(doc.getElementsByTagName("si")).map((si) =>
      Array.from(si.getElementsByTagName("t")).map((t) => t.textContent).join("")
    );
  }
  if (!files["xl/worksheets/sheet1.xml"]) throw new Error("В файле не найден лист с данными.");
  const doc = parser.parseFromString(dec.decode(files["xl/worksheets/sheet1.xml"]), "application/xml");
  const rows = [];
  Array.from(doc.getElementsByTagName("row")).forEach((row) => {
    const cells = {};
    Array.from(row.getElementsByTagName("c")).forEach((cEl) => {
      const idx = colToIndex(cEl.getAttribute("r"));
      const t = cEl.getAttribute("t");
      const vEl = cEl.getElementsByTagName("v")[0];
      let val = "";
      if (t === "s") {
        val = vEl ? (shared[parseInt(vEl.textContent, 10)] || "") : "";
      } else if (t === "inlineStr") {
        const isEl = cEl.getElementsByTagName("is")[0];
        val = isEl ? Array.from(isEl.getElementsByTagName("t")).map((x) => x.textContent).join("") : "";
      } else {
        val = vEl ? vEl.textContent : "";
      }
      cells[idx] = String(val).trim();
    });
    rows.push(cells);
  });
  return rows;
}
async function parseXlsxFile(file) {
  const rows = await readXlsxGrid(file);
  // Столбцы: A(1) — Артикул, D(4) — Номенклатура, H(8) — «В наличии».
  // Именно так устроена выгрузка «Остатки и доступность товаров» из УТ 11.
  // Заголовки ищем сами, чтобы не сломаться, если 1С сдвинет колонки.
  let iCode = 1, iName = 4, iQty = 8, dataStart = 0;
  for (let r = 0; r < Math.min(rows.length, 30); r++) {
    const vals = rows[r];
    let foundCode = 0, foundName = 0, foundQty = 0;
    Object.keys(vals).forEach((k) => {
      const v = (vals[k] || "").toLowerCase();
      if (/артикул|^код$/.test(v)) foundCode = +k;
      else if (/номенклатур|наимен|назван/.test(v)) foundName = +k;
      else if (/в наличии|остат/.test(v)) foundQty = +k;
    });
    if (foundCode && foundName && foundQty) {
      iCode = foundCode; iName = foundName; iQty = foundQty; dataStart = r + 1; break;
    }
  }
  const out = [];
  for (let r = dataStart; r < rows.length; r++) {
    const code = (rows[r][iCode] || "").trim();
    const name = (rows[r][iName] || "").trim();
    const qtyRaw = (rows[r][iQty] || "").replace(/\s|\u00a0/g, "").replace(",", ".");
    const qty = Math.max(0, Math.round(parseFloat(qtyRaw) || 0));
    if (!name) continue;                       // пустые и служебные строки
    if (!code && /^сервисн|^склад|^итог/i.test(name)) continue;  // строка-итог по складу
    out.push({ code, name, qty });
  }
  return out;
}

function parseExcelRows(raw) {
  const lines = raw.split(/\r?\n/).map((l) => l.trim()).filter(Boolean);
  const rows = [];
  lines.forEach((line) => {
    // Excel копирует через табы; поддержим и «;» как запасной разделитель
    const cells = (line.includes("\t") ? line.split("\t") : line.split(";")).map((c) => c.trim());
    if (cells.length < 2) return;
    rows.push(cells);
  });
  if (rows.length === 0) return [];
  // Определяем, какой столбец что означает: ищем заголовок, иначе — по виду
  let header = null;
  const first = rows[0].map((c) => c.toLowerCase());
  if (first.some((c) => /код|наимен|назван|остат|кол-?во|количест/.test(c))) header = first;
  let iCode = 0, iName = 1, iQty = 2;
  if (header) {
    header.forEach((h, i) => {
      if (/код|артикул/.test(h)) iCode = i;
      else if (/наимен|назван|товар/.test(h)) iName = i;
      else if (/остат|кол-?во|количест|шт/.test(h)) iQty = i;
    });
    rows.shift();
  }
  const out = [];
  rows.forEach((cells) => {
    const code = String(cells[iCode] || "").trim();
    const name = String(cells[iName] || "").trim();
    const qtyRaw = String(cells[iQty] || "").replace(/\s|\u00a0/g, "").replace(",", ".");
    const qty = Math.max(0, Math.round(parseFloat(qtyRaw) || 0));
    if (!name && !code) return;
    out.push({ code, name, qty });
  });
  return out;
}
// Поиск позиции 1С отдельно по коду и отдельно по названию — так импорт
// может сначала разобрать все совпадения по коду (приоритет), и только
// потом добирать оставшиеся по названию.
function findOcItemByCode(code) {
  if (!code) return null;
  const want = codeVariants(code);
  for (const section of ["parts", "consumables"]) {
    const list = onec[section];
    for (let i = 0; i < list.length; i++) {
      const it = list[i];
      if (it.code && codeVariants(it.code).some((v) => want.includes(v))) {
        return { section, index: i };
      }
    }
  }
  return null;
}
function findOcItemByName(name) {
  if (!name) return null;
  const target = String(name).toLowerCase().replace(/\s+/g, " ").trim();
  for (const section of ["parts", "consumables"]) {
    const list = onec[section];
    for (let i = 0; i < list.length; i++) {
      if (String(list[i].name || "").toLowerCase().replace(/\s+/g, " ").trim() === target) {
        return { section, index: i };
      }
    }
  }
  return null;
}
function findOcItem(code, name) {
  for (const section of ["parts", "consumables"]) {
    const list = onec[section];
    for (let i = 0; i < list.length; i++) {
      const it = list[i];
      if (code && it.code && codeVariants(it.code).some((v) => codeVariants(code).includes(v))) {
        return { section, index: i };
      }
    }
  }
  if (!name) return null;
  const target = name.toLowerCase();
  for (const section of ["parts", "consumables"]) {
    const list = onec[section];
    for (let i = 0; i < list.length; i++) {
      if ((list[i].name || "").toLowerCase() === target) return { section, index: i };
    }
  }
  return null;
}
function openOcImport() {
  document.getElementById("ocImportArea").value = "";
  document.getElementById("ocImportFile").value = "";
  document.getElementById("ocImportFileBest").value = "";
  ocImportFileRows = null;
  document.getElementById("ocImportFileInfo").textContent = "";
  // Показываем, что уже загружено по второму складу и когда
  const bestInfo = document.getElementById("ocImportFileBestInfo");
  if (bestStock && bestStock.rows) {
    const d = new Date(bestStock.savedAt);
    bestInfo.textContent = `Загружено ${bestStock.rows.length} позиций (${d.toLocaleDateString()} ${d.toLocaleTimeString().slice(0,5)}).`;
  } else {
    bestInfo.textContent = "";
  }
  openModal("ocImportOverlay");
}
function closeOcImport() {
  closeModal("ocImportOverlay");
}
document.getElementById("ocImportBtn").addEventListener("click", openOcImport);
document.getElementById("ocImportClose").addEventListener("click", closeOcImport);
document.getElementById("ocImportOverlay").addEventListener("click", (e) => {
  if (e.target.id === "ocImportOverlay") closeOcImport();
});
// Файл .xlsx, выбранный пользователем (если выбран — он в приоритете
// перед ручной вставкой в текстовое поле).
let ocImportFileRows = null;
document.getElementById("ocImportFile").addEventListener("change", async (e) => {
  const file = e.target.files && e.target.files[0];
  const info = document.getElementById("ocImportFileInfo");
  ocImportFileRows = null;
  if (!file) { info.textContent = ""; return; }
  info.textContent = "Читаю файл…";
  try {
    const rows = await parseXlsxFile(file);
    ocImportFileRows = rows;
    const withQty = rows.filter((r) => r.qty > 0).length;
    info.textContent = `Файл прочитан: ${rows.length} строк (с остатком — ${withQty}). Нажмите «Импортировать».`;
  } catch (err) {
    info.textContent = "Не удалось прочитать файл: " + err.message;
    alert("Не удалось прочитать файл .xlsx: " + err.message +
          "\nМожно вставить строки вручную в поле ниже.");
  }
});

// Второй склад «Бестужевская»: файл только сохраняется отдельно, в список
// 1С ничего не подмешивается. Пригодится для сравнения остатков между
// складами — обрабатываем сразу при выборе файла, кнопка не нужна.
document.getElementById("ocImportFileBest").addEventListener("change", async (e) => {
  const file = e.target.files && e.target.files[0];
  const info = document.getElementById("ocImportFileBestInfo");
  if (!file) { info.textContent = ""; return; }
  info.textContent = "Читаю файл…";
  try {
    const rows = await parseXlsxFile(file);
    bestStock = { savedAt: Date.now(), rows };
    saveBestStock(bestStock);
    const withQty = rows.filter((r) => r.qty > 0).length;
    info.textContent = `Сохранено: ${rows.length} позиций (с остатком — ${withQty}). Список 1С не изменён.`;
  } catch (err) {
    info.textContent = "Не удалось прочитать файл: " + err.message;
    alert("Не удалось прочитать файл .xlsx: " + err.message);
  }
});

/* ==================== Список на перемещение ====================
   Что нужно привезти со второго склада. Пополняется кнопкой «+» из окна
   сравнения; остаток на «Бестужевской» подтягивается заново при открытии,
   чтобы список не устаревал после новой выгрузки. */
/* Добавление позиции в список на перемещение из вкладки «1С»
   (кнопка 🚚 в меню строки). Остаток на «Бестужевской» подтягивается
   автоматически по коду, при отсутствии кода — по названию. */
function addOcItemToTransfer(section, index) {
  const it = onec[section][index];
  if (!it) return;
  if (isInTransfer({ code: it.code, name: it.name })) {
    alert("Эта позиция уже в списке на перемещение.");
    return;
  }
  const idx = buildBestIndex();
  let found = null;
  if (it.code) {
    for (const v of codeVariants(it.code)) { if (idx.byCode[v]) { found = idx.byCode[v]; break; } }
  }
  if (!found) {
    const n = String(it.name || "").toLowerCase().replace(/\s+/g, " ").trim();
    found = idx.byName[n] || null;
  }
  const other = found ? (found.qty || 0) : 0;
  const mine = it.qty || 0;   // остаток по 1С — это и есть моя «Ферма»

  transferList.push({ code: it.code || "", name: it.name, model: it.model || "", mine, other });
  saveTransferList();
  alert(!bestStock
    ? `«${it.name}» добавлена. Данные «Бестужевской» ещё не загружены — остаток появится после загрузки файла.`
    : `«${it.name}» добавлена в список на перемещение. На Бестужевской: ${other}.`);
}
function renderTransferList() {
  const idx = buildBestIndex();
  const summary = document.getElementById("ocTransferSummary");
  const list = document.getElementById("ocTransferList");
  // освежаем цифры по актуальной выгрузке второго склада
  transferList.forEach((t) => {
    let found = null;
    if (t.code) {
      for (const v of codeVariants(t.code)) { if (idx.byCode[v]) { found = idx.byCode[v]; break; } }
    }
    if (!found) {
      const n = String(t.name || "").toLowerCase().replace(/\s+/g, " ").trim();
      found = idx.byName[n] || null;
    }
    t.other = found ? (found.qty || 0) : 0;
    // и свой остаток — из текущего списка 1С
    const mineItem = (t.code ? findOcItemByCode(t.code) : null) || findOcItemByName(t.name);
    if (mineItem) t.mine = onec[mineItem.section][mineItem.index].qty || 0;
  });
  saveTransferList();

  summary.textContent = transferList.length
    ? `Позиций в списке: ${transferList.length}.`
    : "";
  list.innerHTML = transferList.length === 0
    ? '<div class="sync-history-empty">Список пуст. Добавьте позиции кнопкой «+» в окне сравнения остатков.</div>'
    : transferList.map((t, i) => cmpCardHtml(t, i, false, true)).join("");
  Array.from(list.querySelectorAll(".cmp-add")).forEach((btn) => {
    btn.addEventListener("click", () => {
      transferList.splice(parseInt(btn.dataset.idx, 10), 1);
      saveTransferList();
      renderTransferList();
    });
  });
}
function transferAsText() {
  return "Список на перемещение (с Бестужевской)\n" + transferList.map((t) =>
    `${t.code ? t.code + " — " : ""}${t.name}: у меня ${t.mine}, там ${t.other}`
  ).join("\n");
}
document.getElementById("ocTransferBtn").addEventListener("click", () => {
  openModal("ocTransferOverlay");
  renderTransferList();
});
document.getElementById("ocTransferClose").addEventListener("click", () => {
  closeModal("ocTransferOverlay");
});
document.getElementById("ocTransferClear").addEventListener("click", () => {
  if (transferList.length === 0) return;
  if (!confirm("Очистить весь список на перемещение?")) return;
  transferList = [];
  saveTransferList();
  renderTransferList();
});
document.getElementById("ocTransferCopy").addEventListener("click", async () => {
  if (transferList.length === 0) { alert("Список пуст."); return; }
  try {
    await navigator.clipboard.writeText(transferAsText());
    alert("Список скопирован.");
  } catch (e) { alert("Не удалось скопировать."); }
});
document.getElementById("ocTransferPrint").addEventListener("click", () => {
  if (transferList.length === 0) { alert("Список пуст."); return; }
  const rows = transferList.map((t) => `<tr>
      <td>${escapeHtml(t.name)}</td>
      <td>${escapeHtml(t.code || "—")}</td>
      <td style="text-align:center">${t.mine}</td>
      <td style="text-align:center">${t.other}</td>
    </tr>`).join("");
  document.getElementById("printArea").innerHTML = `
    <h1>Список на перемещение</h1>
    <div class="print-date">С «Бестужевской» · позиций: ${transferList.length}</div>
    <table>
      <thead><tr><th>Наименование</th><th style="width:130px;">Код</th><th style="width:70px;">У меня</th><th style="width:90px;">Бестуж.</th></tr></thead>
      <tbody>${rows}</tbody>
    </table>`;
  window.print();
});

/* --- Ручная вставка строк Excel (вынесена в отдельное окно) --- */
document.getElementById("ocPasteBtn").addEventListener("click", () => {
  openModal("ocPasteOverlay");
});
document.getElementById("ocPasteCancel").addEventListener("click", () => {
  closeModal("ocPasteOverlay");
});
document.getElementById("ocPasteApply").addEventListener("click", () => {
  const raw = document.getElementById("ocImportArea").value.trim();
  if (!raw) { alert("Поле пустое."); return; }
  const rows = parseExcelRows(raw);
  if (rows.length === 0) { alert("Не удалось распознать ни одной строки."); return; }
  ocImportFileRows = rows;               // подставляем как источник для «Импортировать»
  document.getElementById("ocImportFileInfo").textContent =
    `Из вставленного текста: ${rows.length} строк. Нажмите «Импортировать».`;
  closeModal("ocPasteOverlay");
});

/* ==================== Сравнение остатков между складами ====================
   Берём свои позиции 1С с остатком меньше порога и ищем их на втором
   складе («Бестужевская», загружается отдельным файлом). Сопоставляем по
   коду, при отсутствии кода — по названию. */
let ocCompareRows = [];
function buildBestIndex() {
  const byCode = {}, byName = {};
  if (!bestStock || !bestStock.rows) return { byCode, byName };
  bestStock.rows.forEach((r) => {
    if (r.code) {
      codeVariants(r.code).forEach((v) => { if (!(v in byCode)) byCode[v] = r; });
    }
    const n = String(r.name || "").toLowerCase().replace(/\s+/g, " ").trim();
    if (n && !(n in byName)) byName[n] = r;
  });
  return { byCode, byName };
}
function runOcCompare() {
  const limit = Math.max(1, parseInt(document.getElementById("ocCompareLimit").value, 10) || 40);
  const mode = document.getElementById("ocCompareMode").value;
  const sectionMode = document.getElementById("ocCompareSection").value;
  const modelMode = document.getElementById("ocCompareModel").value;
  const idx = buildBestIndex();
  const out = [];
  // По умолчанию сравниваем только «Детали»: расходники (химия, спецодежда,
  // инструмент) возить между складами обычно незачем.
  const sectionsToScan = sectionMode === "all" ? ["parts", "consumables"] : [sectionMode];
  sectionsToScan.forEach((section) => {
    onec[section].forEach((it) => {
      const mine = it.qty || 0;
      if (mine >= limit) return;
      // фильтр по модели: «Без версии» — позиции, у которых модель не задана
      if (modelMode !== "all") {
        const m = it.model || "";
        if (modelMode === "none" ? m !== "" : m !== modelMode) return;
      }
      let found = null;
      if (it.code) {
        for (const v of codeVariants(it.code)) { if (idx.byCode[v]) { found = idx.byCode[v]; break; } }
      }
      if (!found) {
        const n = String(it.name || "").toLowerCase().replace(/\s+/g, " ").trim();
        found = idx.byName[n] || null;
      }
      const other = found ? (found.qty || 0) : 0;
      // «Бестужевская > 0» — показываем всё, что там есть в наличии
      // (раньше условие было «больше, чем у меня», и позиции с небольшим,
      // но реальным остатком в список не попадали).
      if (mode === "more" && !(other > 0)) return;
      out.push({ code: it.code || "", name: it.name, model: it.model || "", mine, other, missing: !found });
    });
  });
  out.sort((a, b) => (b.other - b.mine) - (a.other - a.mine));
  ocCompareRows = out;

  const summary = document.getElementById("ocCompareSummary");
  const list = document.getElementById("ocCompareList");
  if (!bestStock || !bestStock.rows) {
    summary.textContent = "Файл склада «Бестужевская» ещё не загружен — загрузите его в окне импорта.";
    list.innerHTML = "";
    return;
  }
  renderOcCompareList();
}
/* Отрисовка списка сравнения с учётом строки поиска. Вынесено отдельно,
   чтобы при вводе в поиск не пересчитывать всё сравнение заново.
   ВАЖНО: в data-idx кладём индекс в ИСХОДНОМ массиве ocCompareRows, а не
   в отфильтрованном — иначе кнопка «+» добавляла бы не ту позицию. */
function renderOcCompareList() {
  const list = document.getElementById("ocCompareList");
  const summary = document.getElementById("ocCompareSummary");
  const limit = document.getElementById("ocCompareLimit").value;
  const q = (document.getElementById("ocCompareSearch").value || "").toLowerCase().trim();
  const shown = ocCompareRows
    .map((r, i) => ({ r, i }))
    .filter(({ r }) => !q ||
      String(r.name || "").toLowerCase().includes(q) ||
      String(r.code || "").toLowerCase().includes(q));

  const secName = { parts: "Детали", consumables: "Расходники", all: "Все разделы" }[
    document.getElementById("ocCompareSection").value] || "";
  const mdl = document.getElementById("ocCompareModel").value;
  const mdlName = mdl === "all" ? "" : " · " + (mdl === "none" ? "без версии" : mdl);
  summary.textContent = q
    ? `Показано: ${shown.length} из ${ocCompareRows.length} (${secName}, порог < ${limit}).`
    : `${secName}${mdlName} · найдено: ${ocCompareRows.length} (порог < ${limit}, данные Бестужевской от ${new Date(bestStock.savedAt).toLocaleDateString()}).`;
  list.innerHTML = shown.length === 0
    ? '<div class="sync-history-empty">Ничего не найдено.</div>'
    : shown.map(({ r, i }) => cmpCardHtml(r, i, isInTransfer(r))).join("");
  // кнопки «+» — добавляют позицию в список на перемещение
  Array.from(list.querySelectorAll(".cmp-add")).forEach((btn) => {
    btn.addEventListener("click", () => {
      const r = ocCompareRows[parseInt(btn.dataset.idx, 10)];
      if (!r || isInTransfer(r)) return;
      transferList.push({ code: r.code, name: r.name, model: r.model || "", mine: r.mine, other: r.other });
      saveTransferList();
      btn.textContent = "✓";
      btn.classList.add("added");
    });
  });
}
/* Карточка позиции в сравнении/перемещении:
     код слева сверху · модель справа сверху
     название по центру
     снизу два блока с остатками (Ферма / Бестужевская) и кнопка действия */
function cmpCardHtml(r, idx, added, removeMode) {
  return `<div class="cmp-card">
    <div class="cmp-top">
      <span class="cmp-code">${r.code ? escapeHtml(r.code) : "—"}</span>
      <span class="cmp-model">${r.model ? escapeHtml(r.model) : ""}</span>
    </div>
    <div class="cmp-name">${escapeHtml(r.name)}</div>
    <div class="cmp-bottom">
      <div class="cmp-cell">
        <div class="cmp-cell-label">Ферма</div>
        <div class="cmp-cell-val">${r.mine}</div>
      </div>
      <div class="cmp-cell">
        <div class="cmp-cell-label">Бестуж.</div>
        <div class="cmp-cell-val">${r.other}${r.missing ? " *" : ""}</div>
      </div>
      ${removeMode
        ? `<button class="cmp-add danger" data-idx="${idx}" title="Убрать из списка">✕</button>`
        : `<button class="cmp-add${added ? " added" : ""}" data-idx="${idx}" title="Добавить в список на перемещение">${added ? "✓" : "+"}</button>`}
    </div>
  </div>`;
}
function isInTransfer(r) {
  return transferList.some((t) =>
    (r.code && t.code === r.code) || (!r.code && !t.code && t.name === r.name)
  );
}
function ocCompareAsText() {
  const limit = document.getElementById("ocCompareLimit").value;
  const head = `Сравнение остатков (порог < ${limit})\nМоё / Бестужевская\n`;
  return head + ocCompareRows.map((r) =>
    `${r.code ? r.code + " — " : ""}${r.name}: ${r.mine} / ${r.other}${r.missing ? " (нет в выгрузке)" : ""}`
  ).join("\n");
}
document.getElementById("ocCompareBtn").addEventListener("click", () => {
  document.getElementById("ocCompareSearch").value = "";
  openModal("ocCompareOverlay");
  runOcCompare();
});
document.getElementById("ocCompareLimit").addEventListener("change", runOcCompare);
document.getElementById("ocCompareMode").addEventListener("change", runOcCompare);
document.getElementById("ocCompareSection").addEventListener("change", runOcCompare);
document.getElementById("ocCompareModel").addEventListener("change", runOcCompare);
// Поиск по уже посчитанному списку — пересчитывать сравнение заново не нужно,
// только перерисовать с фильтром.
document.getElementById("ocCompareSearch").addEventListener("input", renderOcCompareList);
document.getElementById("ocCompareSearchClear").addEventListener("click", () => {
  const input = document.getElementById("ocCompareSearch");
  input.value = "";
  renderOcCompareList();
  input.focus();
});
document.getElementById("ocCompareX").addEventListener("click", () => {
  closeModal("ocCompareOverlay");
});
document.getElementById("ocCompareClose").addEventListener("click", () => {
  closeModal("ocCompareOverlay");
});
document.getElementById("ocCompareCopy").addEventListener("click", async () => {
  const text = ocCompareAsText();
  try {
    await navigator.clipboard.writeText(text);
    alert("Список скопирован.");
  } catch (e) {
    const ta = document.getElementById("ocImportArea");
    ta.value = text; ta.select();
    alert("Скопируйте текст вручную из поля вставки.");
  }
});
document.getElementById("ocComparePrint").addEventListener("click", () => {
  const limit = document.getElementById("ocCompareLimit").value;
  const rows = ocCompareRows.map((r) => `<tr>
      <td>${escapeHtml(r.name)}</td>
      <td>${escapeHtml(r.code || "—")}</td>
      <td style="text-align:center">${r.mine}</td>
      <td style="text-align:center">${r.other}${r.missing ? " *" : ""}</td>
    </tr>`).join("");
  document.getElementById("printArea").innerHTML = `
    <h1>Сравнение остатков</h1>
    <div class="print-date">Порог: меньше ${escapeHtml(String(limit))} · позиций: ${ocCompareRows.length}</div>
    <table>
      <thead><tr><th>Наименование</th><th style="width:130px;">Код</th><th style="width:70px;">У меня</th><th style="width:90px;">Бестуж.</th></tr></thead>
      <tbody>${rows}</tbody>
    </table>`;
  window.print();
});

document.getElementById("ocImportRun").addEventListener("click", () => {
  let rows;
  if (ocImportFileRows && ocImportFileRows.length) {
    rows = ocImportFileRows;
  } else {
    const raw = document.getElementById("ocImportArea").value.trim();
    if (!raw) { alert("Выберите файл .xlsx или вставьте строки из Excel."); return; }
    rows = parseExcelRows(raw);
  }
  if (!rows || rows.length === 0) { alert("Не удалось распознать ни одной строки."); return; }

  // Сопоставление: КОД в приоритете. Позицию, уже занятую совпадением по
  // коду, нельзя переопределить более поздней строкой с тем же названием —
  // иначе одноимённые детали разных моделей (например «Main Cable» под
  // Wind 4.0 и 5.0) перетирали бы остатки друг друга.
  const claimed = new Set();
  const pending = [];
  let updated = 0, added = 0;
  const byName = [];
  rows.forEach((r) => {
    if (!r.code) { byName.push(r); return; }
    const found = findOcItemByCode(r.code);
    if (found) {
      const key = found.section + "|" + found.index;
      if (!claimed.has(key)) {
        claimed.add(key); updated++;
        pending.push({ type: "update", found, qty: r.qty });
        return;
      }
    }
    byName.push(r);
  });
  byName.forEach((r) => {
    const found = findOcItemByName(r.name);
    const key = found ? (found.section + "|" + found.index) : null;
    if (found && !claimed.has(key)) {
      claimed.add(key); updated++;
      pending.push({ type: "update", found, qty: r.qty });
    } else {
      added++; pending.push({ type: "add", row: r });
    }
  });
  // Позиции, которых в свежей выгрузке нет вообще: 1С не выводит их, когда
  // остаток нулевой. Раньше они оставались со старым числом — например
  // «Задний барабан (Wind 5.0)» так и висел с 20 штуками, хотя в 1С уже 0.
  const zeroed = [];
  ["parts", "consumables"].forEach((section) => {
    onec[section].forEach((it, index) => {
      const key = section + "|" + index;
      if (claimed.has(key)) return;          // позиция была в файле — уже обновлена
      if ((it.qty || 0) === 0) return;       // и так ноль, трогать нечего
      zeroed.push({ section, index, name: it.name, was: it.qty });
    });
  });

  // Страховка от неполной выгрузки: если обнулять пришлось бы больше
  // половины позиций с остатком — почти наверняка отчёт выгружен с
  // отбором (по группе, складу или товару), а не по всей номенклатуре.
  // Такой импорт молча обнулил бы почти весь список.
  const withStock = ["parts", "consumables"]
    .reduce((n, s) => n + onec[s].filter((it) => (it.qty || 0) > 0).length, 0);
  const suspicious = withStock > 0 && zeroed.length > withStock / 2;
  if (suspicious) {
    const ok = confirm(
      `⚠️ Похоже, выгрузка неполная.\n\n` +
      `В файле ${rows.length} строк, а обнулить пришлось бы ${zeroed.length} из ${withStock} позиций с остатком.\n\n` +
      `Обычно так бывает, когда отчёт выгружен с отбором — по группе, складу или конкретному товару, а не по всей номенклатуре.\n\n` +
      `Импортировать только остатки из файла, НЕ обнуляя остальные?`
    );
    if (!ok) return;
    zeroed.length = 0;      // обнуление отменяем, обновляем только то, что в файле
  }

  const zeroMsg = zeroed.length
    ? `\n\nЕщё ${zeroed.length} поз. есть в списке, но отсутствуют в файле — их остаток будет обнулён.`
    : "";
  if (!confirm(`Импорт: обновить остаток у ${updated} поз., добавить новых в 1С — ${added}.${zeroMsg}\n\nСклад не изменится. Продолжить?`)) return;

  pushOcHistory();
  pending.forEach((p) => {
    if (p.type === "update") {
      onec[p.found.section][p.found.index].qty = p.qty;   // только остаток
    } else {
      const item = { code: p.row.code, name: p.row.name, qty: p.row.qty };
      const model = detectModelFromName(p.row.name);
      if (model) item.model = model;
      onec.parts.push(item);
    }
  });
  zeroed.forEach((z) => { onec[z.section][z.index].qty = 0; });
  saveOneC();
  renderOneCAll();
  closeOcImport();
  alert(`Готово: обновлено ${updated}, добавлено ${added}` + (zeroed.length ? `, обнулено ${zeroed.length}` : "") + ".");
});

/* ==================== Импорт количеств на склад ====================
   Простой режим: в файле только код и количество. Обновляем количество у
   совпавших по коду позиций, ничего больше не трогаем — ни названия, ни
   разделы, ни модели. Позиции, которых нет в файле, остаются как есть
   (в отличие от импорта в 1С, где отсутствие означает нулевой остаток). */
let whImportRows = null;
/* Простой разбор таблицы для склада: два столбца — код и количество.
   Отдельно от разбора выгрузки 1С, где своя структура с заголовками.
   Строка-заголовок («Код», «Количество») распознаётся и пропускается. */
async function parseSimpleXlsx(file) {
  const rows = await readXlsxGrid(file);
  const out = [];
  rows.forEach((cells) => {
    const keys = Object.keys(cells).map(Number).sort((a, b) => a - b);
    if (keys.length === 0) return;
    const code = String(cells[keys[0]] || "").trim();
    if (!code) return;
    // второй непустой столбец — количество
    let qty = 0;
    for (let k = 1; k < keys.length; k++) {
      const raw = String(cells[keys[k]] || "").replace(/\s|\u00a0/g, "").replace(",", ".");
      if (raw === "") continue;
      const n = parseFloat(raw);
      if (!isNaN(n)) { qty = Math.max(0, Math.round(n)); }
      break;
    }
    // пропускаем заголовок вида «Код | Количество»
    if (/^(код|артикул)$/i.test(code)) return;
    out.push({ code, qty });
  });
  return out;
}
function findWhItemByCode(code) {
  if (!code) return null;
  const want = codeVariants(code);
  for (const section of ["parts", "consumables"]) {
    const list = warehouse[section];
    for (let i = 0; i < list.length; i++) {
      if (list[i].code && codeVariants(list[i].code).some((v) => want.includes(v))) {
        return { section, index: i };
      }
    }
  }
  return null;
}
document.getElementById("whImportBtn").addEventListener("click", () => {
  whImportRows = null;
  document.getElementById("whImportFile").value = "";
  document.getElementById("whImportInfo").textContent = "";
  openModal("whImportOverlay");
});
function closeWhImport() { closeModal("whImportOverlay"); }
document.getElementById("whImportClose").addEventListener("click", closeWhImport);
document.getElementById("whImportX").addEventListener("click", closeWhImport);
document.getElementById("whImportFile").addEventListener("change", async (e) => {
  const file = e.target.files && e.target.files[0];
  const info = document.getElementById("whImportInfo");
  whImportRows = null;
  if (!file) { info.textContent = ""; return; }
  info.textContent = "Читаю файл…";
  try {
    const rows = await parseSimpleXlsx(file);
    whImportRows = rows;
    const known = whImportRows.filter((r) => findWhItemByCode(r.code)).length;
    info.textContent = `Прочитано ${whImportRows.length} строк · найдено на складе: ${known}` +
      (whImportRows.length - known ? ` · нет на складе: ${whImportRows.length - known}` : "");
  } catch (err) {
    info.textContent = "Не удалось прочитать файл: " + err.message;
  }
});
document.getElementById("whImportRun").addEventListener("click", () => {
  if (!whImportRows || whImportRows.length === 0) { alert("Выберите файл .xlsx."); return; }
  let updated = 0, skipped = 0;
  const plan = [];
  whImportRows.forEach((r) => {
    const found = findWhItemByCode(r.code);
    if (!found) { skipped++; return; }
    plan.push({ found, qty: r.qty });
    updated++;
  });
  if (updated === 0) { alert("Ни одного кода из файла нет на складе."); return; }
  const skipMsg = skipped ? `\n\n${skipped} поз. из файла на складе нет — они пропущены.` : "";
  if (!confirm(`Обновить количество у ${updated} поз. на складе?${skipMsg}`)) return;
  pushWhHistory();
  plan.forEach((p) => { warehouse[p.found.section][p.found.index].qty = p.qty; });
  saveWarehouse();
  renderWarehouseAll();
  closeWhImport();
  alert(`Готово: обновлено ${updated}` + (skipped ? `, пропущено ${skipped}` : "") + ".");
});

/* ==================== Резервная копия ====================
   Выгружает все данные приложения в один файл и восстанавливает из него.
   Токен GitHub и пароль настроек НЕ выгружаются — файл можно спокойно
   хранить в почте или облаке. */
const BACKUP_KEYS = [
  "potreblenie_data_by_date_v3",
  "potreblenie_warehouse_v1",
  "potreblenie_onec_v1",
  "potreblenie_transfer_list_v1",
  "potreblenie_best_stock_v1",
  "potreblenie_sort_state_v1",
  "potreblenie_sync_history_v1"
];
function makeBackup() {
  const data = {};
  BACKUP_KEYS.forEach((k) => {
    const v = localStorage.getItem(k);
    if (v !== null) data[k] = v;
  });
  return { app: "sklad", version: APP_VERSION, savedAt: new Date().toISOString(), data };
}
function downloadBackup() {
  const backup = makeBackup();
  const blob = new Blob([JSON.stringify(backup, null, 2)], { type: "application/json" });
  const d = new Date();
  const stamp = d.getFullYear() + "-" + String(d.getMonth() + 1).padStart(2, "0") + "-" + String(d.getDate()).padStart(2, "0");
  const a = document.createElement("a");
  a.href = URL.createObjectURL(blob);
  a.download = `sklad-backup-${stamp}.json`;
  document.body.appendChild(a);
  a.click();
  setTimeout(() => { URL.revokeObjectURL(a.href); a.remove(); }, 1000);
  const size = (JSON.stringify(backup).length / 1024).toFixed(0);
  setSyncStatus("ok", `Копия сохранена (${size} КБ)`);
}
async function restoreBackup(file) {
  let backup;
  try { backup = JSON.parse(await file.text()); }
  catch (e) { alert("Не удалось прочитать файл: это не резервная копия."); return; }
  if (!backup || backup.app !== "sklad" || !backup.data) {
    alert("Это не резервная копия приложения."); return;
  }
  const when = backup.savedAt ? new Date(backup.savedAt).toLocaleString() : "неизвестно когда";
  if (!confirm(`Восстановить данные из копии от ${when}?\n\nВСЕ текущие данные на этом устройстве будут заменены.`)) return;
  Object.keys(backup.data).forEach((k) => {
    if (BACKUP_KEYS.indexOf(k) === -1) return;   // чужие ключи не трогаем
    try { localStorage.setItem(k, backup.data[k]); } catch (e) {}
  });
  alert("Данные восстановлены. Приложение перезагрузится.");
  location.reload();
}
document.getElementById("backupSaveBtn").addEventListener("click", downloadBackup);
document.getElementById("backupLoadBtn").addEventListener("click", () => {
  document.getElementById("backupFileInput").click();
});
document.getElementById("backupFileInput").addEventListener("change", (e) => {
  const file = e.target.files && e.target.files[0];
  if (file) restoreBackup(file);
  e.target.value = "";
});

/* ==================== Копировать / Вставить список склада (перенос между файлами) ==================== */
let whTransferMode = null; // { type: 'copy'|'paste', section: 'parts'|'consumables' }
const whTransferArea = document.getElementById("whTransferArea");
const whTransferHint = document.getElementById("whTransferHint");
const whTransferAction = document.getElementById("whTransferAction");

function openWhCopy() {
  whTransferMode = { type: "copy" };
  document.getElementById("whTransferTitle").textContent = "Копировать весь склад";
  const pack = (arr) => arr.map((it) => ({ code: it.code, name: it.name, qty: it.qty || 0 }));
  whTransferArea.value = JSON.stringify({ parts: pack(warehouse.parts), consumables: pack(warehouse.consumables) });
  whTransferArea.readOnly = true;
  whTransferHint.textContent = "Текст ниже уже выделен — нажмите «Скопировать в буфер» (или Ctrl+C), затем вставьте его кнопкой «Вставить весь склад» в другом файле.";
  whTransferAction.textContent = "Скопировать в буфер";
  openModal("whTransferOverlay");
  setTimeout(() => { whTransferArea.focus(); whTransferArea.select(); }, 60);
}

function openWhPaste() {
  whTransferMode = { type: "paste" };
  document.getElementById("whTransferTitle").textContent = "Вставить весь склад";
  whTransferArea.value = "";
  whTransferArea.readOnly = false;
  whTransferHint.textContent = "Вставьте сюда (Ctrl+V или долгим тапом) текст, скопированный кнопкой «Копировать весь склад».";
  whTransferAction.textContent = "Заменить склад";
  openModal("whTransferOverlay");
  setTimeout(() => whTransferArea.focus(), 60);
}

function closeWhTransfer() {
  whTransferMode = null;
  closeModal("whTransferOverlay");
}
document.getElementById("whTransferClose").addEventListener("click", closeWhTransfer);
document.getElementById("whTransferOverlay").addEventListener("click", (e) => {
  if (e.target.id === "whTransferOverlay") closeWhTransfer();
});


whTransferAction.addEventListener("click", () => {
  if (!whTransferMode) return;
  if (whTransferMode.type === "copy") {
    whTransferArea.focus();
    whTransferArea.select();
    let copied = false;
    try { copied = document.execCommand("copy"); } catch (e) {}
    if (navigator.clipboard && navigator.clipboard.writeText) {
      navigator.clipboard.writeText(whTransferArea.value).catch(() => {});
    }
    const old = whTransferAction.textContent;
    whTransferAction.textContent = copied ? "✓ Скопировано" : "Текст выделен — нажмите Ctrl+C";
    setTimeout(() => { whTransferAction.textContent = old; }, 1500);
    return;
  }
  // paste — вставляется всегда весь склад целиком
  const raw = whTransferArea.value.trim();
  if (!raw) { alert("Вставьте список в поле перед импортом."); return; }
  let parsed;
  try { parsed = JSON.parse(raw); } catch (e) {
    alert("Не удалось распознать список — проверьте, что скопирован именно текст из кнопки «Копировать весь склад».");
    return;
  }
  if (!parsed || !Array.isArray(parsed.parts) || !Array.isArray(parsed.consumables)) {
    alert("Ожидался полный список с разделами «Детали» и «Расходники».");
    return;
  }
  const clean = (items) => items
    .filter((it) => it && typeof it === "object" && it.name)
    .map((it) => ({
      code: String(it.code || "").trim(),
      name: String(it.name).trim(),
      qty: parseInt(it.qty, 10) || 0,
    }));
  const p = clean(parsed.parts), c = clean(parsed.consumables);
  if (p.length + c.length === 0) { alert("В списке не найдено позиций с названием."); return; }
  if (!confirm(`Заменить склад целиком (сейчас ${warehouse.parts.length} + ${warehouse.consumables.length} поз.) на вставленный (${p.length} + ${c.length} поз.)?`)) return;
  pushWhHistory();
  warehouse.parts = p;
  warehouse.consumables = c;
  selectedWH.parts.clear();
  selectedWH.consumables.clear();
  saveWarehouse();
  renderWarehouseAll();
  closeWhTransfer();
});

/* ==================== ВКЛАДКА 1C: справочник номенклатуры ====================
   Только код + наименование. Отдельное хранилище, не связано со складом. */
const ONEC_KEY = "potreblenie_onec_v1";

function loadOneC() {
  try {
    const raw = localStorage.getItem(ONEC_KEY);
    if (!raw) return JSON.parse(JSON.stringify(ONEC_SEED));
    const parsed = JSON.parse(raw);
    if (parsed && Array.isArray(parsed.parts) && Array.isArray(parsed.consumables)) {
      ["parts", "consumables"].forEach((sec) => {
        parsed[sec].forEach((it) => { if (typeof it.qty !== "number") it.qty = 0; });
      });
      return parsed;
    }
    return JSON.parse(JSON.stringify(ONEC_SEED));
  } catch (e) { return JSON.parse(JSON.stringify(ONEC_SEED)); }
}
function saveOneC() {
  try { localStorage.setItem(ONEC_KEY, JSON.stringify(onec)); } catch (e) {}
  scheduleSync();
}
// Локально, без синхронизации — та же логика, что у saveWarehouseLocalOnly.
function saveOneCLocalOnly() {
  try { localStorage.setItem(ONEC_KEY, JSON.stringify(onec)); } catch (e) {}
}
let onec = loadOneC();

/* ==================== Одноразовая простановка моделей ====================
   Ищем в названиях метку версии (wind/ygw 5.0 или 4.0) и проставляем поле
   model. На СКЛАДЕ метку из названия дополнительно вырезаем (она переехала
   в отдельное поле), в 1С — оставляем как есть, только проставляем модель.
   Важно: «wind 25» и подобное НЕ трогаем — это не версия 4.0/5.0, там
   модель остаётся пустой (дефолт «без версии»).
   Выполняется один раз, факт запуска помечаем в localStorage. */
/* ==================== Автоопределение модели по названию ====================
   Метки версии в названиях: «wind/ygw 5.0», «wind/ygw 25» и просто «(25)»
   в скобках → YGW 5.0; «wind/ygw 4.0» → YGW 4.0. Где метки нет — модель
   пустая (без версии). Голое «25» вне скобок НЕ считается меткой, иначе
   ловились бы размеры вроде «Винт M6x25».
   Используется и при автопростановке, и при добавлении новых позиций
   (в т.ч. при импорте остатков из Excel). */
const MODEL_TAG_RE = /[\(\[]\s*25\s*[\)\]]|[\(\[]?\s*(?:wind|ygw)\s*(25|[45][.,]0)\s*[\)\]]?/i;
const BOT_TAG_RE = /\b9\s*[-]?\s*bot\b|ninebot|найнбот/i;
function detectModelFromName(name) {
  // 9Bot определяем отдельно — метка не по шаблону wind/ygw
  if (BOT_TAG_RE.test(name || "")) return "9Bot";
  const m = MODEL_TAG_RE.exec(name || "");
  if (!m) return "";
  if (!m[1]) return "YGW 5.0";           // сработала ветка «(25)» без слова wind/ygw
  const tag = m[1].replace(",", ".");
  if (tag === "25" || tag === "5.0") return "YGW 5.0";
  if (tag === "4.0") return "YGW 4.0";
  return "";
}
function stripModelTag(name) {
  return (name || "").replace(MODEL_TAG_RE, "").replace(/\s{2,}/g, " ").trim().replace(/[\s\-–—]+$/, "");
}
function runModelMigrationOnce() {
  // Проходим при каждом запуске и после получения данных с сервера:
  // иначе синхронизация привозила старые названия с метками обратно и
  // затирала результат разовой миграции.
  // Трогаем только позиции, у которых поле model ещё НИ РАЗУ не задавалось
  // (undefined) — явно выбранное «без версии» (пустая строка) не перетираем.
  let touched = false;
  // Коды вида «AB.xx.xxxx» — это номенклатура 9Bot. Раньше они шли отдельной
  // категорией «временные», теперь просто помечаем моделью. Ставим даже
  // поверх пустого значения, потому что раньше эти позиции сохранялись
  // без модели вовсе.
  ["parts", "consumables"].forEach((section) => {
    [warehouse[section], onec[section]].forEach((list) => {
      list.forEach((it) => {
        if (isAbCode(it.code) && (it.model || "") !== "9Bot") { it.model = "9Bot"; touched = true; }
      });
    });
  });
  ["parts", "consumables"].forEach((section) => {
    warehouse[section].forEach((it) => {
      const model = detectModelFromName(it.name);
      if (model && it.model === undefined) { it.model = model; touched = true; }
      // Метку из названия на СКЛАДЕ убираем всегда — она живёт в поле model
      const cleaned = stripModelTag(it.name);
      if (cleaned && cleaned !== it.name) { it.name = cleaned; touched = true; }
    });
    onec[section].forEach((it) => {
      const model = detectModelFromName(it.name);
      if (model && it.model === undefined) { it.model = model; touched = true; } // название в 1С не чистим
    });
  });
  if (touched) {
    try { localStorage.setItem(WAREHOUSE_KEY, JSON.stringify(warehouse)); } catch (e) {}
    try { localStorage.setItem(ONEC_KEY, JSON.stringify(onec)); } catch (e) {}
  }
  return touched;
}
const migrationTouchedAtStartup = runModelMigrationOnce();

const selectedOC = { parts: new Set(), consumables: new Set() };

/* ==================== Связь Склад <-> 1C по коду ====================
   Один код на складе может быть составным («A / B») — сверяем по каждой части.
   Дубли в 1C не удаляются: один код может встречаться в нескольких строках. */
function codeVariants(code) {
  return String(code || "").split("/").map((c) => c.trim().toLowerCase()).filter(Boolean);
}

/* ==================== Ручные связки кодов Склад <-> 1C ====================
   Один и тот же товар, но артикулы в системах разные. Коды НЕ меняются —
   в каждой вкладке остаётся свой, связь учитывается только при сопоставлении.
   Чтобы добавить новую связку — допишите строку { wh: "код склада", oc: "код 1C" }. */
const CODE_LINKS = [
  { wh: "00-00002861", oc: "796" },   // Защитная пластина номера
];

const linkWhToOc = {};
const linkOcToWh = {};
CODE_LINKS.forEach((l) => {
  const w = String(l.wh).trim().toLowerCase();
  const o = String(l.oc).trim().toLowerCase();
  (linkWhToOc[w] = linkWhToOc[w] || []).push(o);
  (linkOcToWh[o] = linkOcToWh[o] || []).push(w);
});


/* code -> [{section, index, name}] по справочнику 1C */
function buildOneCIndex() {
  const map = {};
  ["parts", "consumables"].forEach((section) => {
    onec[section].forEach((it, index) => {
      codeVariants(it.code).forEach((c) => {
        (map[c] = map[c] || []).push({ section, index, name: it.name });
      });
    });
  });
  return map;
}

/* code -> название позиции на складе */
function buildWarehouseIndex() {
  const map = {};
  ["parts", "consumables"].forEach((section) => {
    warehouse[section].forEach((it, index) => {
      codeVariants(it.code).forEach((c) => {
        if (!map[c]) map[c] = { name: it.name, qty: it.qty || 0, code: it.code, model: it.model || "", section, index };
      });
    });
  });
  return map;
}

let oneCIndex = {};
let warehouseIndex = {};
function rebuildCrossIndexes() {
  oneCIndex = buildOneCIndex();
  warehouseIndex = buildWarehouseIndex();
}

/* Позиции 1C, совпадающие с данным кодом склада */
function oneCMatchesFor(code) {
  const out = [];
  const seen = new Set();
  const lookup = (c) => {
    (oneCIndex[c] || []).forEach((m) => {
      const k = m.section + "#" + m.index;
      if (!seen.has(k)) { seen.add(k); out.push(m); }
    });
  };
  codeVariants(code).forEach((c) => {
    lookup(c);
    (linkWhToOc[c] || []).forEach(lookup);   // ручная связка
  });
  return out;
}

/* Название со склада для данного кода 1C */
/* Модель — свойство самой детали, а не списка, поэтому держим её
   одинаковой на «Складе» и в «1С». Меняешь в одном месте — меняется в
   обоих; сопоставляем по коду. */
function syncModelByCode(code, model) {
  if (!code) return 0;
  const want = codeVariants(code);
  let changed = 0;
  [["warehouse", warehouse], ["onec", onec]].forEach(([, store]) => {
    ["parts", "consumables"].forEach((section) => {
      store[section].forEach((it) => {
        if (!it.code) return;
        if (!codeVariants(it.code).some((v) => want.includes(v))) return;
        if ((it.model || "") === (model || "")) return;
        it.model = model;
        changed++;
      });
    });
  });
  return changed;
}
function warehouseInfoFor(code) {
  for (const c of codeVariants(code)) {
    if (warehouseIndex[c]) return warehouseIndex[c];
    const linked = linkOcToWh[c] || [];
    for (const w of linked) {
      if (warehouseIndex[w]) return warehouseIndex[w];
    }
  }
  return null;
}

/* Первая позиция 1C, связанная с данным кодом склада */
function oneCInfoFor(code) {
  const m = oneCMatchesFor(code);
  if (m.length === 0) return null;
  const item = onec[m[0].section][m[0].index];
  return item ? { name: item.name, code: item.code, qty: item.qty || 0 } : null;
}

/* Вторая строка в ячейке наименования: код другой системы (если отличается) + название */
function altNameHtml(selfCode, info, clickable) {
  if (!info) return "";
  const a = codeVariants(selfCode).join("/");
  const b = codeVariants(info.code).join("/");
  const codePart = (a === b) ? "" : `<span class="alt-code">${escapeHtml(info.code)}</span> `;
  const cls = "alt-name" + (clickable ? " alt-link" : "");
  // Зелёный дубль названия печатаем сокращённо — только первое слово и
  // многоточие, чтобы не раздувать строку. Полное название остаётся в
  // подсказке (title), переход по клику между вкладками не меняется.
  const full = info.name || "";
  const firstWord = full.split(/\s+/)[0] || "";
  const short = (full.trim() === firstWord) ? firstWord : firstWord + "…";
  return `<div class="${cls}" title="${escapeHtml(full)}">${codePart}${escapeHtml(short)}</div>`;
}

/* Прокрутка к строке с учётом закреплённой шапки.
   scrollIntoView({block:"center"}) не годится: на телефоне он центрирует по
   ВИДИМОЙ области (при открытой клавиатуре она низкая), и строка уезжала под
   шапку — особенно после того, как в шапку добавилась панель добавления.
   Здесь строка ставится по центру свободной полосы НИЖЕ шапки. */
function scrollRowIntoView(tr) {
  if (!tr || !tr.getBoundingClientRect) return;
  const head = document.getElementById("stickyTop");
  const top = head ? Math.max(0, head.getBoundingClientRect().bottom) : 0;
  const vv = window.visualViewport;
  const vh = (vv && vv.height) || window.innerHeight;
  const free = Math.max(60, vh - top);            // сколько видно под шапкой
  const r = tr.getBoundingClientRect();
  const pad = Math.max(0, (free - r.height) / 2); // центр этой полосы
  const y = window.pageYOffset + r.top - top - pad;
  window.scrollTo(0, Math.max(0, y));
}

/* Подсветить строки и прокрутить к первой из них */
function highlightRows(rows) {
  document.querySelectorAll(".row-flash").forEach((el) => el.classList.remove("row-flash"));
  let first = null;
  rows.forEach((tr) => { if (tr && !first) first = tr; });
  scrollRowIntoView(first);
  // ВАЖНО: подсветку ставим следующим тактом. Иначе клик, которым вызвана
  // сама операция (например ↶), долетит до общего обработчика документа
  // и тут же снимет её.
  setTimeout(() => {
    rows.forEach((tr) => { if (tr) tr.classList.add("row-flash"); });
  }, 0);
}

/* Кнопка возврата: помнит, из какой позиции склада был переход */
let backTarget = null;   // { tab, section, index, dest }
const backBtn = document.getElementById("backBtn");
const bodyPrefix = { warehouse: "wh-body-", onec: "oc-body-" };

function showBackBtn(tab, section, index, dest) {
  backTarget = { tab, section, index, dest };
  backBtn.classList.add("show");
}
function hideBackBtn() {
  backTarget = null;
  backBtn.classList.remove("show");
}
backBtn.addEventListener("click", (e) => {
  e.stopPropagation();
  if (!backTarget) { hideBackBtn(); return; }
  const { tab, section, index } = backTarget;
  hideBackBtn();
  switchTab(tab);
  setTimeout(() => {
    const tab2 = tab === "warehouse" ? "wh" : "oc";
    highlightRows([revealRow(tab2, section, index)]);
  }, 60);
});

/* Показывает строку, даже если её прячет фильтр вкладки: тогда фильтр
   сбрасывается на «все» и таблица перерисовывается. Иначе переход по ссылке
   выглядел бы как «ничего не произошло» — позиция есть, но она отфильтрована. */
function revealRow(tab, section, index) {
  const tbodyId = (tab === "wh" ? "wh-body-" : "oc-body-") + section;
  let tr = rowByDataIndex(tbodyId, index);
  if (tr) return tr;
  if (tab === "wh") {
    whFilterModel = "all";
    saveWhFilter();
    renderWarehouseAll();
  } else {
    ocFilter.mode = "all";
    saveOcFilter();
    renderOneCAll();
  }
  return rowByDataIndex(tbodyId, index);
}

/* Переход «Склад» -> 1C с подсветкой */
function jumpToOneC(code, fromSection, fromIndex) {
  const matches = oneCMatchesFor(code);
  if (matches.length === 0) return;
  switchTab("onec");
  showBackBtn("warehouse", fromSection, fromIndex, "onec");
  setTimeout(() => {
    highlightRows(matches.map((m) => revealRow("oc", m.section, m.index)));
  }, 60);
}

/* Переход 1C -> «Склад» с подсветкой */
function jumpToWarehouse(code, fromSection, fromIndex) {
  const info = warehouseInfoFor(code);
  if (!info) return;
  switchTab("warehouse");
  showBackBtn("onec", fromSection, fromIndex, "warehouse");
  setTimeout(() => {
    highlightRows([revealRow("wh", info.section, info.index)]);
  }, 60);
}


/* ==================== Фильтр списка 1С ====================
   Список стал длинным (500+ позиций), поэтому вместо одной простыни —
   выбор: всё / детали / расходники / по остатку / временные коды AB.
   Фильтр только для просмотра: данные не меняются, синхронизация не
   запускается. Выбор запоминается на устройстве. */
const OC_FILTER_KEY = "potreblenie_oc_filter_v1";
let ocFilter = loadOcFilter();
function loadOcFilter() {
  try {
    const raw = localStorage.getItem(OC_FILTER_KEY);
    const f = raw ? JSON.parse(raw) : null;
    if (f && f.mode) { if (f.showCons === undefined) f.showCons = true; return f; }
    return { mode: "all", qty: 20, showCons: true };
  } catch (e) { return { mode: "all", qty: 20, showCons: true }; }
}
function saveOcFilter() {
  try { localStorage.setItem(OC_FILTER_KEY, JSON.stringify(ocFilter)); } catch (e) {}
}
function isAbCode(code) { return /^AB[\s.]/i.test(String(code || "")); }
// Возвращает пары { item, i } — исходный индекс обязателен, иначе кнопки
// в строках стали бы указывать не на те позиции.
function ocFilteredPairs(section) {
  // Тумблер «расходники» имеет смысл только в режиме «Остаток»: там в один
  // список попадают и детали, и расходка, а везти между складами обычно
  // нужны только детали. В остальных режимах раздел и так задан выбором.
  if (section === "consumables" && ocFilter.mode === "stock" && !ocFilter.showCons) return [];
  const pairs = onec[section].map((item, i) => ({ item, i }));
  switch (ocFilter.mode) {
    case "consumables":  return section === "consumables" ? pairs : [];
    case "stock":        return pairs.filter(({ item }) => (item.qty || 0) < (ocFilter.qty || 0));
    case "9Bot":
    case "YGW 4.0":
    case "YGW 5.0":      return pairs.filter(({ item }) => (item.model || "") === ocFilter.mode);
    default:             return pairs;
  }
}
function renderOneCSection(section) {
  // См. комментарий в renderWarehouseSection — тот же индекс код→позиция,
  // без него переход «1С» → «Склад» и колонка сравнения указывали бы не
  // на ту строку после сортировки/добавления/редактирования.
  rebuildCrossIndexes();
  const tbody = document.getElementById("oc-body-" + section);
  const picked = selectedOC[section];
  const pairs = ocFilteredPairs(section);
  // собираем разметку одной строкой — заметно быстрее на длинных списках
  tbody.innerHTML = pairs.map(({ item, i }) => {
    const key = section + "::" + item.code + "::" + item.name;
    const whInfo = warehouseInfoFor(item.code);
    const diff = whInfo ? whInfo.qty - (item.qty || 0) : null;
    return `<tr data-key="${escapeHtml(key)}" data-index="${i}"${picked.has(key) ? ' class="row-picked"' : ""}>
      <td class="code wh-code">${item.code ? `<span class="code-text" data-code="${escapeHtml(item.code)}">${formatCodeDisplay(item.code)}</span>` : "—"}${item.model ? `<span class="wh-model-badge">${escapeHtml(item.model)}</span>` : ""}</td>
      <td class="name">
        <div class="main-name">${escapeHtml(item.name)}</div>
        ${altNameHtml(item.code, whInfo, true)}
        ${whInfo ? "" : `<button class="oc-addwh oc-addwh-badge" data-section="${section}" data-index="${i}" title="Добавить эту позицию на склад">+</button>`}
      </td>
      <td class="count-merged wh-merged three-tier">
        <div class="count-tier count-tier-num">
          <input type="number" min="0" value="${item.qty || 0}" class="ocQtyInput"
            data-section="${section}" data-index="${i}">
        </div>
        <div class="count-tier count-tier-oc">${whInfo ? whInfo.qty : "—"}</div>
        <div class="count-tier count-tier-diff"><span class="diff ${diff === null ? "" : (diff > 0 ? "plus" : (diff < 0 ? "minus" : "zero"))}">${diff === null ? "" : (diff > 0 ? "+" + diff : diff)}</span></div>
      </td>
      <td class="buttons oc-buttons">
        <div class="menu-wrap oc-menu-wrap">
          <button class="iconbtn oc-menu-btn" data-section="${section}" data-index="${i}" title="Действия">⋮</button>
          <div class="menu-panel oc-menu-panel">
            <button class="oc-edit" data-section="${section}" data-index="${i}" title="Редактировать">✎</button>
            <button class="oc-transfer" data-section="${section}" data-index="${i}" title="В список на перемещение">🚚</button>
            <button class="oc-del danger" data-section="${section}" data-index="${i}" title="Удалить из 1C">✕</button>
          </div>
        </div>
      </td>
    </tr>`;
  }).join("");
  updateOneCTotals(section);
  if (typeof applyOcSearch === "function") applyOcSearch();
}
function renderOneCAll() {
  rebuildCrossIndexes();
  renderOneCSection("parts");
  renderOneCSection("consumables");
  updateOcFilterUI();
}
/* Прячем заголовок и таблицу раздела, если после фильтра он пуст —
   иначе на экране висели бы пустые «Детали (1C)» / «Расходники (1C)». */
function updateOcFilterUI() {
  const view = document.getElementById("viewOneC");
  if (!view) return;
  hideEmptySections("oc-body-", "ocFilterBtn");
  // подпись рядом с кнопкой фильтра + поле количества только для «Остаток»
  const label = document.getElementById("ocFilterLabel");
  const qtyInput = document.getElementById("ocFilterQty");
  const names = { all: "Всё", consumables: "Расходники", stock: "Остаток",
                  "9Bot": "9Bot", "YGW 4.0": "YGW 4.0", "YGW 5.0": "YGW 5.0" };
  const shown = ocFilteredPairs("parts").length + ocFilteredPairs("consumables").length;
  if (label) label.textContent = `${names[ocFilter.mode] || ""} · ${shown}`;
  if (qtyInput) {
    qtyInput.style.display = ocFilter.mode === "stock" ? "" : "none";
    qtyInput.value = ocFilter.qty;
  }
  const consBtn = document.getElementById("ocConsToggle");
  if (consBtn) {
    consBtn.style.display = ocFilter.mode === "stock" ? "" : "none";
    consBtn.classList.toggle("off", !ocFilter.showCons);
    consBtn.title = ocFilter.showCons ? "Расходники показаны — нажмите, чтобы скрыть"
                                      : "Расходники скрыты — нажмите, чтобы показать";
  }
  // отмечаем выбранный пункт в меню
  document.querySelectorAll("#ocFilterPanel [data-filter]").forEach((b) => {
    b.classList.toggle("primary", b.dataset.filter === ocFilter.mode);
  });
}
/* --- кнопка фильтра --- */
document.getElementById("ocFilterBtn").addEventListener("click", (e) => {
  e.stopPropagation();
  document.getElementById("ocFilterPanel").classList.toggle("open");
});
document.querySelectorAll("#ocFilterPanel [data-filter]").forEach((btn) => {
  btn.addEventListener("click", () => {
    ocFilter.mode = btn.dataset.filter;
    saveOcFilter();
    document.getElementById("ocFilterPanel").classList.remove("open");
    renderOneCAll();          // только перерисовка, без синхронизации
    applyOcSearch();
  });
});
document.getElementById("ocConsToggle").addEventListener("click", () => {
  ocFilter.showCons = !ocFilter.showCons;
  saveOcFilter();
  renderOneCAll();          // только перерисовка, без синхронизации
  applyOcSearch();
});
document.getElementById("ocFilterQty").addEventListener("input", (e) => {
  ocFilter.qty = Math.max(0, parseInt(e.target.value, 10) || 0);
  saveOcFilter();
  renderOneCAll();
  applyOcSearch();
});
document.addEventListener("click", (e) => {
  if (!e.target.closest(".oc-filter-wrap")) {
    const p = document.getElementById("ocFilterPanel");
    if (p) p.classList.remove("open");
  }
});

/* --- клики по строкам 1C --- */
function closeAllOcMenus(except) { closeAllRowMenus("oc-menu-panel", except); }
function handleOneCClick(e) {
  const section = e.currentTarget.id.replace("oc-body-", "");
  const btn = e.target.closest("button");
  if (btn) {
    const index = parseInt(btn.dataset.index, 10);
    if (btn.classList.contains("oc-menu-btn")) {
      e.stopPropagation();
      toggleRowMenu(btn, "oc-menu-panel", closeAllOcMenus);
      return;
    }
    closeAllOcMenus();
    if (btn.classList.contains("oc-del")) {
      openConfirmOneC(section, index);
    } else if (btn.classList.contains("oc-edit")) {
      openOcEdit(section, index);
    } else if (btn.classList.contains("oc-addwh")) {
      openOcToWh(section, index);
    } else if (btn.classList.contains("oc-transfer")) {
      addOcItemToTransfer(section, index);
    }
    return;
  }
  const link = e.target.closest(".alt-link");
  if (link) {
    // См. комментарий в handleWarehouseClick: в «1С» фильтр по разделу/остатку
    // точно так же оставляет на экране лишь часть списка.
    const tr = link.closest("tr");
    const index = parseInt(tr.dataset.index, 10);
    const item = onec[section][index];
    if (!item) return;
    jumpToWarehouse(item.code, section, index);
    return;
  }
  const nameTd = e.target.closest("td.name");
  if (nameTd) {
    if (String(window.getSelection())) return;      // идёт выделение текста
    const tr = nameTd.closest("tr");
    const key = tr.dataset.key;
    if (selectedOC[section].has(key)) {
      selectedOC[section].delete(key);
      tr.classList.remove("row-picked");
    } else {
      selectedOC[section].add(key);
      tr.classList.add("row-picked");
    }
  }
}
document.getElementById("oc-body-parts").addEventListener("click", handleOneCClick);
document.getElementById("oc-body-consumables").addEventListener("click", handleOneCClick);

function handleOneCQtyEdit(e) {
  const input = e.target.closest(".ocQtyInput");
  if (!input) return;
  const section = input.dataset.section;
  const index = parseInt(input.dataset.index, 10);
  let val = parseInt(input.value, 10);
  if (isNaN(val) || val < 0) val = 0;
  if (onec[section][index].qty === val) { renderOneCSection(section); return; }
  pushOcHistory();
  onec[section][index].qty = val;
  saveOneC();
  renderOneCSection(section);
}
document.getElementById("oc-body-parts").addEventListener("change", handleOneCQtyEdit);
document.getElementById("oc-body-consumables").addEventListener("change", handleOneCQtyEdit);
["oc-body-parts","oc-body-consumables"].forEach((id) => {
  document.getElementById(id).addEventListener("keydown", (e) => {
    if (e.key === "Enter" && e.target.classList.contains("ocQtyInput")) e.target.blur();
  });
});

/* --- удаление позиции 1C --- */
function openConfirmOneC(section, index) {
  const item = onec[section][index];
  openConfirmDelete("onec",
    `Удалить «${item.name}» (${item.code || "без кода"}) из справочника 1C?`,
    section, index);
}

/* --- редактирование позиции 1C --- */
let ocEditingTarget = null;
function openOcEdit(section, index) {
  ocEditingTarget = { section, index };
  const item = onec[section][index];
  document.getElementById("ocEditSection").value = section;
  document.getElementById("ocEditCode").value = item.code || "";
  document.getElementById("ocEditName").value = item.name;
  document.getElementById("ocEditQty").value = item.qty || 0;
  setModelSwitch(document.getElementById("ocEditModel"), item.model || "");
  openModal("ocEditOverlay");
}
function closeOcEdit() {
  ocEditingTarget = null;
  closeModal("ocEditOverlay");
}
document.getElementById("ocEditCancel").addEventListener("click", closeOcEdit);
document.getElementById("ocEditOverlay").addEventListener("click", (e) => {
  if (e.target.id === "ocEditOverlay") closeOcEdit();
});
document.getElementById("ocEditSave").addEventListener("click", () => {
  if (!ocEditingTarget) return;
  const { section, index } = ocEditingTarget;
  const newSection = document.getElementById("ocEditSection").value;
  const code = document.getElementById("ocEditCode").value.trim();
  const name = document.getElementById("ocEditName").value.trim();
  let qty = parseInt(document.getElementById("ocEditQty").value, 10);
  if (isNaN(qty) || qty < 0) qty = 0;
  if (!name) { closeOcEdit(); return; }
  const sectionChanged = newSection !== section;
  // проверка дубля кода в целевом разделе
  if (code) {
    const dup = onec[newSection].some((it, i) =>
      it.code === code && !(!sectionChanged && i === index));
    if (dup) { alert("Позиция с таким кодом уже есть в этом разделе 1C."); return; }
  }
  const model = getModelSwitch(document.getElementById("ocEditModel"));
  pushOcHistory();
  const updated = { code, name, qty, model };
  if (sectionChanged) {
    onec[section].splice(index, 1);
    onec[newSection].push(updated);
  } else {
    onec[section][index] = updated;
  }
  syncModelByCode(code, model);   // модель едина для склада и 1С
  saveOneC();
  saveWarehouseLocalOnly();
  renderWarehouseAll();
  if (sectionChanged) {
    renderOneCAll();
    const tbody = document.getElementById("oc-body-" + newSection);
    const rows = tbody.querySelectorAll("tr");
    flashRow(rows[rows.length - 1]);
  } else {
    renderOneCSection(section);
  }
  closeOcEdit();
});

/* --- добавление позиции в 1C --- */
document.getElementById("ocAddBtn").addEventListener("click", () => {
  const section = document.getElementById("ocNewSection").value;
  const code = document.getElementById("ocNewCode").value.trim();
  const name = document.getElementById("ocNewName").value.trim();
  if (!name) return;
  if (code) {
    const dupIndex = onec[section].findIndex((it) => it.code === code);
    if (dupIndex !== -1) {
      alert("Позиция с таким кодом уже есть в этом разделе 1C.");
      // Дубль может быть скрыт фильтром — тогда показываем его, иначе
      // сообщение «позиция уже есть» указывало бы в пустоту.
      flashRow(revealRow("oc", section, dupIndex));
      return;
    }
  }
  pushOcHistory();
  onec[section].push({ code, name, qty: 0 });
  saveOneC();
  document.getElementById("ocNewCode").value = "";
  document.getElementById("ocNewName").value = "";
  renderOneCSection(section);
  const tbody = document.getElementById("oc-body-" + section);
  const rows = tbody.querySelectorAll("tr");
  flashRow(rows[rows.length - 1]);
});



/* ==================== Подсказки из 1C для вкладки «Склад» ====================
   Работают как автоподсказка склада в «Учёте» (см. searchStore/renderSuggestionsFrom
   выше), но источник — справочник 1C. Искать можно и по названию, и по коду. */
function renderOneCSuggestions(box, query, onPick) {
  renderSuggestionsFrom(box, query, onPick, onec,
    "В 1C совпадений нет — можно добавить позицию вручную.");
}

/* --- панель добавления позиции на склад --- */
const whNewCodeInput = document.getElementById("whNewCode");
const whNewNameInput = document.getElementById("whNewName");
const whAddSuggestBox = document.getElementById("whAddSuggestBox");

function pickIntoWhAdd(r) {
  whNewCodeInput.value = r.code;
  whNewNameInput.value = r.name;
  // раздел подставляем как в 1C — при необходимости меняется вручную
  document.getElementById("whNewSection").value = r.section;
}
[whNewCodeInput, whNewNameInput].forEach((input) => {
  input.addEventListener("input", () => {
    renderOneCSuggestions(whAddSuggestBox, input.value, pickIntoWhAdd);
  });
});

registerSuggestZone(whAddSuggestBox, [whNewCodeInput, whNewNameInput]);

/* --- модалка редактирования позиции склада --- */
const whEditCodeInput = document.getElementById("whEditCode");
const whEditNameInput = document.getElementById("whEditName");
const whEditSuggestBox = document.getElementById("whEditSuggestBox");

function pickIntoWhEdit(r) {
  whEditCodeInput.value = r.code;
  whEditNameInput.value = r.name;
}
[whEditCodeInput, whEditNameInput].forEach((input) => {
  input.addEventListener("input", () => {
    renderOneCSuggestions(whEditSuggestBox, input.value, pickIntoWhEdit);
  });
});
registerSuggestZone(whEditSuggestBox, [whEditCodeInput, whEditNameInput]);


/* ==================== Кнопка «наверх» ==================== */
const topBtn = document.getElementById("topBtn");
window.addEventListener("scroll", () => {
  topBtn.classList.toggle("show", window.scrollY > 400);
}, { passive: true });
topBtn.addEventListener("click", (e) => {
  e.stopPropagation();
  window.scrollTo(0, 0);          // мгновенно, без плавной анимации
  updateStickySearch();
});

/* ==================== Экспорт 1C в PDF ==================== */
function buildOneCExport(section, mode) {
  const title = section === "parts" ? "Детали (1C)" : "Расходники (1C)";
  const rows = [];
  onec[section].forEach((item) => {
    const info = warehouseInfoFor(item.code);
    const diff = info ? info.qty - (item.qty || 0) : null;
    if (mode === "diff" && !(diff !== null && diff !== 0)) return;
    if (mode === "plus" && !(diff !== null && diff > 0)) return;
    if (mode === "minus" && !(diff !== null && diff < 0)) return;
    rows.push({ item, info, diff });
  });
  if (rows.length === 0) return "";
  const body = rows.map(({ item, info, diff }) => {
    const d = diff === null ? "—" : (diff > 0 ? "+" + diff : String(diff));
    const color = diff === null ? "#888" : (diff > 0 ? "#16a34a" : (diff < 0 ? "#c62828" : "#888"));
    return `<tr>
      <td>${escapeHtml(item.code || "—")}</td>
      <td>${escapeHtml(item.name)}${info ? `<br><span style="color:#0f766e;font-size:11px">${escapeHtml(info.name)}</span>` : ""}</td>
      <td style="text-align:center">${info ? info.qty : "—"}</td>
      <td style="text-align:center">${item.qty || 0}</td>
      <td style="text-align:center; color:${color}; font-weight:700">${d}</td>
    </tr>`;
  }).join("");
  return `
    <h3>${title}</h3>
    <table>
      <thead><tr>
        <th style="width:110px;">Код</th>
        <th>Наименование</th>
        <th style="width:70px;">На складе</th>
        <th style="width:70px;">В 1C</th>
        <th style="width:70px;">Разница</th>
      </tr></thead>
      <tbody>${body}</tbody>
    </table>`;
}

function runOneCExport(mode) {
  const html = buildOneCExport("parts", mode) + buildOneCExport("consumables", mode);
  const emptyMessages = {
    all: "Справочник 1C пуст.",
    diff: "Расхождений со складом не найдено.",
    plus: "Позиций с излишком (+) не найдено.",
    minus: "Позиций с недостачей (−) не найдено.",
  };
  if (!html) {
    alert(emptyMessages[mode] || "Нет данных для выгрузки.");
    return;
  }
  const titleSuffixes = {
    all: "",
    diff: " — расхождения со складом",
    plus: " — только излишки (+)",
    minus: " — только недостачи (−)",
  };
  const printArea = document.getElementById("printArea");
  printArea.innerHTML = `
    <h1>Справочник 1C${titleSuffixes[mode] || ""}</h1>
    <div class="print-date">Дата: ${escapeHtml(formatFull(todayStr()))}</div>
    ${html}
  `;
  window.print();
}

const ocExportOverlay = document.getElementById("ocExportOverlay");
document.getElementById("ocExportCancel").addEventListener("click", () => closeModal(ocExportOverlay));
ocExportOverlay.addEventListener("click", (e) => { if (e.target.id === "ocExportOverlay") closeModal(ocExportOverlay); });
document.getElementById("ocExportAll").addEventListener("click", () => { closeModal(ocExportOverlay); runOneCExport("all"); });
document.getElementById("ocExportDiff").addEventListener("click", () => { closeModal(ocExportOverlay); runOneCExport("diff"); });
document.getElementById("ocExportPlus").addEventListener("click", () => { closeModal(ocExportOverlay); runOneCExport("plus"); });
document.getElementById("ocExportMinus").addEventListener("click", () => { closeModal(ocExportOverlay); runOneCExport("minus"); });


/* ==================== Навигация по совпадениям поиска ====================
   Работает одинаково для «Учёта», «Склада» и 1C: счётчик «N / всего»
   и стрелки ‹ › для быстрого перехода между найденными строками. */
const searchState = {
  ca: { rows: [], pos: -1 },
  wh: { rows: [], pos: -1 },
  oc: { rows: [], pos: -1 },
};

function updateSearchCounter(k) {
  const st = searchState[k];
  const el = document.getElementById(k + "SearchCount");
  const q = document.getElementById(k + "SearchInput").value.trim();
  if (!q) { el.textContent = ""; return; }
  if (st.rows.length === 0) { el.textContent = "нет"; return; }
  el.textContent = `${st.pos < 0 ? 1 : st.pos + 1} / ${st.rows.length}`;
}

/* Собирает совпадения по разделам таблицы: fn(item, q) -> boolean */
function runSearch(k, bodyPrefix, store, matchFn) {
  const q = document.getElementById(k + "SearchInput").value.trim().toLowerCase();
  const st = searchState[k];
  st.rows = [];
  st.pos = -1;
  ["parts", "consumables"].forEach((section) => {
    const tbody = document.getElementById(bodyPrefix + section);
    if (!tbody) return;
    // У каждой строки есть свой индекс в массиве — берём позицию по нему.
    // Раньше здесь номер строки на экране использовался как индекс в данных,
    // и приходилось отдельно пересчитывать отфильтрованные списки склада и
    // 1С. Теперь это не нужно и работает при любом порядке строк: и с
    // фильтром, и с группировкой по моделям на «Учёте». Заголовки групп
    // пропускаются сами — у них нет data-index.
    Array.prototype.forEach.call(tbody.querySelectorAll("tr[data-index]"), (tr) => {
      const item = store[section][parseInt(tr.dataset.index, 10)];
      if (!item) return;
      const hit = q.length > 0 && matchFn(item, q);
      tr.classList.toggle("row-match", hit);
      tr.classList.remove("row-current");
      if (hit) st.rows.push(tr);
    });
  });
  updateSearchCounter(k);
}

function goToMatch(k, delta) {
  const st = searchState[k];
  if (st.rows.length === 0) return;
  st.pos = (st.pos + delta + st.rows.length) % st.rows.length;
  st.rows.forEach((tr) => tr.classList.remove("row-current"));
  const tr = st.rows[st.pos];
  tr.classList.add("row-current");
  scrollRowIntoView(tr);
  setTimeout(updateStickySearch, 350);
  updateSearchCounter(k);
}

["ca", "wh", "oc"].forEach((k) => {
  document.getElementById(k + "SearchClear").addEventListener("click", () => {
    const input = document.getElementById(k + "SearchInput");
    input.value = "";
    ({ ca: applyCaSearch, wh: applyWhSearch, oc: applyOcSearch })[k]();
    // Фокус в поле НЕ возвращаем: на телефоне это поднимало клавиатуру
    // сразу после того, как нужную строку уже нашли. Крестик только чистит
    // текст. Если фокус на поле всё-таки был — снимаем его.
    if (document.activeElement === input) input.blur();
  });
  document.getElementById(k + "SearchPrev").addEventListener("click", () => goToMatch(k, -1));
  document.getElementById(k + "SearchNext").addEventListener("click", () => goToMatch(k, 1));
  document.getElementById(k + "SearchInput").addEventListener("keydown", (e) => {
    if (e.key === "Enter") { e.preventDefault(); goToMatch(k, e.shiftKey ? -1 : 1); }
  });
});

const nameOrCode = (item, q) =>
  nameMatchesQuery(item.name, q) || (item.code && item.code.toLowerCase().includes(q));


/* ==================== Закрепление общей шапки (дата/вкладки/⋮ + поиск) ====================
   Шапка ВСЕГДА position:fixed (см. .sticky-top в style.css) — без
   переключения между sticky/fixed по прокрутке. Раньше здесь была
   пороговая логика «прилипает после прокрутки мимо точки N» — именно она
   раз за разом ломалась в установленном на «Домой» PWA (то не липла
   вообще, то наезжала на панель добавления). Теперь состояния нет вообще:
   шапка просто всегда наверху, а распорка (stickyTopPlaceholder) всегда
   резервирует под неё место в потоке, чтобы контент под ней никогда не
   перекрывался. Единственное, что делает JS — держит высоту распорки в
   актуальном состоянии (высота шапки чуть меняется между вкладками). */
const stickyTopEl = document.getElementById("stickyTop");
let stickyTopPlaceholder = null;
if (stickyTopEl) {
  stickyTopPlaceholder = document.createElement("div");
  stickyTopPlaceholder.className = "sticky-top-placeholder";
  stickyTopPlaceholder.style.marginBottom = "14px";
  stickyTopEl.parentNode.insertBefore(stickyTopPlaceholder, stickyTopEl);
}
function isTypingNow() {
  const el = document.activeElement;
  return !!el && (el.tagName === "INPUT" || el.tagName === "TEXTAREA" || el.tagName === "SELECT");
}
function syncStickyTopHeight() {
  if (!stickyTopEl || !stickyTopPlaceholder) return;
  if (isTypingNow()) return; // не пересчитываем во время анимации клавиатуры
  stickyTopPlaceholder.style.height = stickyTopEl.offsetHeight + "px";
}
window.addEventListener("resize", syncStickyTopHeight);
// Оставлено под старым именем, чтобы не переписывать остальные места
// файла, которые уже вызывают updateStickySearch() после смены вкладки,
// поиска и т.д. — здесь она просто пересчитывает высоту распорки.
function updateStickySearch() { syncStickyTopHeight(); }
syncStickyTopHeight();

/* iOS: после резкой прокрутки (тап по верхней кромке экрана, рывок пальцем
   от края, инерционный «бросок» списка) WebKit оставляет у position:fixed
   старую область нажатий. Визуально шапка наверху, а тапы попадают туда, где
   она была ДО прокрутки — то есть «мимо, ниже по экрану». Само чинится, только
   если оттянуть страницу вниз и отпустить.

   Лечится принудительным пересчётом раскладки самой шапки: на миг убираем
   position:fixed, синхронно читаем размер (это и заставляет браузер пересчитать
   геометрию и область нажатий) и возвращаем обратно. Всё внутри одной задачи —
   между двумя состояниями кадр не рисуется, поэтому шапка не дёргается. */
function recalibrateStickyTop() {
  if (!stickyTopEl) return;
  const prev = stickyTopEl.style.position;
  stickyTopEl.style.position = "static";
  void stickyTopEl.offsetHeight;          // синхронная раскладка — без неё браузер склеит обе строки
  stickyTopEl.style.position = prev;      // пусто = обратно к fixed из CSS
  void stickyTopEl.offsetHeight;
  syncStickyTopHeight();
}

// Дёргаем не на каждый пиксель прокрутки, а когда она остановилась, и только
// если позиция действительно менялась — лишний пересчёт раскладки на длинном
// списке стоит дорого.
let stickyCalibTimer = null;
let stickyCalibAtY = -1;
function scheduleStickyRecalibrate() {
  // Пока курсор в поле, шапку не трогаем: на iOS снятие position:fixed у
  // предка сфокусированного поля заставляет браузер «догонять» его прокруткой.
  if (isTypingNow()) return;
  if (Math.round(window.pageYOffset) === stickyCalibAtY) return;
  clearTimeout(stickyCalibTimer);
  stickyCalibTimer = setTimeout(() => {
    stickyCalibAtY = Math.round(window.pageYOffset);
    recalibrateStickyTop();
  }, 150);
}
window.addEventListener("scroll", scheduleStickyRecalibrate, { passive: true });
window.addEventListener("touchend", scheduleStickyRecalibrate, { passive: true });
window.addEventListener("pageshow", () => { stickyCalibAtY = -1; scheduleStickyRecalibrate(); });
document.addEventListener("visibilitychange", () => {
  if (!document.hidden) { stickyCalibAtY = -1; scheduleStickyRecalibrate(); }
});
if (window.visualViewport) {
  // Клавиатура и смена масштаба тоже сдвигают видимую область.
  window.visualViewport.addEventListener("resize", () => { stickyCalibAtY = -1; scheduleStickyRecalibrate(); });
}


/* ==================== Отмена / повтор на «Складе» ==================== */
const WH_HISTORY_LIMIT = 50;

function whSnapshot() { return JSON.stringify(warehouse); }
function restoreWhSnapshot(json) {
  const before = warehouse;
  warehouse = JSON.parse(json);
  selectedWH.parts.clear();
  selectedWH.consumables.clear();
  saveWarehouse();
  renderWarehouseAll();
  const hit = findChange(before, warehouse);
  if (hit) highlightAt("wh-body-", hit.section, hit.index);
}
const whHistory = makeUndoRedo({
  snapshotFn: whSnapshot,
  restoreFn: restoreWhSnapshot,
  undoBtnId: "whUndoBtn",
  redoBtnId: "whRedoBtn",
  limit: WH_HISTORY_LIMIT
});
function pushWhHistory() { whHistory.push(); }
document.getElementById("whResetBtn").addEventListener("click", () => {
  const total = warehouse.parts.length + warehouse.consumables.length;
  openConfirmDelete("whReset",
    `Обнулить количество у всех позиций склада (${total} шт.)? Сами позиции останутся, можно отменить кнопкой ↶.`);
});


/* ==================== Чистое копирование и вставка ====================
   Наименование выводится двумя блочными строками, поэтому браузер
   добавляет в копию перенос строки — в поле ввода он превращается
   в лишний пробел. Здесь копия и вставка нормализуются. */
function normalizeText(str) {
  return String(str).replace(/[\r\n\t]+/g, " ").replace(/\s{2,}/g, " ").trim();
}

document.addEventListener("copy", (e) => {
  const src = e.target;
  // из полей ввода копируем как есть (там может быть JSON переноса списков)
  if (src && (src.tagName === "INPUT" || src.tagName === "TEXTAREA")) return;
  const sel = String(window.getSelection() || "");
  if (!sel) return;
  const clean = normalizeText(sel);
  if (clean === sel) return;
  if (e.clipboardData) {
    e.clipboardData.setData("text/plain", clean);
    e.preventDefault();
  }
});

document.addEventListener("paste", (e) => {
  const el = e.target;
  if (!el || el.tagName !== "INPUT") return;          // textarea не трогаем
  if (el.type !== "text" && el.type !== "search") return;
  const data = e.clipboardData && e.clipboardData.getData("text/plain");
  if (!data) return;
  const clean = normalizeText(data);
  if (clean === data) return;
  e.preventDefault();
  const start = el.selectionStart, end = el.selectionEnd;
  el.value = el.value.slice(0, start) + clean + el.value.slice(end);
  const pos = start + clean.length;
  el.setSelectionRange(pos, pos);
  el.dispatchEvent(new Event("input", { bubbles: true }));
});


/* ==================== Экспорт «Склада» в PDF ==================== */
function buildWarehouseExport(section) {
  const title = section === "parts" ? "Детали (склад)" : "Расходники (склад)";
  const list = warehouse[section];
  if (list.length === 0) return "";
  const body = list.map((item) => {
    const oc = oneCInfoFor(item.code);
    return `<tr>
      <td>${escapeHtml(item.code || "—")}</td>
      <td>${escapeHtml(item.name)}${oc ? `<br><span style="color:#0f766e;font-size:11px">${escapeHtml(oc.name)}</span>` : ""}</td>
      <td style="text-align:center">${item.qty || 0}</td>
      <td style="text-align:center">${oc ? oc.qty : "—"}</td>
    </tr>`;
  }).join("");
  return `
    <h3>${title}</h3>
    <table>
      <thead><tr>
        <th style="width:120px;">Код</th>
        <th>Наименование</th>
        <th style="width:70px;">На складе</th>
        <th style="width:70px;">1C остаток</th>
      </tr></thead>
      <tbody>${body}</tbody>
    </table>`;
}

function runWarehouseExport() {
  const html = buildWarehouseExport("parts") + buildWarehouseExport("consumables");
  if (!html) { alert("Склад пуст."); return; }
  const printArea = document.getElementById("printArea");
  printArea.innerHTML = `
    <h1>Склад</h1>
    <div class="print-date">Дата: ${escapeHtml(formatFull(todayStr()))}</div>
    ${html}
  `;
  window.print();
}

/* Кнопка в шапке работает в контексте активной вкладки */
document.getElementById("exportBtn").addEventListener("click", () => {
  if (document.getElementById("tabWarehouseBtn").classList.contains("active")) {
    runWarehouseExport();
  } else if (document.getElementById("tabOneCBtn").classList.contains("active")) {
    openModal(ocExportOverlay);
  } else {
    runConsumptionExport();
  }
});


/* ==================== Отмена / повтор во вкладке 1C ==================== */
function ocSnapshot() { return JSON.stringify(onec); }
function restoreOcSnapshot(json) {
  const before = onec;
  onec = JSON.parse(json);
  selectedOC.parts.clear();
  selectedOC.consumables.clear();
  saveOneC();
  renderOneCAll();
  renderWarehouseAll();   // на складе показывается «1C остаток»
  const hit = findChange(before, onec);
  if (hit) highlightAt("oc-body-", hit.section, hit.index);
}
const ocHistory = makeUndoRedo({
  snapshotFn: ocSnapshot,
  restoreFn: restoreOcSnapshot,
  undoBtnId: "ocUndoBtn",
  redoBtnId: "ocRedoBtn",
  limit: WH_HISTORY_LIMIT
});
function pushOcHistory() { ocHistory.push(); }


/* ==================== Ноль в числовых полях ====================
   При установке курсора в поле со значением 0 оно очищается,
   чтобы ввод не превращался в «0129». Если ушли, ничего не введя,
   ноль возвращается. Программная правка value не вызывает change,
   поэтому лишних сохранений не происходит. */
function isNumberField(el) {
  return el && el.tagName === "INPUT" && el.type === "number";
}
document.addEventListener("focusin", (e) => {
  const el = e.target;
  if (isNumberField(el) && el.value === "0") el.value = "";
});
document.addEventListener("focusout", (e) => {
  const el = e.target;
  if (isNumberField(el) && el.value.trim() === "") el.value = "0";
});


/* ==================== Перенос позиции 1C на склад ====================
   Кнопка «+» есть только у позиций без связи со складом. Код берётся из 1C —
   именно по нему связь и возникает; название на складе можно задать своё. */
let ocToWhTarget = null;

function openOcToWh(section, index) {
  const item = onec[section][index];
  if (!item) return;
  if (warehouseInfoFor(item.code)) {          // связь уже есть — нечего добавлять
    alert("Эта позиция уже есть на складе.");
    return;
  }
  ocToWhTarget = { section, index };
  document.getElementById("ocToWhSection").value = section === "parts" ? "Детали" : "Расходники";
  document.getElementById("ocToWhCode").value = item.code || "";
  document.getElementById("ocToWhName").value = item.name;
  document.getElementById("ocToWhQty").value = 0;
  // Модель берём из позиции 1С; если там пусто — пробуем определить по
  // названию (метка wind/ygw в тексте), чтобы не выбирать руками.
  setModelSwitch(document.getElementById("ocToWhModel"),
    item.model || detectModelFromName(item.name) || "");
  openModal("ocToWhOverlay");
  setTimeout(() => {
    const n = document.getElementById("ocToWhName");
    n.focus(); n.select();
  }, 50);
}
function closeOcToWh() {
  ocToWhTarget = null;
  closeModal("ocToWhOverlay");
}
document.getElementById("ocToWhCancel").addEventListener("click", closeOcToWh);
document.getElementById("ocToWhOverlay").addEventListener("click", (e) => {
  if (e.target.id === "ocToWhOverlay") closeOcToWh();
});
document.getElementById("ocToWhSave").addEventListener("click", () => {
  if (!ocToWhTarget) return;
  const { section, index } = ocToWhTarget;
  const item = onec[section][index];
  if (!item) { closeOcToWh(); return; }
  const name = document.getElementById("ocToWhName").value.trim();
  let qty = parseInt(document.getElementById("ocToWhQty").value, 10);
  if (isNaN(qty) || qty < 0) qty = 0;
  if (!name) { alert("Укажите наименование для склада."); return; }
  const code = item.code || "";
  if (code && warehouseInfoFor(code)) { alert("Позиция с таким кодом уже есть на складе."); closeOcToWh(); return; }

  const model = getModelSwitch(document.getElementById("ocToWhModel"));
  pushWhHistory();
  warehouse[section].push({ code, name, qty, model });
  // ту же модель проставляем и позиции в 1С, чтобы не расходились
  if (model) {
    item.model = model;
    syncModelByCode(code, model);
    saveOneCLocalOnly();
  }
  saveWarehouse();
  renderWarehouseAll();
  renderOneCAll();
  closeOcToWh();

  // подсветим строку 1C — у неё появилась зелёная связь со складом
  flashRow(revealRow("oc", section, index));
});
document.getElementById("ocToWhName").addEventListener("keydown", (e) => {
  if (e.key === "Enter") document.getElementById("ocToWhSave").click();
});


/* ==================== Итоги по колонкам ====================
   Под заголовком показывается сумма по разделу. Позиции без связи
   в сумму «чужой» колонки не попадают (там прочерк, а не ноль). */
function fmtTotal(n) {
  return String(n).replace(/\B(?=(\d{3})+(?!\d))/g, " ");
}
function setTotal(id, value) {
  const el = document.getElementById(id);
  if (el) el.textContent = fmtTotal(value);
}

function updateWarehouseTotals(section) {
  const list = warehouse[section];
  let own = 0, oc = 0;
  list.forEach((it) => {
    own += it.qty || 0;
    const info = oneCInfoFor(it.code);
    if (info) oc += info.qty;
  });
  setTotal(`total-wh-${section}-qty`, own);
  setTotal(`total-wh-${section}-ocqty`, oc);
}

function updateOneCTotals(section) {
  const list = onec[section];
  let own = 0, wh = 0;
  list.forEach((it) => {
    own += it.qty || 0;
    const info = warehouseInfoFor(it.code);
    if (info) wh += info.qty;
  });
  setTotal(`total-oc-${section}-qty`, own);
  setTotal(`total-oc-${section}-whqty`, wh);
}

/* ==================== Синхронизация через GitHub Gist ====================
   Данные (dataByDate, warehouse, onec, currentDate) хранятся дополнительно
   в приватном Gist пользователя. При изменениях — отправляем туда (с
   задержкой, чтобы не слать запрос на каждое нажатие). При открытии —
   подтягиваем актуальную версию, чтобы на любом устройстве было видно
   одно и то же. Правило при конфликте — побеждает последнее сохранение
   (кто последний нажал/подождал синхронизации, тот и главный). */
const SYNC_TOKEN_KEY = "potreblenie_sync_token_v1";
const SYNC_GIST_ID_KEY = "potreblenie_sync_gist_id_v1";
// Роль устройства: "главное" всегда побеждает при конфликте и не спрашивает
// подтверждения; "подчинённое" при конфликте всегда молча берёт версию с
// сервера и вдобавок сам периодически проверяет сервер, пока открыто.
const SYNC_PRIMARY_KEY = "potreblenie_sync_primary_v1";
/* ==================== Заморозка синхронизации ====================
   Синхронизация с сервером приостановлена: приложение работает полностью
   локально, и НИЧЕГО не может перетереть то, что вы ввели.

   Зачем: загрузка с сервера (pullFromGist) применяла присланные данные
   безусловно, без сверки времени изменения. GitHub Gist умеет ответить на
   чтение сразу после записи ещё СТАРЫМ содержимым (задержка репликации), и
   тогда только что добавленная позиция затиралась версией, где её ещё нет.
   В истории сохранений это выглядело как «+ позиция» (отправлено), а следом
   «− позиция» (загружено с сервера) — вещь пропадала «будто и не добавлял».

   Пока флаг включён, автоматических отправок и загрузок нет вообще: ни по
   таймеру, ни при возврате в приложение, ни при появлении сети. Локальные
   данные как писались в localStorage сразу при каждом изменении, так и
   пишутся — на это заморозка не влияет.

   Снимается переключателем в настройках синхронизации (долгое нажатие на
   облачко). По умолчанию — заморожено. */
const SYNC_FROZEN_KEY = "potreblenie_sync_frozen_v1";
// true — приложение САМО ничего не отправляет и не загружает (режим по умолчанию).
// Обмен идёт только по нажатию кнопки. Ключ остался прежним, чтобы у уже
// настроенных устройств ничего не переключилось при обновлении.
function isAutoSyncOff() { return localStorage.getItem(SYNC_FROZEN_KEY) !== "0"; }
function setAutoSyncOff(off) {
  try { localStorage.setItem(SYNC_FROZEN_KEY, off ? "1" : "0"); } catch (e) {}
}
// Автоматика разрешена: и настроено, и режим не ручной. Этим гейтом закрыты
// ВСЕ автоматические пути — таймер, возврат в приложение, появление сети,
// опрос сервера, старт приложения.
function isAutoSyncActive() { return isSyncConfigured() && !isAutoSyncOff(); }
function isPrimaryDevice() { return localStorage.getItem(SYNC_PRIMARY_KEY) === "1"; }
const SYNC_SETTINGS_PW_KEY = "potreblenie_sync_settings_pw_v1";
function getSyncPassword() { return localStorage.getItem(SYNC_SETTINGS_PW_KEY) || ""; }
function setSyncPassword(pw) { localStorage.setItem(SYNC_SETTINGS_PW_KEY, pw); }
function changeSyncPassword() {
  const current = getSyncPassword();
  if (current) {
    const entered = prompt("Текущий пароль:");
    if (entered === null) return;
    if (entered !== current) { alert("Неверный пароль."); return; }
  }
  const next = prompt(current ? "Новый пароль:" : "Задайте пароль для настроек синхронизации:");
  if (next === null) return;
  if (!next.trim()) { alert("Пароль не может быть пустым."); return; }
  setSyncPassword(next.trim());
  alert("Пароль сохранён.");
}
function verifySyncPassword() {
  const stored = getSyncPassword();
  if (!stored) return true; // пароль ещё не задан — доступ без проверки
  const entered = prompt("Пароль настроек синхронизации:");
  if (entered === null) return false;
  if (entered !== stored) { alert("Неверный пароль."); return false; }
  return true;
}
function openSyncModalGated() {
  if (!verifySyncPassword()) return;
  openSyncModal();
}
/* ==================== Ручной обмен с сервером ====================
   Приложение само на сервер не ходит. Обмен — только по кнопке:
     • главное устройство выгружает готовый список;
     • второе (режим просмотра) загружает последний выгруженный.
   Выгрузка со второго устройства намеренно недоступна — так оно не сможет
   затереть на сервере список, собранный на главном. */
function manualModeStatusText() {
  if (!isSyncConfigured()) {
    return "Не настроено. Долгое нажатие на облачко — настройки синхронизации.";
  }
  return isPrimaryDevice()
    ? "Главное устройство. Тап по облачку — выгрузить список на сервер."
    : "Режим просмотра. Тап по облачку — загрузить список с сервера.";
}
function formatSyncMoment(ts) {
  if (!ts) return "—";
  const d = new Date(ts);
  const pad = (n) => String(n).padStart(2, "0");
  return pad(d.getDate()) + "." + pad(d.getMonth() + 1) + " " + pad(d.getHours()) + ":" + pad(d.getMinutes());
}
function openManualSync() {
  const primary = isPrimaryDevice();
  document.getElementById("manualSyncRole").textContent =
    primary ? "Это устройство: главное" : "Это устройство: второе (только просмотр)";
  // Сбрасываем состояние прошлого обмена (текст ошибки, блокировку кнопок).
  const info = document.getElementById("manualSyncInfo");
  info.classList.remove("err");
  info.textContent =
    "Последний обмен с сервером: " + formatSyncMoment(getLastSyncedAt())
    + (primary && isDirty() ? " · есть невыгруженные правки" : "");
  manualSyncBusy(false);
  // Со второго устройства выгружать нельзя — прячем кнопку целиком.
  document.getElementById("manualPushBtn").hidden = !primary;
  document.getElementById("manualSyncHint").textContent = primary
    ? "Выгрузка заменяет список на сервере текущим. Загрузка заменит данные на этом устройстве версией с сервера."
    : "Загрузка заменит данные на этом устройстве последним списком, выгруженным с главного устройства.";
  openModal("manualSyncOverlay");
}
function quickSyncAction() {
  if (!isSyncConfigured()) { openSyncModalGated(); return; }
  openManualSync();
}

/* Результат обмена показываем ПРЯМО В ОКНЕ. Раньше окно закрывалось сразу, и
   при ошибке оставался только восклицательный знак на облачке — причина была
   не видна вообще. */
function manualSyncBusy(on) {
  ["manualPushBtn", "manualPullBtn", "manualSyncClose"].forEach((id) => {
    const b = document.getElementById(id);
    if (b) b.disabled = on;
  });
}
function showManualResult(text, isErr) {
  const el = document.getElementById("manualSyncInfo");
  if (!el) return;
  el.textContent = text;
  el.classList.toggle("err", !!isErr);
}
// К тексту ошибки добавляем ID Gist — по нему сразу видно, смотрят ли оба
// устройства в один и тот же список.
function manualErrorText(fallback) {
  const { gistId } = getSyncConfig();
  return (lastSyncErrorText || fallback) + (gistId ? "\nGist: " + gistId : "\nID Gist не указан");
}

async function manualPush() {
  manualSyncBusy(true);
  showManualResult("Выгружаем список на сервер…", false);
  setSyncStatus("syncing", "Выгружаем список на сервер…");
  const ok = await pushToGistNow(true);
  manualSyncBusy(false);
  if (ok) {
    showManualResult("Готово — список выгружен на сервер.", false);
    setTimeout(() => closeModal("manualSyncOverlay"), 1400);
  } else {
    showManualResult(manualErrorText("Не удалось выгрузить"), true);
  }
}
async function manualPull() {
  // Загрузка заменяет данные устройства. Если здесь есть свои правки, ещё не
  // ушедшие на сервер, — предупреждаем, иначе они пропадут молча.
  if (isDirty() && !confirm("На этом устройстве есть правки, которых нет на сервере.\n\n"
      + "Загрузка заменит их версией с сервера. Продолжить?")) return;
  manualSyncBusy(true);
  showManualResult("Загружаем список с сервера…", false);
  setSyncStatus("syncing", "Загружаем список с сервера…");
  const ok = await pullFromGist(false, false, true);
  manualSyncBusy(false);
  if (ok) {
    showManualResult("Готово — список загружен с сервера.", false);
    setTimeout(() => closeModal("manualSyncOverlay"), 1400);
    return;
  }
  showManualResult(manualErrorText("Не удалось загрузить"), true);
  // Отказ мог быть защитой от устаревшего ответа сервера — предлагаем настоять.
  const { token, gistId } = getSyncConfig();
  if (!token || !gistId) return;
  let remote = null;
  try {  // ещё раз читаем сервер — понять, был ли это отказ защиты
    const data = await githubGistRequest("GET", "https://api.github.com/gists/" + gistId, token);
    const file = data.files && data.files[SYNC_FILENAME];
    remote = (file && file.content) ? JSON.parse(file.content) : null;
  } catch (e) { return; }
  const verdict = shouldApplyRemote(remote);
  if (verdict.apply || verdict.same) return;
  if (confirm("Версия на сервере " + verdict.reason
      + ".\n\nВсё равно загрузить её и ЗАМЕНИТЬ данные на этом устройстве?")) {
    manualSyncBusy(true);
    const forced = await pullFromGist(false, true, true);
    manualSyncBusy(false);
    if (forced) {
      showManualResult("Готово — список загружен с сервера.", false);
      setTimeout(() => closeModal("manualSyncOverlay"), 1400);
    } else {
      showManualResult(manualErrorText("Не удалось загрузить"), true);
    }
  }
}
document.getElementById("manualPushBtn").addEventListener("click", manualPush);
document.getElementById("manualPullBtn").addEventListener("click", manualPull);
document.getElementById("manualSyncClose").addEventListener("click", () => closeModal("manualSyncOverlay"));
document.getElementById("manualSyncOverlay").addEventListener("click", (e) => {
  if (e.target.id === "manualSyncOverlay") closeModal("manualSyncOverlay");
});
// Гостевой режим: синхронизация настроена, но это устройство не главное —
// на вкладке «Учёт» скрываем +/- и добавление, оставляем только список с поиском.
function isGuestMode() { return isSyncConfigured() && !isPrimaryDevice(); }
function updateGuestModeUI() {
  updateAddPanelVisibility();
  renderAll();
}
const SECONDARY_POLL_MS = 30000; // как часто подчинённое устройство само проверяет сервер
let secondaryPollTimer = null;
const SYNC_FILENAME = "sklad-sync-data.json";
// История сохранений — отдельный файл в том же Gist (Gist поддерживает
// несколько файлов). Так она переживает переустановку приложения и очистку
// браузера, и видна на всех устройствах одинаково.
const SYNC_HISTORY_FILENAME = "sklad-sync-history.json";
// Ждём 60 секунд ПОСЛЕ ПОСЛЕДНЕГО действия, прежде чем отправлять данные на
// сервер (debounce, не троттлинг): если действия идут одно за другим быстрее,
// чем раз в минуту, отправка откладывается до паузы. Не зависит от того,
// остаётесь вы в приложении или закрываете его — при уходе (см. flushSyncNow)
// несохранённые правки всё равно отправляются немедленно.
const SYNC_DEBOUNCE_MS = 20000;
// "Отметки времени", по которым мы понимаем, кто последний менял данные —
// это устройство или сервер (см. подробное объяснение в конце файла / в чате).
const LOCAL_UPDATED_AT_KEY = "potreblenie_local_updated_v1";
const SYNC_LAST_SYNCED_AT_KEY = "potreblenie_last_synced_v1";

let syncTimer = null;
let syncInFlight = false;
let syncQueued = false; // изменение пришло, пока предыдущая отправка ещё была в процессе — досылаем её сразу после
let applyingRemote = false;

/* ---- Отслеживание "есть ли несохранённые на сервере изменения" ---- */
function getLocalUpdatedAt() { return Number(localStorage.getItem(LOCAL_UPDATED_AT_KEY) || 0); }
function getLastSyncedAt() { return Number(localStorage.getItem(SYNC_LAST_SYNCED_AT_KEY) || 0); }

// Вызывается при КАЖДОМ локальном изменении данных (см. scheduleSync ниже).
function markLocalChange() {
  try { localStorage.setItem(LOCAL_UPDATED_AT_KEY, String(Date.now())); } catch (e) {}
  updateDirtyIndicator();
}
// Вызывается после того, как мы точно знаем, что локальные данные и данные
// на сервере совпадают (успешный push или успешный pull). Если передан
// dataSnapshot (сами синхронизированные данные), заодно сравниваем их с
// состоянием на момент предыдущей синхронизации и пишем запись в историю.
function markSyncedTo(updatedAt, dataSnapshot, kind) {
  const t = updatedAt || Date.now();
  try {
    localStorage.setItem(SYNC_LAST_SYNCED_AT_KEY, String(t));
    localStorage.setItem(LOCAL_UPDATED_AT_KEY, String(t));
  } catch (e) {}
  if (dataSnapshot) {
    const firstSync = !loadSyncLastState();
    const lines = describeSyncAttempt(dataSnapshot);
    // Диф (не первая синхронизация) пишем в историю только если реально
    // что-то изменилось — иначе на пустом холостом сохранении появлялась бы
    // запись без единой строки.
    if (firstSync || lines.length > 0) addSyncHistoryEntry(kind || "push", lines, "ok");
    saveSyncLastState(dataSnapshot);
  }
  updateDirtyIndicator();
}
// Есть ли на этом устройстве правки, которые ещё не долетели до сервера?
function isDirty() {
  return isSyncConfigured() && getLocalUpdatedAt() > getLastSyncedAt();
}
function updateDirtyIndicator() {
  const btn = document.getElementById("syncBtn");
  if (!btn) return;
  btn.classList.toggle("dirty", isDirty());
}

function getSyncConfig() {
  return {
    token: localStorage.getItem(SYNC_TOKEN_KEY) || "",
    gistId: localStorage.getItem(SYNC_GIST_ID_KEY) || ""
  };
}
function isSyncConfigured() {
  return !!getSyncConfig().token;
}

// Текст последней ошибки обмена. Нужен, чтобы окно обмена показало ПРИЧИНУ,
// а не только восклицательный знак на облачке.
let lastSyncErrorText = "";
function setSyncStatus(state, text) {
  if (state === "err") lastSyncErrorText = text || "";
  else if (state === "ok") lastSyncErrorText = "";
  const btn = document.getElementById("syncBtn");
  const line = document.getElementById("syncStatusLine");
  btn.classList.remove("err", "syncing");
  const icons = { idle: "⚙", syncing: "⏳", ok: "☁", err: "⚠", off: "⚙", frozen: "⏸" };
  if (state === "syncing") btn.classList.add("syncing");
  if (state === "err") btn.classList.add("err");
  btn.textContent = icons[state] || "⚙";
  if (line) {
    line.textContent = text || "";
    line.classList.toggle("err", state === "err");
  }
}

function buildSyncPayload() {
  return {
    updatedAt: Date.now(),
    dataByDate,
    warehouse,
    onec,
    currentDate
  };
}

/* ==================== История сохранений ====================
   Отдельный локальный журнал (не уходит в Gist): при каждой успешной
   отправке на сервер или загрузке с сервера сравниваем новые данные с
   теми, что были на момент предыдущей синхронизации, и запоминаем время
   + краткое описание того, что изменилось (какие позиции добавлены,
   удалены, у каких изменилось количество). Хранится только на этом
   устройстве и нужна, чтобы можно было понять, что и когда сохранилось. */
const SYNC_LAST_STATE_KEY = "potreblenie_sync_laststate_v1";
const SYNC_HISTORY_KEY = "potreblenie_sync_history_v1";
const SYNC_HISTORY_LIMIT = 40;

function loadSyncLastState() {
  try {
    const raw = localStorage.getItem(SYNC_LAST_STATE_KEY);
    return raw ? JSON.parse(raw) : null;
  } catch (e) { return null; }
}
function saveSyncLastState(data) {
  try {
    localStorage.setItem(SYNC_LAST_STATE_KEY, JSON.stringify({
      dataByDate: data.dataByDate, warehouse: data.warehouse, onec: data.onec
    }));
  } catch (e) {}
}
function loadSyncHistory() {
  try {
    const raw = localStorage.getItem(SYNC_HISTORY_KEY);
    const arr = raw ? JSON.parse(raw) : [];
    return Array.isArray(arr) ? arr : [];
  } catch (e) { return []; }
}
function saveSyncHistoryList() {
  try { localStorage.setItem(SYNC_HISTORY_KEY, JSON.stringify(syncHistory)); } catch (e) {}
}
let syncHistory = loadSyncHistory();

function addSyncHistoryEntry(kind, lines, status) {
  syncHistory.unshift({ time: Date.now(), kind, lines, status: status || "ok" });
  if (syncHistory.length > SYNC_HISTORY_LIMIT) syncHistory.length = SYNC_HISTORY_LIMIT;
  saveSyncHistoryList();
  renderSyncHistory();
}
// Строки-описание того, что изменилось — сравнивает dataSnapshot с
// последним ИЗВЕСТНО-СОХРАНЁННЫМ состоянием. Общая часть для успешной
// записи в историю (markSyncedTo) и для неудачной попытки (logSyncFailure) —
// в обоих случаях интересно "что именно не долетело/долетело".
function describeSyncAttempt(dataSnapshot) {
  const prev = loadSyncLastState();
  if (!prev) return ["Первая синхронизация — сохранено текущее состояние"];
  return buildChangeSummary(prev, dataSnapshot);
}
// Неудачная попытка отправки/загрузки — раньше просто показывала ошибку на
// секунду в статус-строке и всё, при следующем открытии приложения не было
// и следа, что попытка вообще была ("будто ничего и не было"). Теперь
// каждая неудача — отдельная запись в истории с красной точкой; сами
// данные при этом остаются "грязными" (isDirty() не сбрасывается), так что
// следующая же попытка (новая правка, возврат в приложение, восстановление
// сети — см. обработчики ниже) отправит их снова.
function logSyncFailure(kind, dataSnapshot, errorMessage) {
  const lines = dataSnapshot ? describeSyncAttempt(dataSnapshot) : [];
  lines.push("Ошибка: " + errorMessage);
  addSyncHistoryEntry(kind, lines, "failed");
}
// Объединяет историю, полученную с сервера (от другого устройства), с тем,
// что уже есть локально. Записи считаются одинаковыми по паре время+тип —
// этого достаточно, коллизии между разными устройствами практически
// невозможны (миллисекундная точность).
function mergeRemoteHistory(historyFileContent) {
  if (!historyFileContent) return;
  let remoteList;
  try { remoteList = JSON.parse(historyFileContent); } catch (e) { return; }
  if (!Array.isArray(remoteList) || remoteList.length === 0) return;
  const seen = new Set(syncHistory.map((e) => e.time + "|" + e.kind));
  let added = false;
  remoteList.forEach((entry) => {
    const sig = entry.time + "|" + entry.kind;
    if (!seen.has(sig)) { syncHistory.push(entry); seen.add(sig); added = true; }
  });
  if (added) {
    syncHistory.sort((a, b) => b.time - a.time);
    if (syncHistory.length > SYNC_HISTORY_LIMIT) syncHistory.length = SYNC_HISTORY_LIMIT;
    saveSyncHistoryList();
    renderSyncHistory();
  }
}

// Ключ позиции для сравнения "было/стало" — так же, как countKey: по коду,
// если он есть, иначе по названию.
function itemDiffKey(item) {
  return item.code ? ("code::" + item.code) : ("name::" + item.name);
}
function labelItem(item) {
  return item.code ? (item.name + " (" + item.code + ")") : item.name;
}
// Сравнивает два списка позиций (например warehouse.parts до/после) и
// возвращает добавленные / удалённые / те, у кого изменилось количество.
function diffItemList(oldList, newList, qtyOf) {
  const oldMap = new Map((oldList || []).map((it) => [itemDiffKey(it), it]));
  const newMap = new Map((newList || []).map((it) => [itemDiffKey(it), it]));
  const added = [], removed = [], changed = [];
  newMap.forEach((item, key) => { if (!oldMap.has(key)) added.push(item); });
  oldMap.forEach((item, key) => { if (!newMap.has(key)) removed.push(item); });
  if (qtyOf) {
    newMap.forEach((item, key) => {
      const oldItem = oldMap.get(key);
      if (!oldItem) return;
      const a = qtyOf(oldItem), b = qtyOf(item);
      if (a !== b) changed.push({ item, from: a, to: b });
    });
  }
  return { added, removed, changed };
}
// Диф для «Склада» и «1С» — там количество лежит прямо в самой позиции (qty).
function diffStore(oldStore, newStore) {
  const lines = [];
  ["parts", "consumables"].forEach((sec) => {
    const secLabel = sec === "parts" ? "Детали" : "Расходники";
    const oldList = oldStore ? oldStore[sec] : [];
    const newList = newStore ? newStore[sec] : [];
    const { added, removed, changed } = diffItemList(oldList, newList, (it) => it.qty);
    added.forEach((it) => lines.push("+ " + secLabel + ": " + labelItem(it) + " (" + it.qty + ")"));
    removed.forEach((it) => lines.push("− " + secLabel + ": " + labelItem(it)));
    changed.forEach((c) => lines.push("~ " + secLabel + ": " + labelItem(c.item) + "  " + c.from + " → " + c.to));
  });
  return lines;
}
// Диф для одного дня «Учёта» — там количество лежит отдельно, в counts,
// под ключом countKey(section, item).
function diffDateEntry(oldEntry, newEntry) {
  const lines = [];
  const oldCounts = (oldEntry && oldEntry.counts) || {};
  const newCounts = (newEntry && newEntry.counts) || {};
  ["parts", "consumables"].forEach((sec) => {
    const secLabel = sec === "parts" ? "Детали" : "Расходники";
    const oldList = (oldEntry && oldEntry.sections && oldEntry.sections[sec]) || [];
    const newList = (newEntry && newEntry.sections && newEntry.sections[sec]) || [];
    const { added, removed } = diffItemList(oldList, newList, null);
    added.forEach((it) => lines.push("+ " + secLabel + ": " + labelItem(it) + " (" + (newCounts[countKey(sec, it)] || 0) + ")"));
    removed.forEach((it) => lines.push("− " + secLabel + ": " + labelItem(it)));
    const oldMap = new Map(oldList.map((it) => [itemDiffKey(it), it]));
    newList.forEach((it) => {
      const key = itemDiffKey(it);
      const oldItem = oldMap.get(key);
      if (!oldItem) return;
      const a = oldCounts[countKey(sec, oldItem)] || 0;
      const b = newCounts[countKey(sec, it)] || 0;
      if (a !== b) lines.push("~ " + secLabel + ": " + labelItem(it) + "  " + a + " → " + b);
    });
  });
  return lines;
}
// Диф по всем датам «Учёта» — показываем не более 5 дат в одной записи истории.
function diffDataByDate(oldData, newData) {
  const lines = [];
  const oldD = oldData || {};
  const newD = newData || {};
  const dates = Array.from(new Set([...Object.keys(oldD), ...Object.keys(newD)])).sort();
  let shown = 0, skipped = 0;
  dates.forEach((date) => {
    const dLines = diffDateEntry(oldD[date], newD[date]);
    if (dLines.length === 0) return;
    if (shown >= 5) { skipped++; return; }
    lines.push("Учёт за " + formatShort(date) + ":");
    dLines.forEach((l) => lines.push("  " + l));
    shown++;
  });
  if (skipped > 0) lines.push("…и ещё изменения за " + skipped + " дат(у/ы)");
  return lines;
}
function buildChangeSummary(oldSnap, newSnap) {
  const lines = [];
  const caLines = diffDataByDate(oldSnap && oldSnap.dataByDate, newSnap.dataByDate);
  lines.push(...caLines);
  const whLines = diffStore(oldSnap && oldSnap.warehouse, newSnap.warehouse);
  if (whLines.length) { lines.push("Склад:"); whLines.forEach((l) => lines.push("  " + l)); }
  const ocLines = diffStore(oldSnap && oldSnap.onec, newSnap.onec);
  if (ocLines.length) { lines.push("1С:"); ocLines.forEach((l) => lines.push("  " + l)); }
  return lines;
}
function formatHistoryTime(ts) {
  const d = new Date(ts);
  const pad = (n) => String(n).padStart(2, "0");
  return pad(d.getDate()) + "." + pad(d.getMonth() + 1) + " " + pad(d.getHours()) + ":" + pad(d.getMinutes());
}
function renderSyncHistory() {
  const box = document.getElementById("syncHistoryList");
  if (!box) return;
  if (syncHistory.length === 0) {
    box.innerHTML = "<div class=\"sync-history-empty\">Пока нет записей — появятся после первого сохранения.</div>";
    return;
  }
  box.innerHTML = syncHistory.map((entry) => {
    const isLocal = entry.kind === "local";
    const failed = entry.status === "failed";
    const skipped = entry.status === "skipped";
    const kindLabel = isLocal
      ? "✎ сохранено на устройстве"
      : failed
        ? (entry.kind === "pull" ? "⚠ не удалось загрузить" : "⚠ не удалось сохранить")
        : skipped
          ? "⊘ пропущено (данные защищены)"
          : (entry.kind === "pull" ? "⇩ загружено с сервера" : "⇧ сохранено на сервер");
    const body = entry.lines && entry.lines.length
      ? entry.lines.map((l) => escapeHtml(l)).join("\n")
      : "(первая запись — состояние на момент подключения синхронизации)";
    // Маленькая точка в углу записи: зелёная — сохранилось, красная — не
    // долетело (данные при этом никуда не делись, просто ждут следующей
    // попытки — см. logSyncFailure).
    const dotTitle = isLocal ? "Сохранено на этом устройстве"
                   : failed ? "Не удалось отправить"
                   : skipped ? "Устаревшие данные с сервера не применены"
                   : "Сохранено";
    const dotClass = isLocal ? "local" : failed ? "err" : skipped ? "warn" : "ok";
    return "<div class=\"sync-history-entry\">"
      + "<div class=\"sync-history-head\"><b>" + formatHistoryTime(entry.time) + "</b><span>" + kindLabel + "</span></div>"
      + "<pre class=\"sync-history-body\">" + body + "</pre>"
      + `<span class="sync-history-dot ${dotClass}" title="${dotTitle}"></span>`
      + "</div>";
  }).join("");
}

/* ==================== Сверка версий перед применением данных с сервера ====
   ПРИЧИНА ПРОПАЖ. Раньше присланные сервером данные применялись безусловно.
   GitHub Gist может ответить на чтение сразу после записи ещё СТАРЫМ
   содержимым (задержка репликации между репликами API). Тогда только что
   добавленная позиция затиралась версией, где её ещё нет:
       ⇧ сохранено на сервер   + Рамка номера задняя
       ⇩ загружено с сервера   − Рамка номера задняя
   Опорная точка — updatedAt последней версии, к которой мы точно
   синхронизировались (её мы либо сами отправили, либо приняли с сервера).
   Если сервер отдаёт updatedAt СТАРШЕ этой отметки — это заведомо
   устаревший ответ, и применять его нельзя. */
function shouldApplyRemote(remotePayload) {
  if (!remotePayload || typeof remotePayload !== "object") {
    return { apply: false, same: false, reason: "сервер не прислал данные приложения" };
  }
  const remoteAt = Number(remotePayload.updatedAt) || 0;
  const lastSynced = getLastSyncedAt();
  if (!lastSynced) return { apply: true, same: false, reason: "" };   // ещё ни разу не синхронизировались
  if (!remoteAt) {
    return { apply: false, same: false, reason: "у данных с сервера нет отметки времени" };
  }
  if (remoteAt === lastSynced) {
    return { apply: false, same: true, reason: "на сервере та же версия, что уже есть здесь" };
  }
  if (remoteAt < lastSynced) {
    return { apply: false, same: false,
             reason: "сервер вернул версию старше той, что уже сохранена здесь — похоже на устаревший ответ" };
  }
  return { apply: true, same: false, reason: "" };
}

/* ==================== Журнал локальных сохранений ====================
   В историю попадают не только обмены с сервером, но и обычная работа на
   устройстве: добавили позицию, изменили количество, поправили склад/1С.
   Записи ГРУППИРУЮТСЯ: всё, что сделано в пределах минуты, показывается
   одной записью и пересчитывается целиком от состояния на начало группы —
   поэтому три тапа «+» подряд дают «20 → 23», а не три отдельные строки.

   Как считается разница. Держим два снимка:
     • базовый (baseline) — состояние на НАЧАЛО текущей группы, лежит в
       localStorage, чтобы группа пережила перезагрузку приложения;
     • последний (localLogLastState) — состояние на момент прошлой записи,
       живёт в памяти. Когда минута истекает и открывается новая группа,
       именно он становится её базовым: на конец прошлой группы состояние
       было ровно таким.
   Сортировка сюда не попадает — она сохраняется мимо scheduleSync(). */
const LOCAL_LOG_GROUP_MS = 60000;      // окно группировки — одна минута
const LOCAL_LOG_DEBOUNCE_MS = 1200;    // подряд идущие тапы сводим в одну запись
const LOCAL_LOG_BASELINE_KEY = "potreblenie_local_log_baseline_v1";
let localLogTimer = null;
let localLogLastState = null;

function snapshotForLog() {
  return JSON.parse(JSON.stringify({ dataByDate, warehouse, onec }));
}
function loadLocalLogBaseline() {
  try {
    const raw = localStorage.getItem(LOCAL_LOG_BASELINE_KEY);
    return raw ? JSON.parse(raw) : null;
  } catch (e) { return null; }
}
function saveLocalLogBaseline(snap) {
  try { localStorage.setItem(LOCAL_LOG_BASELINE_KEY, JSON.stringify(snap)); } catch (e) {}
}

function logLocalChange() {
  if (localLogTimer) clearTimeout(localLogTimer);
  localLogTimer = setTimeout(flushLocalLog, LOCAL_LOG_DEBOUNCE_MS);
}

function flushLocalLog() {
  localLogTimer = null;
  const current = snapshotForLog();
  // Первый вызов до готовности приложения — просто запоминаем точку отсчёта.
  if (!localLogLastState) { localLogLastState = current; return; }

  const now = Date.now();
  const top = syncHistory[0];
  const inGroup = top && top.kind === "local"
                  && (now - (top.groupStart || top.time)) < LOCAL_LOG_GROUP_MS;

  let baseline;
  if (inGroup) {
    baseline = loadLocalLogBaseline() || localLogLastState;
  } else {
    baseline = localLogLastState;      // состояние на конец предыдущей группы
    saveLocalLogBaseline(baseline);
  }

  const lines = buildChangeSummary(baseline, current);
  localLogLastState = current;
  if (lines.length === 0) return;      // менялось что-то, чего не показываем

  if (inGroup) {
    top.lines = lines;                 // пересчитали группу целиком
    top.time = now;
  } else {
    syncHistory.unshift({ time: now, groupStart: now, kind: "local", lines, status: "local" });
    if (syncHistory.length > SYNC_HISTORY_LIMIT) syncHistory.length = SYNC_HISTORY_LIMIT;
  }
  saveSyncHistoryList();
  renderSyncHistory();
}

function applySyncPayload(payload) {
  applyingRemote = true;
  try {
    if (payload.dataByDate && typeof payload.dataByDate === "object") {
      dataByDate = payload.dataByDate;
      try { localStorage.setItem(DATA_BY_DATE_KEY, JSON.stringify(dataByDate)); } catch (e) {}
    }
    if (payload.warehouse && Array.isArray(payload.warehouse.parts) && Array.isArray(payload.warehouse.consumables)) {
      warehouse = payload.warehouse;
      try { localStorage.setItem(WAREHOUSE_KEY, JSON.stringify(warehouse)); } catch (e) {}
    }
    if (payload.onec && Array.isArray(payload.onec.parts) && Array.isArray(payload.onec.consumables)) {
      onec = payload.onec;
      try { localStorage.setItem(ONEC_KEY, JSON.stringify(onec)); } catch (e) {}
    }
    if (payload.currentDate) {
      currentDate = payload.currentDate;
      try { localStorage.setItem(LAST_DATE_KEY, currentDate); } catch (e) {}
    }
    const entry = entryFor(currentDate);
    sections = entry.sections;
    counts = entry.counts;
    checked = entry.checked;
    // С сервера могли прийти позиции со старыми названиями (метка версии
    // ещё в тексте) — сразу прогоняем автопростановку модели, иначе
    // результат локальной миграции затирался каждым получением данных.
    runModelMigrationOnce();
    // Сервер присылает данные в своём порядке — возвращаем сохранённый
    // локально порядок сортировки, иначе он сбрасывался после каждой
    // синхронизации и выглядело это как «сортировка не срабатывает».
    reapplySavedSort();
    selectedCA.parts.clear();
    selectedCA.consumables.clear();
    selectedWH.parts.clear();
    selectedWH.consumables.clear();
    selectedOC.parts.clear();
    selectedOC.consumables.clear();
    caHistory.reset();
    whHistory.reset();
    ocHistory.reset();
    rebuildCrossIndexes();
    dateInput.value = currentDate;
    renderAll();
    renderWarehouseAll();
    renderOneCAll();
  } finally {
    applyingRemote = false;
  }
}

// Раньше при сетевой ошибке (телефон на секунду потерял связь) синхронизация
// сразу сдавалась и показывала «Не удалось сохранить/загрузить». Теперь один
// раз тихо повторяем запрос после паузы — но только саму сетевую ошибку
// (fetch бросает TypeError, если вообще не достучался до сервера); реальный
// ответ сервера с кодом ошибки (401/404/…) не ретраим — повтор всё равно не
// поможет.
async function githubGistRequest(method, url, token, body, _retried) {
  let res;
  try {
    res = await fetch(url, {
      method,
      headers: {
        "Authorization": "token " + token,
        "Accept": "application/vnd.github+json",
        "Content-Type": "application/json"
      },
      body: body ? JSON.stringify(body) : undefined
    });
  } catch (networkErr) {
    if (!_retried) {
      await new Promise((resolve) => setTimeout(resolve, 1500));
      return githubGistRequest(method, url, token, body, true);
    }
    throw new Error("Нет соединения с сервером");
  }
  if (!res.ok) {
    let msg = "Ошибка " + res.status;
    if (res.status === 401) msg = "Неверный токен";
    if (res.status === 404) msg = "Gist не найден (проверьте ID)";
    throw new Error(msg);
  }
  return res.json();
}

async function pullFromGist(silent, force, manual) {
  // Автоматически — только когда автоматика разрешена. Вручную (manual) —
  // всегда: это осознанное нажатие кнопки пользователем.
  if (!manual && !isAutoSyncActive()) return false;
  const { token, gistId } = getSyncConfig();
  if (!token || !gistId) {
    // Раньше здесь был молчаливый отказ: на втором устройстве с незаполненным
    // ID Gist кнопка просто «ничего не делала».
    if (manual) setSyncStatus("err", !token ? "Не указан GitHub-токен"
                                            : "Не указан ID Gist — впишите тот же ID, что на главном устройстве");
    return false;
  }
  if (!silent) setSyncStatus("syncing", "Загрузка с сервера…");
  try {
    const data = await githubGistRequest("GET", "https://api.github.com/gists/" + gistId, token);
    mergeRemoteHistory(data.files && data.files[SYNC_HISTORY_FILENAME] && data.files[SYNC_HISTORY_FILENAME].content);
    const file = data.files && data.files[SYNC_FILENAME];
    if (!file || !file.content) {
      setSyncStatus("err", "В Gist пока нет данных приложения");
      return false;
    }
    const payload = JSON.parse(file.content);
    const verdict = shouldApplyRemote(payload);
    if (!verdict.apply && !force) {
      if (verdict.same) {
        setSyncStatus("ok", "Уже актуально: " + new Date().toLocaleTimeString().slice(0, 5));
      } else {
        // Локальные данные НЕ трогаем. Раньше именно здесь они и терялись.
        setSyncStatus("ok", "Данные с сервера пропущены — " + verdict.reason);
        addSyncHistoryEntry("pull", ["Загрузка пропущена: " + verdict.reason,
                                     "Локальные данные оставлены без изменений"], "skipped");
      }
      return false;
    }
    applySyncPayload(payload);
    markSyncedTo(payload.updatedAt, payload, "pull");
    setSyncStatus("ok", "Синхронизировано: " + new Date().toLocaleTimeString().slice(0, 5));
    return true;
  } catch (e) {
    setSyncStatus("err", "Не удалось загрузить: " + e.message);
    logSyncFailure("pull", null, e.message);
    return false;
  }
}

// Отправляет данные на сервер. ПЕРЕД отправкой проверяет: не сохранил ли
// кто-то (например, вы же с другого устройства) более новую версию данных
// с момента, когда мы в последний раз читали сервер. Если сохранил — не
// затираем её молча, а показываем пользователю модалку с выбором.
async function pushToGistNow(manual) {
  if (!manual && !isAutoSyncActive()) return false;
  const { token, gistId } = getSyncConfig();
  if (!token) { setSyncStatus("err", "Не указан GitHub-токен"); return false; }
  if (syncInFlight) {
    // Отправка уже идёт (например, только что стартовала по таймеру), а тут
    // подоспело ещё одно изменение. Раньше это изменение просто терялось —
    // теперь запоминаем и досылаем сразу же, как только текущая отправка
    // завершится (см. finally ниже).
    syncQueued = true;
    return;
  }
  syncInFlight = true;
  setSyncStatus("syncing", "Сохранение на сервер…");
  const payload = buildSyncPayload();
  try {
    if (!gistId) {
      // Гиста ещё нет — создаём первый раз, конфликтовать не с чем.
      const created = await githubGistRequest("POST", "https://api.github.com/gists", token, {
        description: "Учёт деталей — Склад (данные приложения)",
        public: false,
        files: {
          [SYNC_FILENAME]: { content: JSON.stringify(payload) },
          [SYNC_HISTORY_FILENAME]: { content: JSON.stringify(syncHistory) }
        }
      });
      localStorage.setItem(SYNC_GIST_ID_KEY, created.id);
      const gistInput = document.getElementById("syncGistIdInput");
      if (gistInput) gistInput.value = created.id;
      markSyncedTo(payload.updatedAt, payload, "push");
      setSyncStatus("ok", "Синхронизировано: " + new Date().toLocaleTimeString().slice(0, 5));
      return true;
    }
    // Сверяем версию на сервере с той, что мы видели в последний раз.
    const remote = await githubGistRequest("GET", "https://api.github.com/gists/" + gistId, token);
    mergeRemoteHistory(remote.files && remote.files[SYNC_HISTORY_FILENAME] && remote.files[SYNC_HISTORY_FILENAME].content);
    const remoteFile = remote.files && remote.files[SYNC_FILENAME];
    const remotePayload = (remoteFile && remoteFile.content) ? JSON.parse(remoteFile.content) : null;
    const lastSynced = getLastSyncedAt();
    const remoteChangedSinceUs = remotePayload && remotePayload.updatedAt && remotePayload.updatedAt !== lastSynced;
    // Проверка перед отправкой тоже читает сервер — и точно так же может
    // получить устаревший ответ. Без сверки подчинённое устройство ниже
    // приняло бы эту старую копию и затёрло свежие локальные данные.
    const verdict = shouldApplyRemote(remotePayload);
    if (remoteChangedSinceUs && !verdict.apply) {
      addSyncHistoryEntry("pull", ["Ответ сервера пропущен: " + verdict.reason,
                                   "Отправляем локальную версию"], "skipped");
    }
    // «Сервер действительно новее» — только если сверка это подтвердила.
    if (remoteChangedSinceUs && verdict.apply) {
      if (isPrimaryDevice()) {
        // Главное устройство: не спрашиваем — наша версия побеждает всегда.
        await githubGistRequest("PATCH", "https://api.github.com/gists/" + gistId, token, {
          files: {
            [SYNC_FILENAME]: { content: JSON.stringify(payload) },
            [SYNC_HISTORY_FILENAME]: { content: JSON.stringify(syncHistory) }
          }
        });
        markSyncedTo(payload.updatedAt, payload, "push");
        setSyncStatus("ok", "Синхронизировано (главное устройство): " + new Date().toLocaleTimeString().slice(0, 5));
        return true;
      } else {
        // Подчинённое устройство: не спрашиваем — молча берём версию с сервера.
        applyingRemote = true;
        try {
          applySyncPayload(remotePayload);
        } finally {
          applyingRemote = false;
        }
        markSyncedTo(remotePayload.updatedAt, remotePayload, "pull");
        setSyncStatus("ok", "Обновлено с сервера (автоматически): " + new Date().toLocaleTimeString().slice(0, 5));
        return true;
      }
    }

    await githubGistRequest("PATCH", "https://api.github.com/gists/" + gistId, token, {
      files: {
        [SYNC_FILENAME]: { content: JSON.stringify(payload) },
        [SYNC_HISTORY_FILENAME]: { content: JSON.stringify(syncHistory) }
      }
    });
    markSyncedTo(payload.updatedAt, payload, "push");
    setSyncStatus("ok", "Синхронизировано: " + new Date().toLocaleTimeString().slice(0, 5));
    return true;
  } catch (e) {
    setSyncStatus("err", "Не удалось сохранить: " + e.message);
    logSyncFailure("push", payload, e.message);
    return false;
  } finally {
    syncInFlight = false;
    // Пока мы отправляли этот запрос, пришло ещё одно изменение — досылаем
    // его сейчас же.
    if (syncQueued) {
      syncQueued = false;
      pushToGistNow(manual);
    }
  }
}

function scheduleSync() {
  if (applyingRemote) return;          // это мы сами только что применили данные с сервера
  logLocalChange();                    // пишем правку в историю (работает и без синхронизации)
  markLocalChange();                   // запоминаем: на этом устройстве есть свежая правка
  if (!isAutoSyncActive()) return;     // ручной режим — отправляем только по кнопке
  if (syncTimer) clearTimeout(syncTimer);
  syncTimer = setTimeout(() => { syncTimer = null; pushToGistNow(); }, SYNC_DEBOUNCE_MS);
}

// Немедленно (без ожидания debounce) пытается отправить несохранённые правки.
// Используется, когда пользователь сворачивает/закрывает вкладку — лучше
// попытаться отправить сразу, чем ждать и потерять правки.
function flushSyncNow() {
  if (!isAutoSyncActive()) return;
  if (!isDirty()) return;
  if (syncTimer) { clearTimeout(syncTimer); syncTimer = null; }
  if (!syncInFlight) pushToGistNow();
}
document.addEventListener("visibilitychange", () => {
  if (document.visibilityState === "hidden") {
    flushSyncNow();
  } else if (document.visibilityState === "visible") {
    // Возврат в приложение: если не открыто ни одной модалки — тихо
    // подтягиваем актуальную версию с сервера. Подчинённое устройство делает
    // это всегда (даже если у него остались свои правки — сервер главнее),
    // главное — только если у него самого нет неотправленных правок. Если
    // же неотправленные правки есть (например, прошлая попытка не удалась
    // из-за сети) — пробуем отправить их снова, а не просто молчим до
    // следующей правки.
    const modalOpen = document.querySelector(".modal-overlay.open");
    if (isAutoSyncActive() && !syncInFlight && !modalOpen) {
      if (!isPrimaryDevice() || !isDirty()) pullFromGist(true);
      else pushToGistNow();
    }
  }
});
window.addEventListener("pagehide", flushSyncNow);
// Браузер сообщает о восстановлении связи (телефон был оффлайн) — если
// остались неотправленные правки, пробуем сразу же, не дожидаясь новой
// правки или следующего сворачивания вкладки.
window.addEventListener("online", () => {
  if (isAutoSyncActive() && isDirty() && !syncInFlight) pushToGistNow();
});

// Подчинённое устройство само периодически проверяет сервер, пока страница
// открыта — чтобы обновления с главного устройства подтягивались без
// необходимости сворачивать/открывать вкладку заново.
function setupSecondaryPolling() {
  if (secondaryPollTimer) { clearInterval(secondaryPollTimer); secondaryPollTimer = null; }
  if (!isAutoSyncActive() || isPrimaryDevice()) return;
  secondaryPollTimer = setInterval(() => {
    if (document.visibilityState !== "visible") return;
    if (syncInFlight || applyingRemote) return;
    const modalOpen = document.querySelector(".modal-overlay.open");
    if (modalOpen) return;
    pullFromGist(true);
  }, SECONDARY_POLL_MS);
}

function openSyncModal() {
  const { token, gistId } = getSyncConfig();
  document.getElementById("syncTokenInput").value = token;
  document.getElementById("syncGistIdInput").value = gistId;
  document.getElementById("syncPrimaryToggle").checked = isPrimaryDevice();
  const frozenToggle = document.getElementById("syncFrozenToggle");
  if (frozenToggle) frozenToggle.checked = isAutoSyncOff();
  openModal("syncOverlay");
}
function closeSyncModal() {
  closeModal("syncOverlay");
}
(function setupSyncBtnPress() {
  const btn = document.getElementById("syncBtn");
  let timer = null, longFired = false, startX = 0, startY = 0;
  function clearTimer() { if (timer) { clearTimeout(timer); timer = null; } }
  btn.addEventListener("pointerdown", (e) => {
    if (e.pointerType === "mouse" && e.button !== 0) return;
    longFired = false;
    startX = e.clientX; startY = e.clientY;
    clearTimer();
    timer = setTimeout(() => {
      longFired = true; timer = null;
      if (navigator.vibrate) navigator.vibrate(15);
      openSyncModalGated();
    }, LONG_PRESS_MS);
  });
  btn.addEventListener("pointermove", (e) => {
    if (!timer) return;
    if (Math.abs(e.clientX - startX) > LONG_PRESS_MOVE_TOLERANCE || Math.abs(e.clientY - startY) > LONG_PRESS_MOVE_TOLERANCE) clearTimer();
  });
  btn.addEventListener("pointerup", () => {
    const hadTimer = !!timer;
    clearTimer();
    if (hadTimer && !longFired) quickSyncAction();
  });
  btn.addEventListener("pointercancel", clearTimer);
  btn.addEventListener("contextmenu", (e) => e.preventDefault());
})();
document.getElementById("syncPwBtn").addEventListener("click", changeSyncPassword);
document.getElementById("syncClose").addEventListener("click", closeSyncModal);
document.getElementById("syncOverlay").addEventListener("click", (e) => {
  if (e.target.id === "syncOverlay") closeSyncModal();
});
document.getElementById("syncPrimaryToggle").addEventListener("change", (e) => {
  localStorage.setItem(SYNC_PRIMARY_KEY, e.target.checked ? "1" : "0");
  setupSecondaryPolling();
  updateGuestModeUI();
});
/* Переключатель «работать только на этом устройстве». Снимать заморозку
   стоит, только когда действительно нужны два устройства: сейчас загрузка с
   сервера применяется без сверки времени и может затереть свежие локальные
   правки (см. комментарий у SYNC_FROZEN_KEY). */
{
  const frozenToggle = document.getElementById("syncFrozenToggle");
  if (frozenToggle) {
    frozenToggle.addEventListener("change", (e) => {
      setAutoSyncOff(e.target.checked);
      setupSecondaryPolling();
      updateGuestModeUI();
      updateDirtyIndicator();
      if (e.target.checked) {
        setSyncStatus(isSyncConfigured() ? "idle" : "off", manualModeStatusText());
      } else if (isSyncConfigured()) {
        setSyncStatus("idle", "Автоматическая синхронизация включена: приложение будет само "
          + "отправлять правки и подтягивать версию с сервера.");
      } else {
        setSyncStatus("off", "Не настроено. Укажите токен, чтобы включить синхронизацию.");
      }
    });
  }
}
document.getElementById("syncSaveBtn").addEventListener("click", async () => {
  const token = document.getElementById("syncTokenInput").value.trim();
  const gistId = document.getElementById("syncGistIdInput").value.trim();
  if (!token) { alert("Укажите токен."); return; }
  localStorage.setItem(SYNC_TOKEN_KEY, token);
  if (gistId) localStorage.setItem(SYNC_GIST_ID_KEY, gistId);
  else localStorage.removeItem(SYNC_GIST_ID_KEY);
  // Если на этом устройстве есть несохранённые правки (например, только что
  // добавили позицию) — ни в коем случае не тянем данные с сервера первым
  // делом, это молча стёрло бы их. Раньше именно так и происходило: указан
  // gistId → сразу pullFromGist() поверх свежих локальных изменений.
  // Теперь: есть несохранённые правки → отправляем их (push сам покажет
  // модалку выбора, если на сервере тем временем что-то ещё изменилось).
  // Нечего терять (uже всё синхронизировано или это первое подключение
  // к существующему Gist) → можно спокойно подтянуть версию с сервера.
  // Нажатие кнопки — осознанное действие, поэтому обмен разрешён и в ручном
  // режиме (manual = true). Второе устройство при этом только ЗАБИРАЕТ список:
  // выгружать ему нечего и незачем, иначе оно могло бы затереть на сервере
  // список, собранный на главном.
  if (!isPrimaryDevice()) {
    if (gistId) await pullFromGist(false, false, true);
    else alert("Укажите ID Gist — второе устройство только загружает список с сервера.");
  } else if (gistId && !isDirty()) {
    const ok = await pullFromGist(false, false, true);
    if (!ok) await pushToGistNow(true);
  } else {
    await pushToGistNow(true);
  }
  setupSecondaryPolling();
  updateGuestModeUI();
});
/* Ручная загрузка. Если автоматика отказалась применять версию с сервера
   (она старше локальной), кнопка честно об этом спрашивает — на случай,
   когда сервер намеренно откатили и загрузить старую версию нужно. */
// Та же кнопка, что и в окне обмена, — поведение одно на оба места.
document.getElementById("syncPullBtn").addEventListener("click", manualPull);
document.getElementById("syncHistoryBtn").addEventListener("click", () => {
  renderSyncHistory();
  openModal("syncHistoryOverlay");
});
document.getElementById("syncHistoryClose").addEventListener("click", () => {
  closeModal("syncHistoryOverlay");
});
document.getElementById("syncHistoryOverlay").addEventListener("click", (e) => {
  if (e.target.id === "syncHistoryOverlay") closeModal("syncHistoryOverlay");
});
document.getElementById("syncForgetBtn").addEventListener("click", () => {
  if (!confirm("Отключить синхронизацию на этом устройстве? Токен и ID Gist будут забыты (сами данные в Gist останутся).")) return;
  localStorage.removeItem(SYNC_TOKEN_KEY);
  localStorage.removeItem(SYNC_GIST_ID_KEY);
  localStorage.removeItem(SYNC_PRIMARY_KEY);
  document.getElementById("syncTokenInput").value = "";
  document.getElementById("syncGistIdInput").value = "";
  document.getElementById("syncPrimaryToggle").checked = false;
  setSyncStatus("off", "Не настроено");
  updateDirtyIndicator();
  setupSecondaryPolling();
  updateGuestModeUI();
});

/* ==================== Выпадающее меню действий (⋮) — Экспорт/История ==================== */
document.getElementById("headMenuBtn").addEventListener("click", (e) => {
  e.stopPropagation();
  document.getElementById("headMenuPanel").classList.toggle("open");
});
document.querySelectorAll("#headMenuPanel button").forEach((btn) => {
  btn.addEventListener("click", () => document.getElementById("headMenuPanel").classList.remove("open"));
});
document.addEventListener("click", (e) => {
  const panel = document.getElementById("headMenuPanel");
  if (!panel.classList.contains("open")) return;
  if (e.target.closest("#headMenuPanel") || e.target.closest("#headMenuBtn")) return;
  panel.classList.remove("open");
});

/* ==================== Старт ==================== */
loadSortState();
reapplySavedSort();
renderAll();
renderWarehouseAll();
renderOneCAll();

updateDirtyIndicator();
// Точка отсчёта для журнала локальных правок. Ставится ПОСЛЕ загрузки данных
// и автопростановки моделей — иначе первое изменение сравнивать было бы не с
// чем, а сама миграция попала бы в историю как правка пользователя.
localLogLastState = snapshotForLog();
// Если при запуске автопростановка модели что-то изменила (например, впервые
// после обновления приложения) — помечаем это как локальную правку, чтобы
// очищенные названия и проставленные модели ушли на сервер, а не остались
// только на этом устройстве.
if (migrationTouchedAtStartup && isPrimaryDevice()) markLocalChange();
// Устройство, настроенное ДО появления ролей, считаем главным: именно на нём
// вы работали. Иначе после обновления оно молча стало бы «вторым» и перешло
// в режим просмотра — без панели добавления и кнопок +/-.
if (isSyncConfigured() && localStorage.getItem(SYNC_PRIMARY_KEY) === null) {
  try { localStorage.setItem(SYNC_PRIMARY_KEY, "1"); } catch (e) {}
}
if (isAutoSyncOff()) {
  setSyncStatus(isSyncConfigured() ? "idle" : "off", manualModeStatusText());
} else if (isSyncConfigured()) {
  if (!isPrimaryDevice()) {
    // Подчинённое устройство: сервер всегда главнее — просто подтягиваем.
    setSyncStatus("syncing", "Загрузка с сервера…");
    pullFromGist();
  } else if (isDirty()) {
    // На этом устройстве остались правки, которые не успели уйти на сервер
    // в прошлый раз (например, вкладку закрыли слишком быстро). Раньше в
    // этом месте всегда делался pullFromGist(), который бы их молча стёр —
    // теперь сначала пробуем их отправить (с проверкой конфликтов внутри).
    setSyncStatus("syncing", "Отправляем несохранённые изменения…");
    pushToGistNow();
  } else {
    setSyncStatus("syncing", "Загрузка с сервера…");
    pullFromGist();
  }
} else {
  setSyncStatus("off", "Не настроено. Нажмите на ⚙, чтобы включить синхронизацию.");
}
setupSecondaryPolling();
updateGuestModeUI();

/* ==================== Поля количества: авто-ширина + курсор в конец ====================
   Ширина поля растёт вслед за введённым числом (не обрезает 1000+), а при
   тапе по уже заполненному полю курсор сразу встаёт после последней цифры. */
const QTY_INPUT_SELECTOR = ".countInput, .whQtyInput, .ocQtyInput";
function autoSizeQtyInput(input) {
  const len = String(input.value || "").length;
  input.style.width = Math.max(len + 1.4, 2.4) + "ch";
}
document.addEventListener("input", (e) => {
  const input = e.target.closest(QTY_INPUT_SELECTOR);
  if (!input) return;
  autoSizeQtyInput(input);
});
document.addEventListener("focus", (e) => {
  const input = e.target.closest(QTY_INPUT_SELECTOR);
  if (!input) return;
  autoSizeQtyInput(input);
  const end = input.value.length;
  setTimeout(() => { try { input.setSelectionRange(end, end); } catch (err) {} }, 0);
}, true);
document.querySelectorAll(QTY_INPUT_SELECTOR).forEach(autoSizeQtyInput);
new MutationObserver((mutations) => {
  mutations.forEach((m) => {
    m.addedNodes.forEach((node) => {
      if (node.nodeType !== 1) return;
      if (node.matches && node.matches(QTY_INPUT_SELECTOR)) autoSizeQtyInput(node);
      if (node.querySelectorAll) node.querySelectorAll(QTY_INPUT_SELECTOR).forEach(autoSizeQtyInput);
    });
  });
}).observe(document.body, { childList: true, subtree: true });

/* ==================== Копирование кода товара по одному тапу ====================
   Один тап (или клик на ПК) на код детали/расходника сразу копирует его
   в буфер обмена — без выделения и без "—" у пустых значений. */
function copyCodeText(span) {
  if (!span) return;
  const text = span.getAttribute("data-code") || span.textContent || "";
  if (!text) return;
  const done = () => {
    span.classList.add("code-copied");
    setTimeout(() => span.classList.remove("code-copied"), 450);
  };
  if (navigator.clipboard && navigator.clipboard.writeText) {
    navigator.clipboard.writeText(text).then(done).catch(() => {
      fallbackCopy(text);
      done();
    });
  } else {
    fallbackCopy(text);
    done();
  }
}
function fallbackCopy(text) {
  const ta = document.createElement("textarea");
  ta.value = text;
  ta.style.position = "fixed";
  ta.style.opacity = "0";
  document.body.appendChild(ta);
  ta.select();
  try { document.execCommand("copy"); } catch (e) {}
  document.body.removeChild(ta);
}
document.addEventListener("click", (e) => {
  const span = e.target.closest(".code-text");
  if (!span) return;
  copyCodeText(span);
});


/* ==================== PWA: регистрация service worker ====================
   Нужно для установки на главный экран телефона и офлайн-работы.
   Работает только при открытии по https (или через GitHub Pages) —
   на file:// service worker браузером не регистрируется, но это не мешает
   основной работе программы, просто эта функция не активируется. */
/* Автообновление. Раньше здесь была только регистрация: новые файлы
   подхватывались лишь при полной перезагрузке страницы. На телефоне это
   происходило само (система постоянно выгружает приложение из памяти), а на
   рабочем столе окно PWA живёт неделями и не перезагружается — поэтому там
   обновление "не доезжало". Теперь сами спрашиваем сервер, не появилась ли
   новая версия, и перезагружаем страницу, когда она приехала. */
const SW_UPDATE_CHECK_MS = 30 * 60 * 1000;   // раз в полчаса, пока окно открыто

// Перезагружаем не мгновенно: если открыта модалка или пользователь что-то
// набирает — ждём, иначе перезагрузка выдернула бы форму из-под рук.
// Данные не теряются в любом случае (они пишутся в localStorage сразу), но
// прерывать ввод всё равно незачем.
function reloadForUpdateWhenSafe() {
  const tryReload = () => {
    if (document.querySelector(".modal-overlay.open") || isTypingNow()) {
      setTimeout(tryReload, 3000);
      return;
    }
    location.reload();
  };
  tryReload();
}

if ("serviceWorker" in navigator) {
  window.addEventListener("load", () => {
    // Смена управляющего service worker = приехала новая версия файлов.
    // Исключение — самая первая установка: там controllerchange срабатывает
    // от clients.claim(), хотя обновляться не с чего. Поэтому первый такой
    // сигнал только запоминаем, а перезагружаемся начиная со второго.
    let hasController = !!navigator.serviceWorker.controller;
    let reloading = false;

    navigator.serviceWorker.addEventListener("controllerchange", () => {
      if (!hasController) { hasController = true; return; }
      if (reloading) return;
      reloading = true;
      reloadForUpdateWhenSafe();
    });

    navigator.serviceWorker.register("sw.js").then((reg) => {
      const check = () => reg.update().catch(() => {});
      check();                                   // сразу при запуске
      setInterval(check, SW_UPDATE_CHECK_MS);    // и дальше периодически
      // Возврат к приложению — самый частый момент, когда стоит проверить.
      document.addEventListener("visibilitychange", () => {
        if (document.visibilityState === "visible") check();
      });
      window.addEventListener("focus", check);
    }).catch(() => {});
  });
}
